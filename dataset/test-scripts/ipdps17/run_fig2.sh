...
ALGO[1]="bfs"
OPTION[1]="--idempotence --src=largestdegree --taversal-mode=LB_CULL --queue-sizing=6.5 --in-sizing=4"
...
for k in {0..2}; do for partitioner in $PARTITIONERS; do for i in {0..2}; do for d in 1 4; do
    ...
    echo ${EXECUTION[$k]} ${GRAPH[$i]} $BASEOPTION ${OPTION[$k]} --partition-method=$partitioner --device=$DEVICE --do_a=${DO_A[$i]} --do_b=${DO_B[$i]} " > $LOGDIR/${ALGO[$k]}x$d.${NAME[$i]}${BASEFLAG}${FLAG[$k]}.$partitioner.txt 2>&1"
         ${EXECUTION[$k]} ${GRAPH[$i]} $BASEOPTION ${OPTION[$k]} --partition-method=$partitioner --device=$DEVICE --do_a=${DO_A[$i]} --do_b=${DO_B[$i]}   > $LOGDIR/${ALGO[$k]}x$d.${NAME[$i]}${BASEFLAG}${FLAG[$k]}.$partitioner.txt 2>&1
