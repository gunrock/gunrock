...
for i in {0..8}
do
    echo $EXEDIR/$EXECUTION ${GRAPH[$i]} $SETTING --tag=$TAG --device=$DEVICE --jsondir=./eval/ "> ./eval/${NAME[$i]}.$EXECUTION.output.txt"
         $EXEDIR/$EXECUTION ${GRAPH[$i]} $SETTING --tag=$TAG --device=$DEVICE --jsondir=./eval/  > ./eval/${NAME[$i]}.$EXECUTION.output.txt
    sleep 1
done
