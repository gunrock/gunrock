...
for i in {0..8}
do
    echo $EXEDIR/$EXECUTION ${GRAPH[$i]} $SETTING --tag=$TAG --device=$DEVICE --traversal-mode=${T_MODE[$i]} --do_a=${DO_A[$i]} --do_b=${DO_B[$i]} --jsondir=./eval/ "> ./eval/${NAME[$i]}.$EXECUTION.output.txt"
         $EXEDIR/$EXECUTION ${GRAPH[$i]} $SETTING --tag=$TAG --device=$DEVICE --traversal-mode=${T_MODE[$i]} --do_a=${DO_A[$i]} --do_b=${DO_B[$i]} --jsondir=./eval/  > ./eval/${NAME[$i]}.$EXECUTION.output.txt
    sleep 1
done
