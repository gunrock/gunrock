    ...
    CommandLineArgs &args)
{
    ...
    if (instrumented) {
            RunBCTests<VertexId, Value, SizeT, true>(
                graph,
                max_grid_size,
                num_gpus,
                max_queue_sizing);
    } else {
            RunBCTests<VertexId, Value, SizeT, false>(
                graph,
                max_grid_size,
                num_gpus,
                max_queue_sizing);
...
int main( int argc, char** argv)
{
	...
	if (graph_type == "market") {
		...
		char *market_filename1 = (graph_args == 3) ? argv[2] : NULL;
		char *market_filename2 = (graph_args == 3) ? argv[3] : NULL;
        ...
        Csr<VertexId, float, SizeT> csr2(false);    // CSR graph for computing BC
        if (graphio::BuildMarketGraph<false>(
            market_filename2,
            csr2,
            g_undirected) != 0)
