...
OPTION="--undirected --idempotence --src=largestdegree --traversal-mode=LB_CULL --direction-optimized --queue-sizing=6.5 --in-sizing=4 --iteration-num=10"
...
do
    ...
    for i in {0..14}
    do
        echo $EXECUTION market $DATADIR/${NAME[$i]}/${NAME[$i]}.mtx $OPTION --device=$DEVICE --do_a=${DO_A[$i]} --do_b=${DO_B[$i]} --jsondir=./eval/$SUFFIX "> ./eval/$SUFFIX/${NAME[$i]}${MARK}.txt"
             $EXECUTION market $DATADIR/${NAME[$i]}/${NAME[$i]}.mtx $OPTION --device=$DEVICE --do_a=${DO_A[$i]} --do_b=${DO_B[$i]} --jsondir=./eval/$SUFFIX > ./eval/$SUFFIX/${NAME[$i]}${MARK}.txt
        sleep 1
    done
done
