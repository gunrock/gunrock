...
cudaError_t UseParameters(util::Parameters &parameters)
{
    ...
    GUARD_CU(parameters.Use<std::string>(
        ...
        util::REQUIRED_ARGUMENT | util::MULTI_VALUE | util::OPTIONAL_PARAMETER,
        "All",
        "Memory access type, <Random | Regular | All>",
        __FILE__, __LINE__));
    ...
    GUARD_CU(parameters.Use<std::string>(
        ...
        util::REQUIRED_ARGUMENT | util::SINGLE_VALUE | util::OPTIONAL_PARAMETER,
        "All",
        "Operations to test, <Read | Write | Update | All>",
        __FILE__, __LINE__));
    ...
    GUARD_CU(parameters.Use<std::string>(
        ...
        util::REQUIRED_ARGUMENT | util::SINGLE_VALUE | util::OPTIONAL_PARAMETER,
        "All",
        "Test type, <Bandwidth | Latency | All>",
        __FILE__, __LINE__));
...
cudaError_t Test_BWL(
    ...
    util::Array1D<typename GraphT::SizeT, typename GraphT::VertexT> *gpu_elements,
    util::Array1D<typename GraphT::SizeT, typename GraphT::VertexT> *gpu_results,
    ArrayT  &host_elements,
    ArrayT  &host_results,
    ArrayT2 &all_elements,
    ArrayT2 &all_results,
    cudaStream_t *gpu_streams,
    int          **peer_accessables,
    float        **timings,
    cudaError_t   *retvals)
{
    ...
    for (int peer_offset = 0; peer_offset <= num_devices + 1; peer_offset++)
    {
        ...
        { do {
            ...
            if (peer_offset <= num_devices)
            {
                ...
                {
                    VertexT *sources = ((peer_offset == num_devices) ? 
                        (host_elements + 0) : peer_elements);
                    retval = results.ForAll(
                    [elements, sources, num_elements,
                    access_flag, num_repeats] 
                    __host__ __device__ (VertexT *result, const SizeT &pos)
                    {
                        for (int i = 0; i < num_repeats; i++)
                        {
                            ...
                            if (access_flag == RANDOM)
                                new_pos = elements[pos];
                            
                            result[pos] = sources[new_pos];
                        }
                    }, (bl_flag == LATENCY) ? 1 : for_size, util::DEVICE, stream, 1280);
                } else if (operation_flag == WRITE)
                {
                    VertexT *targets = ((peer_offset == num_devices) ?
                        (host_results + 0) : peer_results);
                    retval = results.ForAll(
                    [elements, targets, num_elements,
                    access_flag, num_repeats] 
                    __host__ __device__ (VertexT *result, const SizeT &pos)
                    {
                        for (int i = 0; i < num_repeats; i++)
                ...
                if (operation_flag == READ)
                {
                    ...
                    access_flag, num_repeats, all_element, num_devices] 
                    __host__ __device__ (VertexT *result, const SizeT &pos)
                    {
                        for (int i = 0; i < num_repeats; i++)
                        {
                ...
                {
                    retval = results.ForAll(
                    [elements, num_elements, all_result, num_devices,
                    access_flag, num_repeats] 
                    __host__ __device__ (VertexT *result, const SizeT &pos)
                    {
                        for (int i = 0; i < num_repeats; i++)
    ...
    for (int gpu = 0; gpu < num_devices; gpu++)
    {
        ...
        for (int peer = 0; peer <= num_devices + 1; peer++)
        {
            ...
            if (bl_flag == BANDWIDTH)
            {
                std::cout << 1.0 / 1024 / 1024 / 1024 
                    * for_size * num_repeats * sizeof(VertexT) / elapsed * 1000;
            }
...
struct main_struct
{
    ...
    cudaError_t operator()(util::Parameters &parameters, VertexT v, SizeT s, ValueT val)
    {
        ...
        std::vector<std::string> switches{"num-elements", "for-size", "num-repeats",
            "access-type", "operation", "bandwidth-latency"};
        ...
        auto num_elements = parameters.template Get<std::vector<SizeT>>("num-elements");
        SizeT max_elements = 0;
        for (auto num_element : num_elements)
            if (max_elements < num_element)
                max_elements = num_element;
        auto for_sizes = parameters.template Get<std::vector<SizeT>>("for-size");
        SizeT max_for_size = 0;
        for (auto for_size : for_sizes)
            if (max_for_size < for_size)
                max_for_size = for_size;
        ...
        for (SizeT i = 0; i < max_elements; i++)
        {
            ...
            if (host_elements[i] >= max_elements)
                host_elements[i] -= max_elements;
        }
        ...
        { do {
            ...
            if (thread_num == 0)
            {
                retval = util::GRError(cudaMemAdvise(
                    host_elements + 0, ((long long)max_elements) * sizeof(VertexT),
            ...
            if (retval) break;
            retval = util::GRError(cudaMemAdvise(
                host_results + 0, (long long)max_elements * sizeof(VertexT),
                cudaMemAdviseSetAccessedBy, device_idx),
                "cudaMemAdvise failed", __FILE__, __LINE__);
            if (retval) break;
            ...
            retval = results.Allocate(max(max_elements, max_for_size), util::DEVICE);
            if (retval) break;
            ...
            for (SizeT i = 0; i < max_elements; i++)
            {
                ...
                if (elements[i] >= max_elements)
                    elements[i] -= max_elements;
            }
            retval = elements.Move(util::HOST, util::DEVICE, 
                max_elements, 0, stream);
            if (retval) break;
            retval = util::GRError(cudaStreamSynchronize(stream),
        ...
        { 
            ...
            [devices, retvals, gpu_elements, gpu_results, host_elements, host_results,
            gpu_streams, peer_accessables, timings, all_elements, all_results]
            (util::Parameters &parameters, GraphT &graph)
            {
                return Test_BWL(parameters, graph,
                    gpu_elements, gpu_results, host_elements, host_results,
                    all_elements, all_results, gpu_streams, 
                    peer_accessables, timings, retvals);
            }));
        }
