 ...
 ******************************************************************************/
 ...
 { 
     ...
     std::stable_sort(hr_list, hr_list + nodes, HITSCompare<RankPair<SizeT, Value> >);
     std::stable_sort(ar_list, ar_list + nodes, HITSCompare<RankPair<SizeT, Value> >);
    ...
    CudaContext& context)
{
    ...
    typedef HITSProblem<
        ...
        // Copy out results
        util::GRError(csr_problem->Extract(h_hrank, h_arank), "HITS Problem Data Extraction Failed", __FILE__, __LINE__);
