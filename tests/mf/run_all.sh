...
OPTION[6]=${DEFAULT}" --source=35688 --sink=35689"
OPTION[7]=${DEFAULT}" --source=8922 --sink=8923"
...
NAME[6]="$LARGEDIR/larger/file0.mtx"
NAME[7]="$LARGEDIR/small_run1/file0.mtx"
...
do
    ...
    echo $EXCUTION market ${NAME[$k]} ${OPTION[$k]} "> eval/$SUFFIX/$k_$SUFFIX.txt"
    $EXCUTION market ${NAME[$k]} ${OPTION[$k]} > eval/$SUFFIX/$k_$SUFFIX.txt
    sleep 1
done
