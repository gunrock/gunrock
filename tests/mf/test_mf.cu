 ...
 * test_mf.cu
 *
 * @brief Simple test driver program for max flow problem.
 */
...
using namespace gunrock::app::mf;
...
/**
 * @brief Displays the max flow result: emount of flow stored in sink
 *
 * @tparam VertexId
 * @tparam VertexId
 *
...
/**
 * @brief A simple CPU-based reference MF implementation.
 *
 * @tparam VertexId
 * @tparam Value
 * @tparam SizeT
 ...
 * @param[in] src Source node where MF starts
...
cudaError_t RunTests(Info<VertexId, SizeT, Value> *info)
{
    ...
    if (retval = util::GRError(problem->Init(
        ...
        partition_factor,
        partition_seed),
        "MF Problem Init failed", __FILE__, __LINE__))
        return retval;
    ...
    for (int iter = 0; iter < iterations; ++iter)
    {
        ...
        if (retval = util::GRError(problem->Reset(
            ...
            "MF Problem Data Reset Failed", __FILE__, __LINE__))
            return retval;

        if (retval = util::GRError(enactor->Reset(),
            "MF Enactor Reset failed", __FILE__, __LINE__))
            return retval;
        ...
        if (retval = util::GRError(enactor->Enact(src, traversal_mode),
            "MF Problem Enact Failed", __FILE__, __LINE__))
            return retval;
        cpu_timer.Stop();
    ...
    // compute reference CPU MF solution for source-distance
    if (!quick_mode)
    {
...
int main_(CommandLineArgs *args)
{
    ...
    cpu_timer2.Start();
    info->Init("MF", *args, csr);  // initialize Info structure
