...
cudaError_t RunTests(Info<VertexId, SizeT, Value> *info)
{
    ...
    for (SizeT iter = 0; iter < iterations; ++iter)
    {
        if (retval = util::GRError(problem->Reset(
            enactor->GetFrontierType(), max_iteration, max_queue_sizing),
            "LP Problem Data Reset Failed", __FILE__, __LINE__))
            return retval;
        if (retval = util::GRError(enactor->Reset(),
            "LP Enactor Reset failed", __FILE__, __LINE__))
            return retval;
        ...
        if (retval = util::GRError(enactor->Enact(),
            "LP Problem Enact Failed", __FILE__, __LINE__))
            return retval;
        cpu_timer.Stop();
    ...
    if (retval = util::GRError(problem->Extract(h_component_ids),
        "LP Problem Data Extraction Failed", __FILE__, __LINE__))
        return retval;
