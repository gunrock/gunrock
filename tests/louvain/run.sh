...
OPTIONS="--edge-value-min=1 --edge-value-range=1 --device=2 --advance-mode=ALL_EDGES --quick --queue-factor=0.1"
OPTIONS="$OPTIONS --remove-self-loops=false --remove-duplicate-edges=false"
OPTIONS="$OPTIONS --pass-stats --iter-stats --unify-segments=0,1"
...
OPTIONS="$OPTIONS --omp-threads=16 --omp-runs=1 --num-runs=10"
