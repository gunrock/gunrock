 ...
 * test_ss.cu
 *
 * @brief Simple test driver program for scan statistics.
 */
...
struct main_struct
{
        ...
        VertexT v, SizeT s, ValueT val)
    {
        ...
        // compute reference CPU SS solution for source-distance
        if (!quick)
        {
        ...
        GUARD_CU(app::Switch_Parameters(parameters, graph, switches,
            ...
            {
                return app::ss::RunTests(parameters, graph, ref_distances);
            }));
...
{
    cudaError_t retval = cudaSuccess;
    util::Parameters parameters("test ss");
