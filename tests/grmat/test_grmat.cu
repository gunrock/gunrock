...
void Usage()
{
    printf(
        ...
        "Optional arguments:\n"
        "[--file_name=<file name>] If the graph needs to be saved to a file, else it will not be saved physically"
...
Tuple* RemoveSelfLoops(Tuple* coo, SizeT &final_edges, SizeT coo_edges)
{
    ...
    #pragma omp parallel
    {
        ...
        new_coo_arr[thread_num] = (Tuple*) malloc (sizeof(Tuple) * (edge_end - edge_start));
        ...
        )
{
    ...
    if ((!quiet) && (!self_loops))
    {
        printf("Converting %lld vertices, %lld %s edges ( %s tuples) "
                "to remove self loops...\n",
                (long long)coo_nodes, (long long)coo_edges, undirected ? "undirected" : "directed",
    ...
    if (!quiet)
    {
        printf ("Time Elapsed for sorting %s is %f ms\n\n", (self_loops == true)? "": "and removing self loops", cpu_timer.ElapsedMillis());
        printf ("Number of edges %lld, Number of rows %lld and Number of columns %lld\n\n", (long long)final_edges, (long long)rows, (long long)cols);
    }

    if(f != NULL)
    {
        fprintf (f, "%%MatrixMarket matrix coordinate pattern %s\n", (undirected == true)? "symmetric": "");
        fprintf (f, "%lld %lld %lld\n", (long long)rows, (long long)cols, (long long)final_edges);
        for (SizeT i = 0; i < final_edges; i++)
        {
            fprintf (f, "%lld %lld\n", (long long)(coo[i].row), (long long)(coo[i].col));
        }
    }
...
int main_(CommandLineArgs *args)
{
    ...
    cpu_timer2.Start();
    coo = (Coo<VertexId, Value> *)BuildRmatGraph_coo<true, VertexId, SizeT, Value>(rmat_nodes, rmat_edges, undirected, 
    ...
    { 
        ...
        {
            if (!quiet)
                printf ("Error: File path doesn't exist \n");
        }
        //Convert the COO format to Matrix Market format
        if (!quiet)
            printf ("Converting the COO format to Matrix Market format \n");
    }
    coo  = FromCoo_MM <VertexId>(f, coo, rmat_nodes, rmat_all_edges, false, undirected, quiet, self_loops);
    if (f != NULL)
    {
        fclose(f);
        if (!quiet)
            printf ("Converted the COO format to Matrix Market format\n");
    }
    cpu_timer.Stop();
    ...
    {
        if (!quiet)
            printf ("Error: Failed to create the Graph \n");
        return cudaErrorMemoryAllocation;
    }
    else
    {
