...
struct main_struct
{
    ...
    cudaError_t operator()(util::Parameters &parameters, VertexT v, SizeT s, ValueT val)
    {
        ...
        {
            util::PrintMsg("Directed graph is only supported by normalized PR,"
                " Changing graph type to undirected.", quiet);
        ...
        for (auto it = compensate_vec.begin(); it != compensate_vec.end(); it++)
        {
            ...
            GUARD_CU(app::Switch_Parameters(parameters, graph, switches,
                [quick, quiet](util::Parameters &parameters, GraphT &graph)
                {
                    ...
                    [ref_ranks, ref_vertices](
                        util::Parameters &parameters, GraphT &graph)
                    {
                        return app::pr::RunTests(
                            parameters, graph, ref_vertices, ref_ranks);
                    }));
                    ...
                    if (!quick)
                    {
                        for (int i = 0; i < num_srcs; i ++)
                        {
                            free(ref_ranks   [i]); ref_ranks   [i] = NULL;
                            free(ref_vertices[i]); ref_vertices[i] = NULL;
                        }
                        free(ref_ranks   ); ref_ranks    = NULL;
                        free(ref_vertices); ref_vertices = NULL;
                    }
                    return retval;
                }));
        }
