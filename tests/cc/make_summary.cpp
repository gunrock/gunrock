...
int main()
{
    ...
    ofstream Fout;
    string str,pre_str;
    string num_v,num_e,cpu_time,gpu_time,validity,cc;
    char ch;

    file_name=dir_name+"cc"+fil_name+".txt";
    Fout.open(file_name.c_str());
    Fout<<"No.\t"<<"Dataset\t"<<"Flag\t"<<"#Nodes\t"<<"#Edges\t";
    Fout<<"CPU Time (ms)\t"<<"GPU Time (ms)\t";
    Fout<<"Validity\t"<<"#CC";//<<"#Nodes visited\t"<<"#Edges visited\t"<<"rate (MiEdges/s)";
    Fout<<endl;
    ...
    for (int j=0;j<num_flags;j++)
    {
        ...
        while (!Fin.eof())
        {
                ...
                ch=='.' || ch=='(' || ch==')' || ch=='_' )
            {
                ...
                else if (pre_str=="msec." && validity=="") { validity =str;}
                else if (pre_str=="components" && cc=="") { cc=str;}
