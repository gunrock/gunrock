...
struct main_struct
{
        ...
        VertexT v, SizeT s, ValueT val)
    {
        ...
        // compute reference CPU TC solution for source-distance
        if (!quick)
        {            
        ...
        GUARD_CU(app::Switch_Parameters(parameters, graph, switches,
            [ref_tc_count](util::Parameters &parameters, GraphT &graph)
            {
                return app::tc::RunTests(parameters, graph, ref_tc_count);
            }));
...
{
    cudaError_t retval = cudaSuccess;
    util::Parameters parameters("test tc");
