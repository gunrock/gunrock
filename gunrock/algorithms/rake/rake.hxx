...
struct raking {
  ...
  auto log_segments_per_row =
      (no_padding) ? log_raking_threads :  // All raking threads (segments)
          util::limits::min(
              ...
              log_segments_per_bank_array);  // Up to as many segments per lane
                                             // (all lanes must have same amount
                                             // of padding to have constant lane
                                             // stride)
  ...
  auto lane_stride = (no_padding) ? partials_per_lane
  ...
  typedef typename util::meta::_if<
      ...
      raking<                  // Secondary grid
          ...
          util::math::log2(
              util::properties::maximum_threads_per_warp()),  // Raking threads
                                                              // (1 warp)
  ...
  static GUNROCK_HOST_DEVICE void print() {
    printf(
        ...
        partials_per_segment, partials_per_bank_array, segments_per_bank_array,
        no_padding, segments_per_row, partials_per_row, bank_padding_partials,
...
struct raking<grid::raking, util::meta::null_t> : grid::raking {
  ...
  GUNROCK_F_DEVICE raking(value_t* smem_pool)
      : smem_pool(smem_pool),
        lane_partial(grid::raking::my_lane_partial(
            smem_pool))  // set lane partial pointer
  {
