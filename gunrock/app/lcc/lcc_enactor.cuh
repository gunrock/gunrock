...
#include <gunrock/app/lcc/lcc_problem.cuh>
...
/**
 * @brief defination of LCC iteration loop
 * @tparam EnactorT Type of enactor
 */
template <typename EnactorT>
struct LCCIterationLoop : public IterationLoopBase<EnactorT, Use_FullQ | Push> {
  ...
  LCCIterationLoop() : BaseIterationLoop() {}

  /**
   * @brief Core computation of lcc, one iteration
  ...
  cudaError_t Core(int peer_ = 0) {
    ...
    auto intersect_op = [lcc_counts] __host__ __device__(
                            VertexT & comm_node, VertexT & edge) -> bool {
      atomicAdd(lcc_counts + comm_node, 1);
...
//    GUARD_CU2(cudaStreamSynchronize(oprtr_parameters.stream), "cudaStreamSync failed");
   ...
   * @tparam NUM_VERTEX_ASSOCIATES Number of data alccociated with each
   * transmition item, typed VertexT
   * @tparam NUM_VALUE__ASSOCIATES Number of data alccociated with each
  ...
  cudaError_t ExpandIncoming(SizeT &received_length, int peer_) {
    ...
    auto expand_op = [] __host__ __device__(
                         ...
                         VertexT *vertex_alccociate_ins,
                         ValueT *value__alccociate_ins) -> bool { return true; };
...
class Enactor
 ...
 public:
  ...
  typedef LCCIterationLoop<EnactorT> IterationT;
  ...
  /**
   * @brief LCCEnactor constructor
   */
  Enactor() : BaseEnactor("lcc"), problem(NULL) {
    this->max_num_vertex_associates = 0;
    this->max_num_value__associates = 1;
  }

  /**
   * @brief LCCEnactor destructor
   */
  virtual ~Enactor() {
    // Release();
  }
  ...
  /**
   * @brief one run of lcc, to be called within GunrockThread
  ...
  /**
   * @brief Enacts a LCC computing on the specified graph.
   * @param[in] src Source node to start primitive.
   * \return cudaError_t error melccage(s), if any
   */
  cudaError_t Enact() {
    ...
    util::PrintMsg("GPU LCC Done.", this->flag & Debug);
    return retval;
  }
