 ...
 */
template <typename Enactor, typename Functor, typename Iteration,
          int NUM_VERTEX_ASSOCIATES, int NUM_VALUE__ASSOCIATES>
void Iteration_Loop(ThreadSlice *thread_data) {
  ...
  util::CtaWorkProgressLifetime<SizeT> *work_progress =
      &(enactor->work_progress[thread_num * num_gpus]);
  ...
  FrontierAttribute<SizeT> *frontier_attribute_ = NULL;
  EnactorStats<SizeT> *enactor_stats_ = NULL;
  util::CtaWorkProgressLifetime<SizeT> *work_progress_ = NULL;
  util::Array1D<SizeT, SizeT> *scanned_edges_ = NULL;
  ...
  while (!Iteration::Stop_Condition(s_enactor_stats, s_frontier_attribute,
                                    s_data_slice, num_gpus)) {
    Total_Length = 0;
    ...
    if (num_gpus > 1 && enactor_stats[0].iteration != 0) {
      ...
      for (i = 1; i < num_gpus; i++) {
        ...
        frontier_attribute[i].current_label =
            frontier_attribute[0].current_label;
    ...
    if (num_gpus > 1) {
      if (enactor->problem->unified_receive) {
        ...
        data_slice->in_length_out.Move(util::HOST, util::DEVICE, 1, 0,
                                       streams[0]);
    ...
    // util::cpu_mt::PrintGPUArray<SizeT, VertexId>("labels", data_slice ->
    // labels.GetPointer(util::DEVICE), graph_slice -> nodes, thread_num,
    // iteration, -1, streams[0]);

    while (data_slice->wait_counter < num_gpus * 2 &&
           (!Iteration::Stop_Condition(s_enactor_stats, s_frontier_attribute,
                                       s_data_slice, num_gpus))) {
      ...
      for (peer__ = 0; peer__ < num_gpus * 2; peer__++) {
        ...
        if (enactor->debug && to_show[peer__]) {
          mssg = " ";
          mssg[0] = '0' + data_slice->wait_counter;
          ShowDebugInfo<Problem>(thread_num, peer__, frontier_attribute_,
                                 enactor_stats_, data_slice, graph_slice,
                                 work_progress_, mssg, streams[peer__]);
        }
        to_show[peer__] = true;
        ...
        switch (stages[peer__]) {
          case 0:                      // Assign marker & Scan
            ...
            if (peer__ == 0) {
              if (frontier_attribute_->queue_length ==
                  0) {  // empty local queue
                // Set_Record(data_slice, iteration, peer__, 3,
                // streams[peer__]);
                stages[peer__] = 4;
                break;
              } else {
            ...
            if (iteration == 0 &&
                peer__ > num_gpus) {  // first iteration, nothing to send
            ...
            if (peer__ < num_gpus) {  // wait and expand incoming
              if (!(s_data_slice[peer]
                        ->events_set[iteration_][gpu_ + num_gpus][0])) {
              ...
              if (frontier_attribute_->queue_length != 0) {
                if (enactor_stats_->retval = util::GRError(
                        cudaStreamWaitEvent(
                            streams[peer_],
                            s_data_slice[peer]
                                ->events[iteration_][gpu_ + num_gpus][0],
                            0),
                        "cudaStreamWaitEvent failed", __FILE__, __LINE__))
                  break;
              }
              data_slice->in_length[iteration % 2][peer_] = 0;
              s_data_slice[peer]->events_set[iteration_][gpu_ + num_gpus][0] =
                  false;

              if (frontier_attribute_->queue_length == 0) {
                // Set_Record(data_slice, iteration, peer__, 3,
                // streams[peer__]); printf(" %d\t %d\t %d\t Expand and subQ
              ...
              if (expand_latency != 0)
                util::latency::Insert_Latency(
                    expand_latency, frontier_attribute_->queue_length,
                    streams[peer_],
                    data_slice->latency_data.GetPointer(util::DEVICE));

              Iteration::template Expand_Incoming<NUM_VERTEX_ASSOCIATES,
                                                  NUM_VALUE__ASSOCIATES>(
                  enactor, streams[peer_],
                  data_slice->in_iteration[iteration % 2][peer_], peer_,
                  // data_slice -> in_length          [iteration%2],
                  received_length, frontier_attribute_->queue_length,
                  ...
                  data_slice->value__associate_orgs, data_slice,
                  enactor_stats_);
              // printf("%d, Expand, selector = %d, keys = %p\n",
              //    thread_num, selector^1, frontier_queue_ ->
              //    keys[selector^1].GetPointer(util::DEVICE));
              ...
              if (!Iteration::HAS_SUBQ) {
                ...
                Set_Record(data_slice, iteration, peer__, stages[peer__],
                           streams[peer__]);
                stages[peer__] = 1;
              }
            } else {  // Push Neighbor
              if (communicate_latency != 0)
                util::latency::Insert_Latency(
                    communicate_latency, data_slice->out_length[peer_],
                    streams[peer__],
                    data_slice->latency_data.GetPointer(util::DEVICE));

              PushNeighbor<Enactor, GraphSliceT, DataSlice,
                           NUM_VERTEX_ASSOCIATES, NUM_VALUE__ASSOCIATES>(
                  enactor, thread_num, peer, data_slice->out_length[peer_],
                  enactor_stats_,
                  ...
                  streams[peer__], communicate_multipy);
              Set_Record(data_slice, iteration, peer__, stages[peer__],
                         streams[peer__]);
              stages[peer__] = 4;
            }
          ...
          case 1:  // Comp Length
            if (peer_ != 0) {
              if (enactor_stats_->retval = Check_Record(
                      data_slice, iteration, peer_, stages[peer_] - 1,
                      stages[peer_], to_show[peer_]))
                break;
            ...
            if (enactor_stats_->retval = Iteration::Compute_OutputLength(
                    ...
                    scanned_edges_, graph_slice->nodes, graph_slice->edges,
                    context[peer_][0], streams[peer_],
            ...
            if (enactor->size_check ||
              ...
              Set_Record(data_slice, iteration, peer_, stages[peer_],
                         streams[peer_]);
            }
            stages[peer__] = 2;
            break;
            ...
            if (enactor->size_check ||
              ...
              if (enactor->size_check)
                Iteration::Check_Queue_Size(
                    enactor, thread_num, peer_,
                    frontier_attribute_->output_length[0] + 2, frontier_queue_,
                    frontier_attribute_, enactor_stats_, graph_slice);
              if (enactor_stats_->retval) break;
            }
            if (subqueue_latency != 0)
              util::latency::Insert_Latency(
                  subqueue_latency, frontier_attribute_->queue_length,
                  streams[peer_],
                  data_slice->latency_data.GetPointer(util::DEVICE));

            Iteration::SubQueue_Core(
                enactor, thread_num, peer_, frontier_queue_, scanned_edges_,
                frontier_attribute_, enactor_stats_, data_slice,
                s_data_slice[thread_num].GetPointer(util::DEVICE), graph_slice,
                &(work_progress[peer_]), context[peer_], streams[peer_]);
#ifdef ENABLE_PERFORMANCE_PROFILING
            h_nodes_queued[peer_] = enactor_stats_->nodes_queued[0];
            h_edges_queued[peer_] = enactor_stats_->edges_queued[0];
            enactor_stats_->nodes_queued.Move(util::DEVICE, util::HOST, 1, 0,
                                              streams[peer_]);
            enactor_stats_->edges_queued.Move(util::DEVICE, util::HOST, 1, 0,
                                              streams[peer_]);
#endif
          ...
          case 3:  // Copy
            ...
            if (!enactor->size_check && (enactor->debug || num_gpus > 1)) {
              if (Iteration::HAS_SUBQ) {
                if (enactor_stats_->retval =
                        Check_Size</*false,*/ SizeT, VertexId>(
                            ...
                            &frontier_queue_->keys[selector ^ 1], over_sized,
                            thread_num, iteration, peer_, false))
                  break;
              }
              ...
              util::MemsetCopyVectorKernel<<<256, 256, 0, streams[peer_]>>>(
                  data_slice->frontier_queues[num_gpus].keys[0].GetPointer(
                      util::DEVICE) +
                      Total_Length,
                  frontier_queue_->keys[selector].GetPointer(util::DEVICE),
                  frontier_attribute_->queue_length);
              if (problem->use_double_buffer)
                util::MemsetCopyVectorKernel<<<256, 256, 0, streams[peer_]>>>(
                    data_slice->frontier_queues[num_gpus].values[0].GetPointer(
                        util::DEVICE) +
                        Total_Length,
    ...
    if (!Iteration::Stop_Condition(s_enactor_stats, s_frontier_attribute,
                                   s_data_slice, num_gpus)) {
      ...
      if (enactor->debug) {
        printf(
            "%d\t %lld\t \t Subqueue finished. Total_Length= %lld, labels = "
            "%p\n",
      ...
      if (enactor->size_check && !enactor->problem->unified_receive) {
        if (enactor_stats[0].retval = Check_Size</*true,*/ SizeT, VertexId>(
          ...
          break;
        if (problem->use_double_buffer)
          if (enactor_stats[0].retval = Check_Size</*true,*/ SizeT, Value>(
                  true, "total_queue", Total_Length,
                  &data_slice->frontier_queues[0]
      ...
      frontier_queue_ =
          &(data_slice->frontier_queues[(enactor->size_check || num_gpus == 1)
                                            ? 0
                                            : num_gpus]);
      if (Iteration::HAS_FULLQ) {
        ...
        frontier_queue_ =
            &(data_slice->frontier_queues[(enactor->size_check || num_gpus == 1)
                                              ...
                                              : num_gpus]);
        scanned_edges_ =
            &(data_slice->scanned_edges[(enactor->size_check || num_gpus == 1)
                                            ? 0
                                            : num_gpus]);
        frontier_attribute_ = &(frontier_attribute[peer_]);
        enactor_stats_ = &(enactor_stats[peer_]);
        ...
        Iteration::FullQueue_Gather(
            ...
            frontier_attribute_, enactor_stats_, data_slice,
            s_data_slice[thread_num].GetPointer(util::DEVICE), graph_slice,
            work_progress_, context[peer_], streams[peer_]);
        selector = frontier_attribute[peer_].selector;
        if (enactor_stats_->retval) break;
        ...
        if (frontier_attribute_->queue_length != 0) {
          if (enactor->debug) {
            mssg = "";
            ShowDebugInfo<Problem>(thread_num, peer_, frontier_attribute_,
                                   enactor_stats_, data_slice, graph_slice,
                                   work_progress_, mssg, streams[peer_]);
          }
          ...
          enactor_stats_->retval = Iteration::Compute_OutputLength(
              ...
              frontier_queue_->keys[selector].GetPointer(util::DEVICE),
              scanned_edges_, graph_slice->nodes, graph_slice->edges,
              context[peer_][0], streams[peer_], gunrock::oprtr::advance::V2V,
              true, false, false);
          if (enactor_stats_->retval) break;
          ...
          if (fullqueue_latency != 0)
            util::latency::Insert_Latency(
                fullqueue_latency, frontier_attribute_->queue_length,
                streams[peer_],
                data_slice->latency_data.GetPointer(util::DEVICE));
          ...
          while (tretval == cudaErrorNotReady) {
            tretval = cudaStreamQuery(streams[peer_]);
            if (tretval == cudaErrorNotReady) sleep(0);
          }
          if (enactor_stats_->retval = util::GRError(
                  tretval,
                  "FullQueue_Core failed. If size-check is disabled, "
                  "please try to increase the queue-sizing",
                  __FILE__, __LINE__))
            break;
...
#endif
          if (!enactor->size_check) {
            if (enactor_stats_->retval = Check_Size</*false,*/ SizeT, VertexId>(
      ...
      if (num_gpus > 1) {
        ...
        Iteration::Iteration_Update_Preds(
            enactor, graph_slice, data_slice, &frontier_attribute[0],
            &data_slice->frontier_queues[enactor->size_check ? 0 : num_gpus],
            Total_Length, streams[0]);

        if (makeout_latency != 0)
          util::latency::Insert_Latency(
              makeout_latency, Total_Length, streams[0],
              data_slice->latency_data.GetPointer(util::DEVICE));

        Iteration::template Make_Output<NUM_VERTEX_ASSOCIATES,
                                        NUM_VALUE__ASSOCIATES>(
            enactor, thread_num, Total_Length, num_gpus,
            &data_slice->frontier_queues[enactor->size_check ? 0 : num_gpus],
            &data_slice->scanned_edges[0], &frontier_attribute[0],
            enactor_stats, &problem->data_slices[thread_num], graph_slice,
            &work_progress[0], context[0], streams[0]);
      } else {
 ...
 */
template <typename _AdvanceKernelPolicy, typename _FilterKernelPolicy,
          typename _Enactor, bool _HAS_SUBQ, bool _HAS_FULLQ, bool _BACKWARD,
          bool _FORWARD, bool _UPDATE_PREDECESSORS>
struct IterationBase {
 public:
   ...
   * @param[in] stream CUDA stream.
   */
  static void SubQueue_Core(Enactor *enactor, int thread_num, int peer_,
                            Frontier *frontier_queue,
                            util::Array1D<SizeT, SizeT> *scanned_edges,
                            FrontierAttribute<SizeT> *frontier_attribute,
                            EnactorStats<SizeT> *enactor_stats,
                            DataSlice *data_slice, DataSlice *d_data_slice,
  ...
  static void Iteration_Update_Preds(
    ...
    if (Problem::MARK_PREDECESSORS && UPDATE_PREDECESSORS && num_elements > 0) {
      Copy_Preds<VertexId, SizeT><<<grid_size, 256, 0, stream>>>(
          num_elements, frontier_queue->keys[selector].GetPointer(util::DEVICE),
  ...
  static void Check_Queue_Size(Enactor *enactor, int thread_num, int peer_,
    ...
    if (enactor->debug)
      printf("%d\t %d\t %d\t queue_length = %d, output_length = %d\n",
             ...
             frontier_queue->keys[selector ^ 1].GetSize(), request_length);
    fflush(stdout);
    ...
    if (enactor_stats->retval = Check_Size</*true,*/ SizeT, VertexId>(
            ...
            over_sized, thread_num, iteration, peer_, true))
      return;
  ...
  static void Old_Make_Output(
      ...
      FrontierAttribute<SizeT> *frontier_attribute,
      EnactorStats<SizeT> *enactor_stats,
      util::Array1D<SizeT, DataSlice> *data_slice_, GraphSliceT *graph_slice,
      util::CtaWorkProgressLifetime<SizeT> *work_progress, ContextPtr context,
      cudaStream_t stream) {
    ...
    for (peer_ = 0; peer_ < num_gpus; peer_++) {
      if (enactor_stats->retval = Check_Size<SizeT, SizeT>(
              enactor->size_check, "keys_marker", num_elements,
              &data_slice->keys_marker[peer_], over_sized, thread_num,
    ...
    if (over_sized)
      data_slice->keys_markers.Move(util::HOST, util::DEVICE, num_gpus, 0,
                                    stream);
    ...
    for (t = 0; t < 2; t++) {
      ...
      for (peer_ = 0; peer_ < num_gpus; peer_++) {
        Scan<mgpu::MgpuScanTypeInc>(
            ...
            num_elements, (SizeT)0, mgpu::plus<SizeT>(), (SizeT *)0, (SizeT *)0,
            (SizeT *)data_slice->keys_marker[peer_].GetPointer(util::DEVICE),
            context[0]);
      }
      ...
      if (num_elements > 0)
        ...
        for (peer_ = 0; peer_ < num_gpus; peer_++) t_out_length[peer_] = 0;
      }
      ...
      for (peer_ = 0; peer_ < num_gpus; peer_++) {
        ...
        if (keys_over_sized)
          data_slice->keys_outs[peer_] =
              peer_ == 0 ? data_slice->frontier_queues[0]
                               .keys[selector ^ 1]
                               .GetPointer(util::DEVICE)
        ...
        for (i = 0; i < NUM_VERTEX_ASSOCIATES; i++) {
          if (enactor_stats[0].retval = Check_Size<SizeT, VertexId>(
                  enactor->size_check, "vertex_associate_outs",
                  data_slice->out_length[peer_] + t_out_length[peer_],
                  &data_slice->vertex_associate_out[peer_][i], over_sized,
                  thread_num, enactor_stats->iteration, peer_),
              data_slice->out_length[peer_] == 0 ? false : true)
            break;
          if (over_sized)
            data_slice->vertex_associate_outs[peer_][i] =
                data_slice->vertex_associate_out[peer_][i].GetPointer(
                    util::DEVICE);
        }
        ...
        for (i = 0; i < NUM_VALUE__ASSOCIATES; i++) {
          if (enactor_stats->retval = Check_Size<SizeT, Value>(
                  ...
                  data_slice->out_length[peer_] + t_out_length[peer_],
                  &data_slice->value__associate_out[peer_][i], over_sized,
                  thread_num, enactor_stats->iteration, peer_,
                  data_slice->out_length[peer_] == 0 ? false : true))
            break;
          if (over_sized)
            data_slice->value__associate_outs[peer_][i] =
                data_slice->value__associate_out[peer_][i].GetPointer(
                    util::DEVICE);
        }
        if (enactor_stats->retval) break;
        if (over_sized)
      ...
      if (enactor_stats->retval) break;
      if (keys_over_sized)
        data_slice->keys_outs.Move(util::HOST, util::DEVICE, num_gpus, 0,
                                   stream);
      ...
      memcpy(&(data_slice->make_out_array[offset]),
             ...
             sizeof(SizeT) * num_gpus);
      offset += sizeof(SizeT) * num_gpus;
      data_slice->make_out_array.Move(util::HOST, util::DEVICE, offset, 0,
                                      stream);

      if (BACKWARD && t == 1)
        Make_Out_Backward<VertexId, SizeT, Value, NUM_VERTEX_ASSOCIATES,
                          NUM_VALUE__ASSOCIATES>
            <<<grid_size, block_size, sizeof(char) * offset, stream>>>(
                ...
                offset, data_slice->make_out_array.GetPointer(util::DEVICE));
      else if (FORWARD && t == 0)
        Make_Out<VertexId, SizeT, Value, NUM_VERTEX_ASSOCIATES,
                 NUM_VALUE__ASSOCIATES>
            <<<grid_size, block_size, sizeof(char) * offset, stream>>>(
                num_elements, num_gpus,
                frontier_queue->keys[selector].GetPointer(util::DEVICE),
                graph_slice->partition_table.GetPointer(util::DEVICE),
                graph_slice->convertion_table.GetPointer(util::DEVICE), offset,
  ...
  static void Make_Output(Enactor *enactor, int thread_num, SizeT num_elements,
    ...
    for (peer_ = 0; peer_ < num_gpus; peer_++) {
      if (enactor_stats->retval = Check_Size<SizeT, VertexId>(
              enactor->size_check, "keys_out", num_elements * size_multi,
              (peer_ == 0) ? &data_slice->frontier_queues[0].keys[selector ^ 1]
      ...
      data_slice->keys_outs[peer_] =
          (peer_ == 0)
              ? data_slice->frontier_queues[0].keys[selector ^ 1].GetPointer(
                    util::DEVICE)
              : data_slice->keys_out[peer_].GetPointer(util::DEVICE);
      if (peer_ == 0) continue;
      ...
      if (enactor_stats[0].retval = Check_Size<SizeT, VertexId>(
              ...
              &data_slice->vertex_associate_out[peer_], over_sized, thread_num,
              enactor_stats->iteration, peer_),
          false)
        break;
      // if (over_sized)
      ...
      if (enactor_stats->retval = Check_Size<SizeT, Value>(
              ...
              &data_slice->value__associate_out[peer_], over_sized, thread_num,
    ...
    data_slice->vertex_associate_outs.Move(util::HOST, util::DEVICE, num_gpus,
                                           0, stream);
    data_slice->value__associate_outs.Move(util::HOST, util::DEVICE, num_gpus,
                                           0, stream);
    ...
    if (!enactor->problem->skip_makeout_selection) {
      ...
      data_slice->out_length.Move(util::HOST, util::DEVICE, num_gpus, 0,
                                  stream);
      ...
      if (FORWARD)
        Make_Output_Kernel<VertexId, SizeT, Value, NUM_VERTEX_ASSOCIATES,
                           NUM_VALUE__ASSOCIATES,
      ...
      if (BACKWARD)
        Make_Output_Backward_Kernel<
            VertexId, SizeT, Value, NUM_VERTEX_ASSOCIATES,
            NUM_VALUE__ASSOCIATES, AdvanceKernelPolicy::CUDA_ARCH,
            AdvanceKernelPolicy::LOG_THREADS - 1>
            <<<num_blocks, AdvanceKernelPolicy::THREADS / 2, 0, stream>>>(
                num_elements, num_gpus,
                data_slice->out_length.GetPointer(util::DEVICE),
      ...
      data_slice->out_length.Move(util::DEVICE, util::HOST, num_gpus, 0,
                                  stream);
      frontier_attribute->selector ^= 1;
    } else {
      Make_Output_Kernel_SkipSelection<
          VertexId, SizeT, Value, NUM_VERTEX_ASSOCIATES, NUM_VALUE__ASSOCIATES,
          AdvanceKernelPolicy::CUDA_ARCH, AdvanceKernelPolicy::LOG_THREADS>
    ...
    if (enactor_stats->retval =
            util::GRError(cudaStreamSynchronize(stream), "Make_Output failed",
                          __FILE__, __LINE__))
      return;
    if (!enactor->problem->skip_makeout_selection) {
