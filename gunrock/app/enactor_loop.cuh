    ...
    while (!iteration.Stop_Condition(gpu_num))
    {
        ...
        if (num_gpus > 1)
        {
            ...
            if (IterationT::FLAG & Unified_Receive)
            {
                ...
                if (enactor_stats0.retval = util::GRError(cudaStreamSynchronize(stream0),
        ...
        for (int peer = 0; peer < num_gpus; peer++)
        {
            ...
            for (int i = 0; i < mgpu_slice.num_stages; i++)
                mgpu_slice.events_set[enactor_stats0.iteration%4][peer][i]
                    = false;
        }
           ...
           && (!iteration.Stop_Condition(gpu_num)))
        {
            ...
            for (int peer__ = 0; peer__ < num_gpus * 2; peer__++)
            {
                ...
                auto &stream        = (peer__ <= num_gpus) ?
                    enactor_slice.stream : enactor_slice.stream2;
                ...
                case 0: // Assign marker & Scan
                    ...
                    { //wait and expand incoming
                        if (!(mgpu_slices[peer].events_set[iteration_num_][gpu_ + num_gpus][0]))
                        {
                            to_show = false; break;
                        }
                        ...
                        if (frontier.queue_length != 0)
                        {
                            if (retval = util::GRError(cudaStreamWaitEvent(stream,
                                mgpu_slices[peer].events
                                    [iteration_num_][gpu_ + num_gpus][0],0),
                    ...
                    if (peer__ > num_gpus)
                    {
                        ...
                        PushNeighbor <Enactor,
                            NUM_VERTEX_ASSOCIATES, NUM_VALUE__ASSOCIATES> (
                            enactor, gpu_num, peer);
                        SetRecord(mgpu_slice, iteration_num, peer__, stage, stream);
                        stage = 4; break;
                    }
                    break;
                ...
                case 1: //Comp Length
                    if (peer_ != 0)
                    {
                        if (retval = CheckRecord(
                            mgpu_slice, iteration_num, peer_, stage-1, stage, to_show))
                            break;
                        if (!to_show) break;
                        frontier.queue_length = mgpu_slice.in_length_out[peer_];
                    }
                    if (retval = iteration.Compute_OutputLength(peer_))
                        break;
                ...
                case 3: //Copy
                        ...
                        ((enactor.flag & Debug)!=0 || num_gpus > 1))
                    {
                        ...
                        {
                            if (retval =
                                CheckSize<SizeT, VertexT> (
                                    false, "queue3",
                                    frontier.output_length[0] + 2,
                                    frontier.Next_V_Q(),
                                    over_sized, gpu_num, iteration_num, peer_, false))
                                break;
                        }
                        if (frontier.queue_length == 0)
                            break;
                        ...
                        if (retval = CheckSize<SizeT, VertexT>
                                ...
                                enactor_slices[num_gpus].frontier.V_Q(),
                                over_sized, gpu_num, iteration_num, peer_, false))
                            break;
        ...
        if (!iteration.Stop_Condition(gpu_num))
        {
                ...
                !iteration.Stop_Condition(gpu_num))
            {
                for (int peer_ = 0; peer_ < num_gpus; peer_++)
                {
                    ...
                    {
                        enactor_slices[peer_ % num_gpus].enactor_stats.retval
                            = tretval;
                        break;
                    }
                }
            ...
            {
                bool over_sized = false;
                if (enactor_stats0.retval =
                    CheckSize<SizeT, VertexT>(true, "total_queue",
                        total_length, frontier0.V_Q(),
                        over_sized, gpu_num, enactor_stats0.iteration, num_gpus, true))
                    break;
                //if (problem -> use_double_buffer)
                ...
                //            over_sized, thread_num, enactor_stats0.iteration, num_gpus, true))
                //        break;
                ...
                if (enactor_slices[peer_].frontier.queue_length !=0)
                {
                    ...
                    frontier0.V_Q()->ForAll(frontier.V_Q()[0],
                        [offset]__host__ __device__
                        (VertexT* key0, VertexT *key1, const SizeT &pos)
                        {
                            key0[pos + offset] = key1[pos];
                        }, frontier.queue_length,
                        util::LOCATION_DEFAULT, stream0);
            ...
            {
                int peer_ = ((enactor.flag & Size_Check) != 0 || num_gpus == 1) ? 0 : num_gpus;
                auto &enactor_slice = enactor_slices[peer_];
                auto &frontier      = enactor_slice.frontier;
                auto &enactor_stats = enactor_slice.enactor_stats;
                ...
                if (frontier.queue_length !=0)
                {
                    ...
                    {
                        bool over_sized = false;
                        if (retval = CheckSize<SizeT, VertexT> (false, "queue3",
                                frontier.output_length[0] + 2,
                                frontier.Next_V_Q(),
                                over_sized, gpu_num, iteration_num, peer_, false))
                            break;
            ...
            if (num_gpus > 1)
            {
                    ...
                    !iteration.Stop_Condition(gpu_num))
                {
                    for (int peer_ = num_gpus + 1; peer_ < num_gpus*2; peer_++)
                    {
                        if (peer_ == num_gpus || mgpu_slice.wait_marker[peer_]!=0)
                            continue;
                        cudaError_t tretval = cudaStreamQuery(enactor_slices[peer_ - num_gpus].stream2);
                        if (tretval == cudaSuccess)
                        {
                            mgpu_slice.wait_marker[peer_] = 1;
                            wait_count++;
                ...
                iteration.template MakeOutput
                    <NUM_VERTEX_ASSOCIATES, NUM_VALUE__ASSOCIATES> (total_length);
            }
            else
            {
                mgpu_slice.out_length[0] = total_length;
