 ...
 * indexing. (e.g., uint32)
 * @tparam Problem             Problem data type which contains data slice for
 * WTF problem
 * @tparam _LabelT             Vertex label type
 *
 */
template <typename VertexId, typename SizeT, typename Value,
          typename ProblemData, typename _LabelT = VertexId>
struct PRFunctor {
  ...
  static __device__ __forceinline__ bool CondEdge(
    ...
    return (d_data_slice->out_degrees[d_id] > 0 &&
            d_data_slice->out_degrees[s_id] > 0);
  }
  ...
  static __device__ __forceinline__ void ApplyEdge(
      VertexId s_id, VertexId d_id, DataSlice *d_data_slice, SizeT edge_id,
      VertexId input_item, LabelT label, SizeT input_pos, SizeT &output_pos) {
    atomicAdd(&d_data_slice->rank_next[d_id],
              d_data_slice->rank_curr[s_id] / d_data_slice->out_degrees[s_id]);
  }
  ...
  static __device__ __forceinline__ bool CondFilter(VertexId v, VertexId node,
    ...
    problem->rank_next[node] =
        (delta * problem->rank_next[node]) +
...
struct HUBFunctor {
   ...
   */
  static __device__ __forceinline__ void ApplyEdge(
      VertexId s_id, VertexId d_id, DataSlice *problem, SizeT edge_id,
      VertexId input_item, LabelT label, SizeT input_pos, SizeT &output_pos) {
    Value val =
        (s_id == problem->src_node ? problem->alpha / problem->out_degrees[s_id]
                                   : 0) +
        (1 - problem->alpha) * problem->refscore_curr[d_id] /
            problem->in_degrees[d_id];
...
struct AUTHFunctor {
  ...
  static __device__ __forceinline__ void ApplyEdge(
      ...
      VertexId input_item, LabelT label, SizeT input_pos, SizeT &output_pos) {
    Value val =
        problem->rank_curr[s_id] /
        (problem->out_degrees[s_id] > 0 ? problem->out_degrees[s_id] : 1.0);
