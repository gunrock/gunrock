...
// false> // KEEP_NODE_NUM
{
  ...
  struct DataSlice : BaseDataSlice {
    ...
    util::Array1D<SizeT, SizeT>
        out_degrees; /**< Used for keeping out-degree for each vertex */
    ...
    cudaError_t Init(int num_gpus, int gpu_idx, bool use_double_buffer,
                     ...
                     SizeT *num_out_nodes, float queue_sizing = 2.0,
                     float in_sizing = 1.0) {
     ...
     */
    cudaError_t Reset(
        VertexId src, Value delta, Value alpha, Value threshold,
        FrontierType frontier_type,  // type (i.e., edge / vertex / mixed)
        GraphSlice<VertexId, SizeT, Value> *graph_slice,
        double queue_sizing = 1.0, bool use_double_buffer = false,
        double queue_sizing1 = -1.0, bool skip_scanned_edges = false) {
      cudaError_t retval = cudaSuccess;
      ...
      util::MemsetKernel<<<128, 128>>>(rank_next.GetPointer(util::DEVICE),
                                       (Value)0.0, nodes);

      util::MemsetKernel<<<128, 128>>>(rank_curr.GetPointer(util::DEVICE),
                                       (Value)1.0 / nodes, nodes);

      util::MemsetKernel<<<128, 128>>>(refscore_curr.GetPointer(util::DEVICE),
                                       (Value)0.0, nodes);

      util::MemsetKernel<<<128, 128>>>(refscore_next.GetPointer(util::DEVICE),
                                       (Value)0.0, nodes);

      // Compute degrees
      util::MemsetKernel<<<128, 128>>>(out_degrees.GetPointer(util::DEVICE),
                                       (SizeT)0, nodes);
      ...
      util::MemsetKernel<<<128, 128>>>(in_degrees.GetPointer(util::DEVICE),
                                       (SizeT)0, nodes);

      // util::MemsetMadVectorKernel<<<128, 128>>>(
      //    in_degrees   .GetPointer(util::DEVICE),
      //    graph_slice -> column_offsets.GetPointer(util::DEVICE),
      //    graph_slice -> column_offsets.GetPointer(util::DEVICE) +1, -1,
      //    nodes);

      util::MemsetIdxKernel<<<128, 128>>>(node_ids.GetPointer(util::DEVICE),
                                          nodes);

      util::MemsetKernel<<<128, 128>>>(cot_map.GetPointer(util::DEVICE), false,
                                       nodes);

      // Fillin the initial input_queue for WTF problem, this needs to be
      // modified in multi-GPU scene Put every vertex in there
  ...
  cudaError_t Extract(Value *h_rank, VertexId *h_node_id) {
    ...
    if (this->num_gpus == 1) {
      ...
      if (retval =
              data_slices[gpu]->refscore_curr.Move(util::DEVICE, util::HOST))
        return retval;
   ...
   * @param[in] inv_graph Reference to the CSC graph object we process on. @see
   * Csr
  ...
  cudaError_t Init(bool stream_from_host,  // Only meaningful for single-GPU
    ...
    cudaError_t retval = cudaSuccess;
    if (retval = BaseProblem::Init(stream_from_host, graph, inv_graph, num_gpus,
                                   gpu_idx, partition_method, queue_sizing,
  ...
  cudaError_t Reset(VertexId src, Value delta, Value alpha, Value threshold,
                    FrontierType frontier_type,  // The frontier type (i.e.,
                                                 // edge/vertex/mixed)
                    double queue_sizing = 1.0, double queue_sizing1 = -1.0) {
    cudaError_t retval = cudaSuccess;
    ...
    for (int gpu = 0; gpu < this->num_gpus; ++gpu) {
      ...
      if (retval = data_slices[gpu]->Reset(
              src, delta, alpha, threshold, frontier_type,
              this->graph_slices[gpu], queue_sizing, queue_sizing1))
        return retval;
