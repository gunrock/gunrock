...
class WTFEnactor
 ...
 public:
   ...
   */
  WTFEnactor(int num_gpus = 1, int* gpu_idx = NULL, bool instrument = false,
  ...
  void NormalizeRank(int hub_or_auth, cudaStream_t stream = 0) {
    ...
    if (hub_or_auth == 0) {
      ...
      rank_curr =
          problem->data_slices[0]->refscore_curr.GetPointer(util::DEVICE);
      rank_next =
          problem->data_slices[0]->refscore_next.GetPointer(util::DEVICE);
      // printf("auth\n");
    }
    ...
    util::MemsetKernel<<<128, 128, 0, stream>>>(rank_next, (Value)0.0,
                                                this->problem->nodes);
  ...
  cudaError_t InitWTF(ContextPtr* context, Problem* problem,
                      int max_grid_size = 0) {
   ...
   */
  template <typename AdvanceKernelPolicy, typename FilterKernelPolicy>
  cudaError_t EnactWTF(VertexId src, Value alpha, SizeT max_iteration) {
    ...
    typedef COTFunctor<VertexId, SizeT, Value, Problem> CotFunctor;
    ...
    typename Problem::DataSlice* d_data_slice =
        problem->data_slices[0].GetPointer(util::DEVICE);
    ...
    while (frontier_attribute->queue_length > 0) {
      ...
      if (this->debug) {
        if (retval = util::GRError(cudaStreamSynchronize(stream),
                                   "edge_map_forward::Kernel failed", __FILE__,
                                   __LINE__))
          return retval;
      ...
      gunrock::oprtr::filter::LaunchKernel<FilterKernelPolicy, Problem,
                                           PrFunctor>(
          enactor_stats[0], frontier_attribute[0], (VertexId)0, data_slice,
          d_data_slice, (SizeT*)NULL,
          ...
          enactor_stats->filter_kernel_stats, true, false);
      ...
      if (retval = work_progress->GetQueueLength(
        ...
        return retval;
      if (retval = util::GRError(cudaStreamSynchronize(stream),
                                 "filter_forward::Kernel failed", __FILE__,
                                 __LINE__))
        return retval;
      ...
      util::MemsetCopyVectorKernel<<<128, 128, 0, stream>>>(
          ...
          data_slice->rank_next.GetPointer(util::DEVICE), nodes);
      util::MemsetKernel<<<128, 128, 0, stream>>>(
          data_slice->rank_next.GetPointer(util::DEVICE), (Value)0.0, nodes);
    ...
    util::CUBRadixSort<Value, VertexId>(
        false, nodes, data_slice->rank_curr.GetPointer(util::DEVICE),
    ...
    // neighbors. 2 compute atomicAdd their neighbors' incoming node number.
    ...
    // if (retval = work_progress.SetQueueLength(frontier_attribute.queue_index,
    // cot_size)) break;

    // Edge Map
    gunrock::oprtr::advance::LaunchKernel<
        AdvanceKernelPolicy, Problem, CotFunctor, gunrock::oprtr::advance::V2V>(
        enactor_stats[0], frontier_attribute[0], typename CotFunctor::LabelT(),
        data_slice, d_data_slice, (VertexId*)NULL, (bool*)NULL, (bool*)NULL,
        d_scanned_edges,
        ...
        (Value*)NULL, graph_slice->row_offsets.GetPointer(util::DEVICE),
        graph_slice->column_indices.GetPointer(util::DEVICE), (SizeT*)NULL,
    ...
    util::MemsetKernel<<<128, 128, 0, stream>>>(
        data_slice->rank_curr.GetPointer(util::DEVICE), (Value)0.0, nodes);

    util::MemsetKernel<<<128, 128, 0, stream>>>(
        data_slice->refscore_curr.GetPointer(util::DEVICE), (Value)0.0, nodes);

    util::MemsetKernel<<<128, 128, 0, stream>>>(
        data_slice->refscore_next.GetPointer(util::DEVICE), (Value)0.0, nodes);
    ...
    if (retval = util::GRError(
            cudaMemcpy(data_slice->rank_curr.GetPointer(util::DEVICE) + src,
    ...
    util::MemsetIdxKernel<<<128, 128, 0, stream>>>(
        data_slice->node_ids.GetPointer(util::DEVICE), nodes);
    ...
    util::CUBRadixSort<Value, VertexId>(
        false, nodes, data_slice->refscore_curr.GetPointer(util::DEVICE),
        data_slice->node_ids.GetPointer(util::DEVICE));
  ...
  cudaError_t Init(ContextPtr* context, Problem* problem,
    ...
    if (min_sm_version >= 300) {
      return InitWTF<AdvanceKernelPolicy, FilterKernelPolicy>(context, problem,
                                                              max_grid_size);
    }
   ...
   * calls.
   */
  cudaError_t Enact(VertexId src, Value alpha, SizeT max_iteration) {
    int min_sm_version = -1;
    for (int i = 0; i < this->num_gpus; i++) {
      if (min_sm_version == -1 ||
          this->cuda_props[i].device_sm_version < min_sm_version)
    ...
    if (min_sm_version >= 300) {
      return EnactWTF<AdvanceKernelPolicy, FilterKernelPolicy>(src, alpha,
                                                               max_iteration);
    }
