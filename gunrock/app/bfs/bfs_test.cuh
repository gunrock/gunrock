...
void DisplaySolution(T *values, SizeT length)
{
    ...
    for (SizeT i = 0; i < length; ++i)
    {
        util::PrintMsg(std::to_string(i) + ":"
            + std::to_string(values[i]) + " ", true, false);
    }
    util::PrintMsg("]");
}
 ...
 * @param[out]  labels        Computed hop distances from the source to each vertex
 * @param[out]  preds         Computed predecessors for each vertex
 * \return      double        Time taken for the BFS
 */
template <
    typename GraphT,
    typename LabelT = typename GraphT::VertexT>
 ...
 * @param[in]  graph         Input graph
 * @param[in]  src           The source vertex
 * @param[in]  h_labels      Computed hop distances from the source to each vertex
...
template <
    typename GraphT,
    typename LabelT = typename GraphT::LabelT>
