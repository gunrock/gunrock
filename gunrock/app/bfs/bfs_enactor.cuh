...
__global__ void Expand_Incoming_BFS(const SizeT num_elements,
                                    const VertexId *const keys_in,
                                    VertexId *keys_out, const size_t array_size,
                                    char *array, int gpu_idx, VertexId label,
                                    VertexId *d_labels) {
...
struct LoadLabel<int, SizeT> {
  static __device__ __forceinline__ int Load(int *&d_labels, SizeT &pos) {
    // return tex1Dfetch(gunrock::oprtr::cull_filter::LabelsTex<int>::labels,
    // pos);
...
struct LoadLabel<long long, SizeT> {
  static __device__ __forceinline__ long long Load(long long *&d_labels,
                                                   SizeT &pos) {
    return _ldg(d_labels + pos);
  }
};
...
__global__ void Expand_Incoming_Kernel(
    int thread_num, typename KernelPolicy::VertexId label,
  ...
  while (x - threadIdx.x < num_elements) {
    ...
    if (x < num_elements) {
      ...
      if (KernelPolicy::Problem::ENABLE_IDEMPOTENCE) {
        mask_pos = (key & KernelPolicy::LOAD_BALANCED_CULL::ELEMENT_ID_MASK) >>
                   (2 + sizeof(MaskT));
      ...
      if (to_process) {
        VertexId loaded_label =
            LoadLabel<VertexId, VertexId>::Load(d_labels, key);
...
__global__ void From_Unvisited_Queue(typename Problem::SizeT num_nodes,
  ...
  while (x - threadIdx.x < num_nodes) {
    ...
    if (x < num_nodes) {
      ...
      // if
      // (tex1Dfetch(gunrock::oprtr::cull_filter::LabelsTex<VertexId>::labels, x)
      //    != util::MaxValue<VertexId>())
      if (LoadLabel<VertexId, SizeT>::Load(d_labels, x) !=
          util::MaxValue<VertexId>())
        to_process = false;
    } else
      to_process = false;
...
__global__ void From_Unvisited_Queue_IDEM(
    typename Problem::SizeT num_nodes, typename Problem::SizeT *d_out_length,
    typename Problem::VertexId *d_keys_out,
  ...
  while ((x - threadIdx.x) * sizeof(MaskT) * 8 < num_nodes) {
    ...
    if (x * (sizeof(MaskT) << 3) < num_nodes) {
      ...
      if (mask_byte != util::AllOnes<MaskT>())
        for (int i = 0; i < (1 << (2 + sizeof(MaskT))); i++) {
          ...
          // if (to_process)
          {
              ...
              // key) ==
              // tex1Dfetch(gunrock::oprtr::edge_map_partitioned::RowOffsetsTex<SizeT>::row_offsets,
              // key+1)) to_process = false; if (_ldg(d_row_offsets + key) ==
              // _ldg(d_row_offsets + (key+1)))
              //    continue;
          }  // if (to_process)
          {
      ...
      if (changed) d_visited_mask[x] = mask_byte;
    }
...
__global__ void From_Unvisited_Queue_Local_IDEM(
  ...
  while ((x - threadIdx.x) < num_local_vertices) {
    ...
    if (x < num_local_vertices) {
      key = d_local_vertices[x];
      SizeT mask_pos =
          (key & KernelPolicy::LOAD_BALANCED_CULL::ELEMENT_ID_MASK) >>
          (2 + sizeof(MaskT));
...
__global__ void Inverse_Expand(
    ...
    typename Problem::MaskT *d_visited_mask,
    typename Problem::VertexId *d_labels, typename Problem::VertexId *d_preds) {
  typedef typename Problem::SizeT SizeT;
  typedef typename Problem::VertexId VertexId;
  typedef typename Problem::MaskT MaskT;
  typedef util::Block_Scan<SizeT, KernelPolicy::CUDA_ARCH,
  ...
  while (x - threadIdx.x < num_unvisited_vertices) {
    ...
    if (to_process) {
      SizeT edge_start =  // tex1Dfetch(gunrock::oprtr::edge_map_partitioned::RowOffsetsTex<SizeT>::row_offsets,
                          // key);
          _ldg(d_inverse_row_offsets + key);
      SizeT edge_end =  // tex1Dfetch(gunrock::oprtr::edge_map_partitioned::RowOffsetsTex<SizeT>::row_offsets,
                        // key+1);
      ...
      for (SizeT edge_id = edge_start; edge_id < edge_end; edge_id++) {
        ...
        // if
        // (tex1Dfetch(gunrock::oprtr::cull_filter::LabelsTex<VertexId>::labels,
        // neighbor) == label-1) printf("%lld <- %lld, edge_id = %lld\n",
        //    (long long)key, (long long)neighbor, (long long)edge_id);
        if (LoadLabel<VertexId, VertexId>::Load(d_labels, neighbor) ==
            label - 1) {
          discoverable = true;
          parent = neighbor;
    ...
    if (discoverable) {
      ...
      d_labels[key] = label;
      if (Problem::MARK_PREDECESSORS) d_preds[key] = parent;
    }
    ...
    if (threadIdx.x == blockDim.x - 1) {
      if (output_pos != 0 || discoverable)
        block_offset = atomicAdd(d_split_lengths + 1,
                                 output_pos + ((discoverable) ? 1 : 0));
    }
    __syncthreads();
    ...
    if (threadIdx.x == blockDim.x - 1) {
      if (output_pos != 0 || to_process)
        block_offset =
            atomicAdd(d_split_lengths, output_pos + ((to_process) ? 1 : 0));
    }
    __syncthreads();
...
__global__ void Update_Mask_Kernel(typename Problem::SizeT num_nodes,
...
template <typename Problem, typename KernelPolicy>
__global__ void Combind_Masks(typename Problem::SizeT num_nodes, int num_gpus,
                              typename Problem::VertexId label,
  ...
  while (x - threadIdx.x < end_mask_pos) {
    ...
    if (x < end_mask_pos) {
      ...
      for (int gpu = 1; gpu < num_gpus; gpu++) {
        MaskT in_mask = 0;
        if (d_out_key_length[gpu] != 0) in_mask = _ldg(s_mask_ins[gpu] + x);
      ...
      // printf("(%d, %d) : new_mask = %#x, local_mask = %#x, old_mask = %#x,
      // update_mask = %#x, label = %d\n",
      //    blockIdx.x, threadIdx.x, new_mask, local_mask, old_mask,
      //    update_mask, label);
      if (update_mask != 0) {
        VertexId v = (x << 3) * sizeof(MaskT);
        for (int i = 0; i < (sizeof(MaskT) << 3); i++) {
          ...
          if ((update_mask & mask_bit) && (v < num_nodes)) {
            ...
            if (!(local_mask & mask_bit)) d_labels[v] = label;
          }
          v++;
        }
      }
    ...
    if (threadIdx.x == blockDim.x - 1) {
      if (output_pos + l_vertex_counter != 0)
        block_offset =
            atomicAdd(d_out_key_length, output_pos + l_vertex_counter);
    }
    __syncthreads();
...
struct BFSIteration
  ...
  typedef IterationBase<AdvanceKernelPolicy, FilterKernelPolicy, Enactor, false,
                        true, false, true, Enactor::Problem::MARK_PREDECESSORS>
      BaseIteration;
  ...
  static void FullQueue_Gather(
    ...
    if (false)  //(Problem::ENABLE_IDEMPOTENCE && (!Problem::MARK_PREDECESSORS)
                //&&
                // (data_slice -> num_gpus > 1) && (enactor_stats -> iteration >
                // 0))
    {
      for (int gpu = 1; gpu < data_slice->num_gpus; gpu++) {
        data_slice->in_masks[gpu] =
            (MaskT *)(data_slice->keys_in[enactor_stats->iteration % 2][gpu]
                          .GetPointer(util::DEVICE));
        // printf("%d\t \t %d, in_mask = %p\n",
        //    thread_num, gpu, data_slice -> in_masks[gpu]);
      }
      data_slice->in_masks.Move(util::HOST, util::DEVICE, data_slice->num_gpus,
                                0, stream);
      data_slice->in_length_out[0] = 0;
      data_slice->in_length_out.Move(util::HOST, util::DEVICE,
                                     data_slice->num_gpus, 0, stream);

      int num_blocks = graph_slice->nodes / 32 / sizeof(MaskT) /
                           AdvanceKernelPolicy::THREADS +
                       1;
      if (num_blocks > 480) num_blocks = 480;
      Combind_Masks<Problem, AdvanceKernelPolicy>
          <<<num_blocks, AdvanceKernelPolicy::THREADS, 0, stream>>>(
              graph_slice->nodes, data_slice->num_gpus,
    ...
    data_slice->num_unvisited_vertices =
        graph_slice->nodes - data_slice->num_visited_vertices;
    float predicted_backward_visits =
        data_slice->num_visited_vertices == 0
            ? std::numeric_limits<float>::infinity()
            : data_slice->num_unvisited_vertices * 1.0 * graph_slice->nodes /
                  data_slice->num_visited_vertices;
    // predicted_reverse_edge_visits /= enactor -> num_gpus;
    float predicted_forward_visits = frontier_attribute->queue_length * 1.0 *
                                     graph_slice->edges / graph_slice->nodes;
    // printf("%d\t %d\t %d\t queue_length = %d, output_length = %d, visited =
    // %d, rev = %f\n",
    ...
    if (enactor->direction_optimized) {
      if (data_slice->previous_direction == FORWARD) {
        if (predicted_forward_visits >
                predicted_backward_visits * enactor->do_a &&
        ...
        if (predicted_forward_visits >
            predicted_backward_visits * enactor->do_b)
          data_slice->direction_votes[iteration_] = BACKWARD;
        else
          data_slice->direction_votes[iteration_] = FORWARD;
      }
    ...
    if (enactor->num_gpus > 1 && enactor_stats->iteration != 0 &&
      ...
      while (data_slice -> current_direction == UNDECIDED)
      {
          ...
          for (int peer = 0; peer < enactor -> num_gpus; peer++)
              vote_counter[enactor -> problem -> data_slices[peer] ->
      direction_votes[iteration_]]++;
          ...
          if (vote_counter[FORWARD] > enactor -> num_gpus / 2)
              ...
              data_slice -> current_direction = enactor -> problem ->
      data_slices[0] -> direction_votes[iteration_]; else if
      (vote_counter[FORWARD] + vote_counter[UNDECIDED] <= vote_counter[BACKWARD]
      && enactor -> problem -> data_slices[0] -> direction_votes[iteration_] ==
      BACKWARD) data_slice -> current_direction = BACKWARD; else if
      (vote_counter[BACKWARD] + vote_counter[UNDECIDED] <= vote_counter[FORWARD]
      && enactor -> problem -> data_slices[0] -> direction_votes[iteration_] ==
      FORWARD) data_slice -> current_direction = FORWARD;
          //std::this_thread::yield();
          sleep(0);
      }*/
      while (enactor->problem->data_slices[0]->direction_votes[iteration_] ==
             UNDECIDED) {
        sleep(0);
        // std::this_thread::yield();
      }
    ...
    if (false)  //(thread_num == 0 && enactor -> direction_optimized)
      printf(
          "%d\t %lld\t \t queue = %lld,\t pre_output = %lld,\t visited = "
          "%lld,\t "
          "unvisited = %lld,\t predicted_forward = %.0f,\t predicted_backward "
          "= %.0f,\t "
          "direction = %s, ratio = %.8f\n",
          //" ratio2 = %.4f, ratio3 = %.4f, %.4f, ratio4 = %.4f, ratio5 =
          //%.4f\n",
          thread_num, enactor_stats->iteration,
    ...
    // data_slice -> num_visited_vertices * 1.0 / data_slice ->
    // num_unvisited_vertices, data_slice -> num_unvisited_vertices * 1.0 /
    // data_slice -> num_visited_vertices, frontier_attribute -> queue_length
    // * 1.0 / data_slice -> num_unvisited_vertices, frontier_attribute ->
    // queue_length * 1.0 * data_slice -> edges / data_slice -> nodes /
    //(data_slice -> num_unvisited_vertices * 1.0 * data_slice -> nodes /
    //data_slice -> num_visited_vertices));
  }
  ...
  static void FullQueue_Core(
    ...
    if (TO_TRACK) {
      printf("%d\t %lld\t %d SubQueue_Core queue_length = %lld\n", thread_num,
             (long long)enactor_stats->iteration, peer_,
             (long long)frontier_attribute->queue_length);
      fflush(stdout);
      // util::MemsetKernel<<<256, 256, 0, stream>>>(
      //    frontier_queue -> keys[frontier_attribute ->
      //    selector^1].GetPointer(util::DEVICE), (VertexId)-2, frontier_queue
      //    -> keys[frontier_attribute -> selector^1].GetSize());
      // util::Check_Exist<<<256, 256, 0, stream>>>(
      //    frontier_attribute -> queue_length,
      //    data_slice->gpu_idx, 2, enactor_stats -> iteration,
      //    frontier_queue -> keys[
      //    frontier_attribute->selector].GetPointer(util::DEVICE));
      // util::Verify_Value<<<256, 256, 0, stream>>>(
      //    data_slice -> gpu_idx, 2, frontier_attribute -> queue_length,
      //    enactor_stats -> iteration,
      //    frontier_queue -> keys[
      //    frontier_attribute->selector].GetPointer(util::DEVICE), data_slice
      //    -> labels.GetPointer(util::DEVICE), (Value)(enactor_stats ->
      //    iteration));
    }
    ...
    if (data_slice->current_direction == FORWARD) {
      ...
      if (enactor->debug)
        util::cpu_mt::PrintMessage("Forward Advance begin", thread_num,
                                   enactor_stats->iteration, peer_);
      VertexId *output_key_pointer =
      ...
      if (false)  //(gunrock::oprtr::advance::isFused<AdvanceKernelPolicy::ADVANCE_MODE>()
                  //&& data_slice -> num_gpus > 1 && Problem::ENABLE_IDEMPOTENCE
                  // && !Problem::MARK_PREDECESSORS)
      {
      ...
      gunrock::oprtr::advance::LaunchKernel<
          ...
          frontier_queue->keys[frontier_attribute->selector].GetPointer(
              util::DEVICE),
          output_key_pointer,  // frontier_queue->keys
                               // [frontier_attribute->selector^1].GetPointer(util::DEVICE),
          (Value *)NULL,
          frontier_queue->values[frontier_attribute->selector ^ 1].GetPointer(
              util::DEVICE),
          graph_slice->row_offsets.GetPointer(util::DEVICE),
          graph_slice->column_indices.GetPointer(util::DEVICE), (SizeT *)NULL,
          (VertexId *)NULL, graph_slice->nodes,
      ...
      if (enactor->debug)
        util::cpu_mt::PrintMessage("Forward Advance end", thread_num,
                                   enactor_stats->iteration, peer_);
      ...
      //    (long long)frontier_attribute -> queue_length,
      //    (long long)frontier_queue -> keys[frontier_attribute ->
      //    selector^1].GetSize());
      // fflush(stdout);
      ...
      if (!gunrock::oprtr::advance::isFused<
        ...
        gunrock::oprtr::filter::LaunchKernel<FilterKernelPolicy, Problem,
            ...
            data_slice->vertex_markers[enactor_stats->iteration % 2].GetPointer(
                util::DEVICE),
            data_slice->visited_mask.GetPointer(util::DEVICE),
            frontier_queue->keys[frontier_attribute->selector].GetPointer(
                util::DEVICE),
            frontier_queue->keys[frontier_attribute->selector ^ 1].GetPointer(
                util::DEVICE),
            frontier_queue->values[frontier_attribute->selector].GetPointer(
                util::DEVICE),
            (Value *)NULL, frontier_attribute->output_length[0],
        ...
        // if (FilterKernelPolicy::FILTER_MODE ==
        // gunrock::oprtr::filter::SIMPLIFIED)
        //    util::MemsetKernel<<<256, 256, 0, stream>>>(
        //        data_slice -> vertex_markers[(enactor_stats -> iteration
        //        +1)%2].GetPointer(util::DEVICE), (SizeT)0, graph_slice ->
        //        nodes + 1);
        // if (enactor -> debug && (enactor_stats->retval =
        //    util::GRError("filter_forward::Kernel failed", __FILE__,
        //    __LINE__))) return;
        frontier_attribute->queue_index++;
        frontier_attribute->selector ^= 1;
      ...
      if (data_slice->previous_direction == FORWARD) {
        ...
        // if (true)
        {
          if (Problem::ENABLE_IDEMPOTENCE) {
            num_blocks =
                (graph_slice->nodes >> (2 + /*sizeof(Problem::MaskT)*/ 1)) /
                    AdvanceKernelPolicy::THREADS +
                1;
            if (num_blocks > 480) num_blocks = 480;
            From_Unvisited_Queue_IDEM<Problem, AdvanceKernelPolicy>
                <<<num_blocks, AdvanceKernelPolicy::THREADS, 0, stream>>>(
                    ...
                    data_slice->unvisited_vertices[frontier_attribute->selector]
                        .GetPointer(util::DEVICE),
                    data_slice->visited_mask.GetPointer(util::DEVICE),
                    graph_slice->row_offsets.GetPointer(util::DEVICE),
                    data_slice->labels.GetPointer(util::DEVICE));
          ...
          num_blocks = data_slice->local_vertices.GetSize() /
                           AdvanceKernelPolicy::THREADS +
                       1;
          if (num_blocks > 480) num_blocks = 480;
          if (Problem::ENABLE_IDEMPOTENCE) {
        ...
        data_slice->num_visited_vertices =
            graph_slice->nodes - data_slice->num_unvisited_vertices;
        // printf("%d\t actual unvisited = %lld\n", thread_num, (long
        // long)num_unvisited_vertices);
      } else {
        data_slice->num_unvisited_vertices = data_slice->split_lengths[0];
      }
      // util::cpu_mt::PrintGPUArray<SizeT, VertexId>("unvisited0",
      //    data_slice -> unvisited_vertices[frontier_attribute ->
      //    selector].GetPointer(util::DEVICE), num_unvisited_vertices,
      //    data_slice -> gpu_idx, enactor_stats -> iteration, -1, stream);
      ...
      VertexId *output_key_pointer =
          frontier_queue->keys[frontier_attribute->selector ^ 1].GetPointer(
              util::DEVICE);
      if (false)  //(data_slice -> num_gpus > 1 && Problem::ENABLE_IDEMPOTENCE
                  //&& !Problem::MARK_PREDECESSORS)
        output_key_pointer = NULL;
      num_blocks =
          data_slice->num_unvisited_vertices / AdvanceKernelPolicy::THREADS + 1;
      if (num_blocks > 480) num_blocks = 480;
      ...
      Inverse_Expand<Problem, AdvanceKernelPolicy>
          <<<num_blocks, AdvanceKernelPolicy::THREADS, 0, stream>>>(
              data_slice->num_unvisited_vertices, enactor_stats->iteration + 1,
              data_slice->unvisited_vertices[frontier_attribute->selector]
                  .GetPointer(util::DEVICE),
              ...
              data_slice->split_lengths.GetPointer(util::DEVICE),
              data_slice->unvisited_vertices[frontier_attribute->selector ^ 1]
                  .GetPointer(util::DEVICE),
              output_key_pointer,  // frontier_queue -> keys[frontier_attribute
      ...
      data_slice->num_visited_vertices =
          data_slice->nodes - data_slice->num_unvisited_vertices;
      enactor_stats->edges_queued[0] += frontier_attribute->output_length[0];
      frontier_attribute->queue_reset = false;
      frontier_attribute->queue_index++;
      frontier_attribute->selector ^= 1;
    ...
    //    "cudaStreamSynchronize failed", __FILE__, __LINE__))
    //    return;
    // printf("%d, resulted queue_length = %d\n", thread_num, frontier_attribute
    // ->queue_length); if (enactor_stats -> retval =
    // util::cpu_mt::PrintGPUArray<SizeT, VertexId>("PostAdvance",
    //    frontier_queue -> keys[frontier_attribute ->
    //    selector].GetPointer(util::DEVICE), frontier_attribute ->
    //    queue_length, thread_num, enactor_stats -> iteration, peer_, stream))
    //    return;
    ...
    if (TO_TRACK) {
      ...
      //    frontier_queue -> keys[
      //    frontier_attribute->selector].GetPointer(util::DEVICE), data_slice
      //    -> labels.GetPointer(util::DEVICE), enactor_stats -> iteration+1);
      // util::MemsetCASKernel<<<256, 256, 0, stream>>>(
      //    frontier_queue -> keys[
      //    frontier_attribute->selector].GetPointer(util::DEVICE), -2, -1,
      //    work_progress -> template GetQueueLengthPointer<unsigned int,
      //    SizeT>(
      //        frontier_attribute->queue_index));
    }
  ...
  static void Expand_Incoming(
    ...
    // printf("Expand_Incoming, num_elements = %d, queue_size = %d, size_check =
    // %s, labels = %p\n",
    //    num_elements, keys_out.GetSize(), enactor -> size_check ? "true" :
    //    "false", h_data_slice -> labels.GetPointer(util::DEVICE));
    // fflush(stdout);
    ...
    Expand_Incoming_Kernel<AdvanceKernelPolicy, NUM_VERTEX_ASSOCIATES,
                           NUM_VALUE__ASSOCIATES>
        <<<num_blocks, AdvanceKernelPolicy::THREADS, 0, stream>>>(
            h_data_slice->gpu_idx, iteration, num_elements,
  ...
  static void Check_Queue_Size(Enactor *enactor, int thread_num, int peer_,
    ...
    {
      if (enactor_stats->retval = Check_Size</*true,*/ SizeT, VertexId>(
      ...
      if (enactor->problem->use_double_buffer) {
        if (enactor_stats->retval = Check_Size</*true,*/ SizeT, Value>(
                true, "queue3", request_length,
  ...
  static void Make_Output(Enactor *enactor, int thread_num, SizeT num_elements,
                ...
                //num_gpus > 1)
    {
      ...
      Update_Mask_Kernel<Problem>
          <<<num_blocks, AdvanceKernelPolicy::THREADS, 0, stream>>>(
              graph_slice->nodes, data_slice->old_mask.GetPointer(util::DEVICE),
              data_slice->visited_mask.GetPointer(util::DEVICE),
              data_slice->labels.GetPointer(util::DEVICE));
      for (int gpu = 0; gpu < num_gpus; gpu++) {
        data_slice->out_length[gpu] =
            (num_elements == 0) ? 0
                                : (graph_slice->nodes / 8 / sizeof(MaskT) + 1) *
                                          sizeof(MaskT) / sizeof(VertexId) +
                                      1;
        data_slice->in_length_out[gpu] = 0;
        data_slice->keys_out[gpu].SetPointer(
            (VertexId *)(data_slice->visited_mask.GetPointer(util::DEVICE)),
            (graph_slice->nodes / (sizeof(MaskT) * 8) + 1) * sizeof(MaskT) /
                    sizeof(VertexId) +
                1,
            util::DEVICE);
        // printf("keys_out = %p\n",
        //    data_slice -> keys_out[gpu].GetPointer(util::DEVICE));
        data_slice->keys_outs[gpu] =
            data_slice->keys_out[gpu].GetPointer(util::DEVICE);
      }
    } else {
      BaseIteration::template Make_Output<NUM_VERTEX_ASSOCIATES,
                                          NUM_VALUE__ASSOCIATES>(
          enactor, thread_num, num_elements, num_gpus, frontier_queue,
          scanned_edges, frontier_attribute, enactor_stats, data_slice_,
          graph_slice, work_progress, context, stream);
    }
  }
...
template <typename AdvanceKernelPolicy, typename FilterKernelPolicy,
          typename Enactor>
static CUT_THREADPROC BFSThread(void *thread_data_) {
  typedef typename Enactor::Problem Problem;
  typedef typename Enactor::SizeT SizeT;
  typedef typename Enactor::VertexId VertexId;
  typedef typename Enactor::Value Value;
  ...
  while (thread_data->status != ThreadSlice::Status::ToKill) {
    ...
    for (int peer = 0; peer < num_gpus; peer++) {
      ...
      frontier_attribute[peer].queue_length =
          peer == 0 ? thread_data->init_size : 0;
      frontier_attribute[peer].selector =
          0;  // frontier_attribute[peer].queue_length ==0 ? 0 : 1;
    ...
    gunrock::app::Iteration_Loop<
        ...
        Problem::MARK_PREDECESSORS ? 1 : 0, 0>(thread_data);
...
class BFSEnactor
 ...
 public:
   ...
   */
  BFSEnactor(int num_gpus = 1, int *gpu_idx = NULL, bool instrument = false,
   ...
   */
  template <typename AdvanceKernelPolicy, typename FilterKernelPolicy>
  cudaError_t InitBFS(ContextPtr *context, Problem *problem,
                      int max_grid_size = 0) {
    cudaError_t retval = cudaSuccess;
    ...
    for (int gpu = 0; gpu < this->num_gpus; gpu++) {
      ...
      thread_slices[gpu].thread_Id =
          cutStartThread((CUT_THREADROUTINE) &
                             (BFSThread<AdvanceKernelPolicy, FilterKernelPolicy,
                                        BFSEnactor<Problem>>),
   ...
   */
  template <typename AdvanceKernelPolicy, typename FilterKernelPolicy>
  cudaError_t EnactBFS(VertexId src) {
  ...
  template <typename AdvanceKernelPolicy, typename FilterKernelPolicy,
            typename AdvanceKernelPolicy_IDEM, typename FilterKernelPolicy_IDEM,
            bool ENABLE_IDEMPOTENCE>
  struct IDEM_SWITCH {};

  template <typename AdvanceKernelPolicy, typename FilterKernelPolicy,
            typename AdvanceKernelPolicy_IDEM, typename FilterKernelPolicy_IDEM>
  struct IDEM_SWITCH<AdvanceKernelPolicy, FilterKernelPolicy,
                     AdvanceKernelPolicy_IDEM, FilterKernelPolicy_IDEM, true> {
    static cudaError_t Enact(Enactor &enactor, VertexId src) {
      return enactor
          .EnactBFS<AdvanceKernelPolicy_IDEM, FilterKernelPolicy_IDEM>(src);
    }

    static cudaError_t Init(Enactor &enactor, ContextPtr *context,
                            Problem *problem, int max_grid_size = 0) {
      return enactor.InitBFS<AdvanceKernelPolicy_IDEM, FilterKernelPolicy_IDEM>(
  ...
  struct IDEM_SWITCH<AdvanceKernelPolicy, FilterKernelPolicy,
    ...
    static cudaError_t Init(Enactor &enactor, ContextPtr *context,
                            Problem *problem, int max_grid_size = 0) {
      return enactor.InitBFS<AdvanceKernelPolicy, FilterKernelPolicy>(
          context, problem, max_grid_size);
    }
  };

  template <bool ENABLE_IDEMPOTENCE, gunrock::oprtr::advance::MODE A_MODE>
  struct MODE_SWITCH {};

  template <bool ENABLE_IDEMPOTENCE>
  struct MODE_SWITCH<ENABLE_IDEMPOTENCE, gunrock::oprtr::advance::LB> {
    typedef IDEM_SWITCH<LBAdvanceKernelPolicy, FilterKernelPolicy,
  ...
  struct MODE_SWITCH<ENABLE_IDEMPOTENCE, gunrock::oprtr::advance::TWC_FORWARD> {
    typedef IDEM_SWITCH<TWCAdvanceKernelPolicy, FilterKernelPolicy,
  ...
  struct MODE_SWITCH<ENABLE_IDEMPOTENCE, gunrock::oprtr::advance::LB_LIGHT> {
    typedef IDEM_SWITCH<LB_LIGHT_AdvanceKernelPolicy, FilterKernelPolicy,
  ...
  struct MODE_SWITCH<ENABLE_IDEMPOTENCE, gunrock::oprtr::advance::LB_CULL> {
    typedef IDEM_SWITCH<LB_CULL_AdvanceKernelPolicy, FilterKernelPolicy,
  ...
  template <bool ENABLE_IDEMPOTENCE>
  struct MODE_SWITCH<ENABLE_IDEMPOTENCE,
                     gunrock::oprtr::advance::LB_LIGHT_CULL> {
    typedef IDEM_SWITCH<LB_LIGHT_CULL_AdvanceKernelPolicy, FilterKernelPolicy,
                        LB_LIGHT_CULL_AdvanceKernelPolicy_IDEM,
                        FilterKernelPolicy, ENABLE_IDEMPOTENCE>
        I_SWITCH;
  ...
  cudaError_t Enact(VertexId src, std::string traversal_mode = "LB") {
    if (this->min_sm_version >= 300) {
      if (this->min_sm_version < 350 && sizeof(VertexId) == 8)
        return util::GRError(
            "64bit-VertexId disabled, because atomicMin(long long) is only "
            "supported by compute capability 3.5 or higher",
            __FILE__, __LINE__);
      else if (traversal_mode == "LB")
        return MODE_SWITCH<Problem::ENABLE_IDEMPOTENCE,
                           gunrock::oprtr::advance::LB>::Enact(*this, src);
      else if (traversal_mode == "TWC")
        return MODE_SWITCH<Problem::ENABLE_IDEMPOTENCE,
                           gunrock::oprtr::advance::TWC_FORWARD>::Enact(*this,
                                                                        src);
      else if (traversal_mode == "LB_CULL")
        return MODE_SWITCH<Problem::ENABLE_IDEMPOTENCE,
                           gunrock::oprtr::advance::LB_CULL>::Enact(*this, src);
      else if (traversal_mode == "LB_LIGHT")
        return MODE_SWITCH<Problem::ENABLE_IDEMPOTENCE,
                           gunrock::oprtr::advance::LB_LIGHT>::Enact(*this,
                                                                     src);
   ...
   * \return cudaError_t object Indicates the success of all CUDA calls.
   */
  cudaError_t Init(ContextPtr *context, Problem *problem, int max_grid_size = 0,
    ...
    if (this->min_sm_version >= 300) {
      if (this->min_sm_version < 350 && sizeof(VertexId) == 8)
        ...
        return MODE_SWITCH<Problem::ENABLE_IDEMPOTENCE,
                           gunrock::oprtr::advance::LB>::Init(*this, context,
                                                              problem,
                                                              max_grid_size);
      else if (traversal_mode == "TWC")
        return MODE_SWITCH<
            Problem::ENABLE_IDEMPOTENCE,
            gunrock::oprtr::advance::TWC_FORWARD>::Init(*this, context, problem,
                                                        max_grid_size);
      else if (traversal_mode == "LB_CULL")
        return MODE_SWITCH<
            Problem::ENABLE_IDEMPOTENCE,
            gunrock::oprtr::advance::LB_CULL>::Init(*this, context, problem,
                                                    max_grid_size);
      else if (traversal_mode == "LB_LIGHT")
        return MODE_SWITCH<
            Problem::ENABLE_IDEMPOTENCE,
            gunrock::oprtr::advance::LB_LIGHT>::Init(*this, context, problem,
                                                     max_grid_size);
      else if (traversal_mode == "LB_LIGHT_CULL")
        return MODE_SWITCH<
            Problem::ENABLE_IDEMPOTENCE,
            gunrock::oprtr::advance::LB_LIGHT_CULL>::Init(*this, context,
                                                          problem,
                                                          max_grid_size);
    }
