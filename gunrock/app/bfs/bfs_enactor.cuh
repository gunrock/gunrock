...
struct BFSIterationLoop : public IterationLoopBase
    ...
    Update_Predecessors : 0x0)>
{
    ...
    cudaError_t Gather(int peer_)
    {
        ...
        {
            while (this -> enactor -> problem ->
                data_slices[0] -> direction_votes[iteration_] == UNDECIDED)
            {
                sleep(0);
            }
            data_slice.current_direction = this -> enactor -> problem ->
                data_slices[0] -> direction_votes[iteration_];
    ...
    cudaError_t Compute_OutputLength(int peer_)
    {
        ...
        if (((this -> enactor -> flag & Size_Check) == 0 &&
            ...
            (this -> enactor -> problem -> data_slices[0] -> current_direction
                == BACKWARD))
    ...
    cudaError_t Core(int peer_ = 0)
    {
        ...
        if (data_slice.current_direction == FORWARD)
        {
            ...
            auto filter_op = [idempotence, mark_preds, preds, original_vertex]
            __host__ __device__ (
                const VertexT &src, VertexT &dest, const SizeT &edge_id,
                const VertexT &input_item, const SizeT &input_pos,
                SizeT &output_pos) -> bool
                ...
                oprtr_parameters.advance_mode != "LB_LIGHT_CULL")
            {
                // Filter
                if (debug)
                    util::PrintMsg("Filter begin.", gpu_num, iteration, peer_);
            ...
            if (data_slice.previous_direction == FORWARD)
            {
                GUARD_CU(data_slice.split_lengths.ForEach(
                    [] __host__ __device__ (SizeT &length)
                    {
                        length = 0;
                    }, 1, target, stream));
                ...
                if (this -> enactor -> num_gpus == 1)
                {
                    if (idempotence)
                    {
                        num_blocks = (graph.nodes >> (2 + sizeof(MaskT))) /
                            (1 << LOG_THREADS) + 1;
                        if (num_blocks > 480)
                            num_blocks = 480;
    ...
    cudaError_t ExpandIncoming(SizeT &received_length, int peer_)
    {
        ...
        auto expand_op = [labels, label, preds, masks, idempotence, mark_preds]
        __host__ __device__(
        ...
        retval = BaseIterationLoop:: template ExpandIncomingBase
