...
float RunTests_enable_idempotence(GRGraph *output, BFS_Parameter *parameter) {
  if (parameter->enable_idempotence)
    return runBFS<VertexId, SizeT, Value,
                  /*INSTRUMENT, DEBUG,
...
float RunTests_mark_predecessors(GRGraph *output, BFS_Parameter *parameter) {
  if (parameter->mark_predecessors)
    return RunTests_enable_idempotence<VertexId, SizeT, Value, true>(output,
                                                                     parameter);
  else
    return RunTests_enable_idempotence<VertexId, SizeT, Value, false>(
        output, parameter);
}
...
float runBFS(GRGraph *output, BFS_Parameter *parameter) {
  ...
  Enactor *enactor =
      new Enactor(num_gpus, gpu_idx, instrument, debug, size_check,
                  direction_optimized);  // BFS enactor map
  ...
  for (int i = 0; i < num_iters; ++i) {
    ...
    util::GRError(problem->Reset(parameter->src[i], enactor->GetFrontierType(),
    ...
    util::GRError(enactor->Enact(parameter->src[i], traversal_mode),
 ...
 * \return Elapsed run time in milliseconds
 */
float dispatch_bfs(GRGraph *grapho, const GRGraph *graphi,
        ...
        case SIZET_INT: {
          switch (data_t.VALUE_TYPE) {
            case VALUE_INT:  // template type = <int, int, int>
            {
              ...
              elapsed_time =
                  RunTests_mark_predecessors<int, int, int>(grapho, parameter);
 ...
 */
float gunrock_bfs(GRGraph *grapho, const GRGraph *graphi, const GRSetup *config,
 ...
 *
 * @param[out] bfs_label            Return BFS label (depth) per nodes or the
 * predecessor per nodes
 ...
 */
float bfs(int *bfs_label, int *bfs_preds, const int num_nodes,
  ...
  struct GRSetup *config =
      InitSetup(num_iters, source);  // primitive-specific configures
