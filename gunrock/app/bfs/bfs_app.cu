 ...
 */
double bfs(const int num_nodes,
           const int num_edges,
  ...
  return bfs<int, int, int>(num_nodes, num_edges, row_offsets, col_indices,
    sources, mark_pred, direction_optimized, idempotence, labels, preds,
    num_runs, allocated_on);
}
