...
/** * Data slice per GPU
    ...
    // Size of smem storage the problem uses
    static const SizeT               SMEM_SIZE = 0;
	        ...
	        double queue_sizing)                    // Size scaling factor for work queue allocation (e.g., 1.0 creates n-element and m-element vertex and edge frontiers, respectively). 0.0 is unspecified.
	{
	    ...
	    for (int gpu = 0; gpu < num_gpus; ++gpu) {
	        ...
	        int BlockSize = (nodes + 128)/128;
	        util::MemsetKernel<<<BlockSize, 128>>>(data_slices[gpu]->d_labels, -1, nodes);
	        ...
	        // Allocate preds if necessary
	            ...
	            util::MemsetKernel<<<BlockSize, 128>>>(data_slices[gpu]->d_preds, -2, nodes);
