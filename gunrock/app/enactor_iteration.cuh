...
public:
    ...
    {
        return All_Done(enactor[0], gpu_num);
    }
    ...
    cudaError_t Check_Queue_Size(int peer_)
    {
        ...
        retval = CheckSize<SizeT, VertexT> (
                ...
                over_sized, gpu_num, iteration, peer_, false);
        if (retval) return retval;
        retval = CheckSize<SizeT, VertexT> (
                true, "queue3", request_length, frontier.V_Q(),
                over_sized, gpu_num, iteration, peer_, true );
        if (retval) return retval;
        // TODO
    ...
    cudaError_t MakeOutput(SizeT num_elements)
    {
        ...
        for (int peer_ = 0; peer_ < num_gpus; peer_++)
        {
            if (retval = CheckSize<SizeT, VertexT> (
                enactor -> flag & Size_Check, "keys_out",
                ...
                break;
            //if (keys_over_sized)
                mgpu_slice.keys_outs[peer_] = (peer_==0) ?
            ...
            //{
                if (retval = CheckSize <SizeT, VertexT>(
                    ...
                    &mgpu_slice.vertex_associate_out[peer_],
                    over_sized, gpu_num, enactor_stats.iteration, peer_, false))
                    break;
                //if (over_sized)
            ...
            //{
                if (retval = CheckSize<SizeT, ValueT>(
                    ...
                    &mgpu_slice.value__associate_out[peer_],
                    over_sized, gpu_num, enactor_stats.iteration, peer_, false))
                    break;
                //if (over_sized)
        ...
        mgpu_slice.vertex_associate_outs.Move(util::HOST, util::DEVICE, num_gpus, 0, stream);
        mgpu_slice.value__associate_outs.Move(util::HOST, util::DEVICE, num_gpus, 0, stream);
        ...
        if ((FLAG & Skip_Makeout_Selection) == 0)
        {
            ...
            if (FLAG & Push)
                MakeOutput_Kernel < VertexT, SizeT, ValueT,
                    NUM_VERTEX_ASSOCIATES, NUM_VALUE__ASSOCIATES>
            ...
            MakeOutput_SkipSelection_Kernel < VertexT, SizeT, ValueT,
                NUM_VERTEX_ASSOCIATES, NUM_VALUE__ASSOCIATES>
                <<< grid_size, block_size, 0, stream>>> (
                num_elements,
                frontier.V_Q() -> GetPointer(util::DEVICE),
