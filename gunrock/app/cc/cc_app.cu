    ...
    int          num_gpus)
{
    ...
    typedef CCProblem<
        ...
        SizeT,
        Value,
        true> Problem; //use double buffer for edge_map and vertex_map.

    // Allocate host-side label array (for both reference and gpu-computed results)
    VertexId *h_component_ids = (VertexId*)malloc(sizeof(VertexId) * csr_graph.nodes);

    // Allocate CC enactor map
    CCEnactor<false> cc_enactor(g_verbose);
...
*
* @param[out] output subgraph of cc problem
* @param[in] input graph need to process on
* @param[in] gunrock cc configurations
