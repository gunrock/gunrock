...
template <
    ...
    typename    _SizeT,                          
    typename    _Value>
struct HITSProblem : ProblemBase<_VertexId, _SizeT, false> // USE_DOUBLE_BUFFER = false
{
            ...
            double queue_sizing = 1.0)             // The frontier type (i.e., edge/vertex/mixed)
    {
        ...
        for (int gpu = 0; gpu < num_gpus; ++gpu) {
            ...
            // Initial rank_curr = 0 
            util::MemsetKernel<<<128, 128>>>(data_slices[gpu]->d_hrank_curr, (Value)0, nodes);
            util::MemsetKernel<<<128, 128>>>(data_slices[gpu]->d_arank_curr, (Value)0, nodes);
        ...
        if (retval = util::GRError(cudaMemcpy(
                        data_slices[0]->d_hrank_curr+src,
                        &init_score,
                        sizeof(Value),
                        cudaMemcpyHostToDevice),
