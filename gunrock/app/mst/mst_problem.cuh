...
struct MSTProblem : ProblemBase<_VertexId, _SizeT, _USE_DOUBLE_BUFFER> // USE_DOUBLE_BUFFER = false
{
	...
	~MSTProblem()
	{
		...
		if (data_slices[i]->d_edges)          	util::GRError(cudaFree(data_slices[i]->d_edges),
		...
		if (data_slices[i]->d_weights)		util::GRError(cudaFree(data_slices[i]->d_weights), 
	 	...
	 	if (data_slices[i]->d_reducedWeights)  	util::GRError(cudaFree(data_slices[i]->d_reducedWeights),
		...
		if (data_slices[i]->d_represent)	util::GRError(cudaFree(data_slices[i]->d_represent),
                        "GpuSlice cudaFree d_represent failed", __FILE__, __LINE__);
		if (data_slices[i]->d_superVertex)	util::GRError(cudaFree(data_slices[i]->d_superVertex),
			"GpuSlice cudaFree d_superVertex failed", __FILE__, __LINE__);
		if (data_slices[i]->d_row_offsets)	util::GRError(cudaFree(data_slices[i]->d_row_offsets),
                        "GpuSlice cudaFree d_row_offsets failed", __FILE__, __LINE__);
		if (data_slices[i]->d_edgeId)		util::GRError(cudaFree(data_slices[i]->d_edgeId),
                        "GpuSlice cudaFree d_edgeId failed", __FILE__, __LINE__);
		if (data_slices[i]->d_vertex_flag)	util::GRError(cudaFree(data_slices[i]->d_vertex_flag),
			 "GpuSlice cudaFree d_vertex_flag failed", __FILE__, __LINE__);
		if (data_slices[i]->d_nodes)      	util::GRError(cudaFree(data_slices[i]->d_nodes),
                         "GpuSlice cudaFree d_nodes failed", __FILE__, __LINE__);
		if (data_slices[i]->d_Cflag)            util::GRError(cudaFree(data_slices[i]->d_Cflag),
                        "GpuSlice cudaFree d_Cflag failed", __FILE__, __LINE__);
		if (data_slices[i]->d_Ckeys)            util::GRError(cudaFree(data_slices[i]->d_Ckeys),
                        "GpuSlice cudaFree d_Ckeys failed", __FILE__, __LINE__);	
		if (data_slices[i]->d_eId)             	util::GRError(cudaFree(data_slices[i]->d_eId),
                        "GpuSlice cudaFree d_eId failed", __FILE__, __LINE__);
		if (data_slices[i]->d_selector)         util::GRError(cudaFree(data_slices[i]->d_selector),
		...
		int         _num_gpus)
	{
		...
		if (num_gpus <= 1) {
			...
			bool           *d_selector;        // has the same size as #edges
                        if (retval = util::GRError(cudaMalloc(
                                (void**)&d_selector,
                                edges * sizeof(SizeT)),
