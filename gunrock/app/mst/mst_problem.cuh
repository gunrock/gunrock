...
struct MSTProblem : ProblemBase<_VertexId, _SizeT, _USE_DOUBLE_BUFFER>
{
	...
	SizeT        *d_edge_offsets; //!< edge offsets used for filter edges
    };
	...
	do {
		...
		VertexId *d_super_vids;
		if (retval = util::GRError(cudaMalloc(
		    (void**)&d_super_vids,
		    nodes * sizeof(VertexId)),
		    "MSTProblem cudaMalloc d_super_vids Failed",
		    __FILE__, __LINE__)) return retval;
		data_slices[0]->d_super_vids = d_super_vids;
		util::MemsetKernel<<<128, 128>>>(
		    data_slices[0]->d_super_vids, 0, nodes);
	...
	for (int gpu = 0; gpu < num_gpus; ++gpu) {
	    ...
	    if (!data_slices[gpu]->d_super_vids) {
		VertexId *d_super_vids;
		if (retval = util::GRError(cudaMalloc(
		    (void**)&d_super_vids,
		    nodes * sizeof(VertexId)),
		    "MSTProblem cudaMalloc d_super_vids Failed",
		    __FILE__, __LINE__)) return retval;
		data_slices[gpu]->d_super_vids = d_super_vids;
	    }
