 ...
 * mst_enactor.cuh
 *
 * @brief Minimum Spaning Tree Problem Enactor
 */
...
namespace gunrock {
  namespace app     {
    namespace mst     {
       ...
       *
       * @tparam INSTRUMWENT Boolean type to show whether or not to
       * collect per-CTA clock-count statistics
       */
	 ...
	 *
	 * @param[in] problem MST Problem object which holds the graph
	 * data and MST data to compute.
	...
	cudaError_t Setup(ProblemData *problem)
	{
	  ...
	  if (!done) {
	    ...
	    if (retval = util::GRError(cudaHostGetDevicePointer((void**)&d_done, (void*) done, 0),
				       "MSTEnactor cudaHostGetDevicePointer done failed", __FILE__, __LINE__)) return retval;

	    // Create throttle event
	    if (retval = util::GRError(cudaEventCreateWithFlags(&throttle_event, cudaEventDisableTiming),
				       "MSTEnactor cudaEventCreateWithFlags throttle_event failed", __FILE__, __LINE__)) return retval;
	  }
	...
	template<
	  ...
	  typename MSTProblem>
	cudaError_t EnactMST(CudaContext &context,
			     MSTProblem *problem,
			     int         max_grid_size = 0)
	{
	  ...
	  typedef MSTFunctor<VertexId, SizeT, Value, MSTProblem> MstFunctor;
	 ...
	 * @param[in] max_grid_size Max grid size for MST kernel calls.
	 *
	 * \return cudaError_t object which indicates the success of all
	 * CUDA function calls.
	 */
