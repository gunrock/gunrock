    ...
    <EnactorT, Use_FullQ | Push>
{
    ...
    cudaError_t Core(int peer_ = 0)
    {
        ...
        // Pass initialization
        GUARD_CU(w_v2.ForAll([w_v2self, current_communities, community_sizes]
            __host__ __device__ (ValueT *w_v2_, const SizeT &v)
            {
                w_v2_    [v] = 0;
                w_v2self [v] = 0;
        ...
        GUARD_CU(w_c2.ForAll([w_v2]
            __host__ __device__ (ValueT *w_c, const VertexT &v)
            {
                w_c[v] = w_v2[v];
                //w_c = w_v;
                //printf("w_v2[%d] = %lf\n",
        ...
        while (to_continue)
        {
            ...
            if (unify_segments)
            {
                ...
                GUARD_CU(util::cubSegmentedSortPairs(
                    ...
                    0, std::ceil(std::log2(graph.nodes)), stream));
            ...
            GUARD_CU(util::cubSelectIf(
                cub_temp_space,
                seg_offsets0, seg_offsets1,
                num_neighbor_comms, graph.edges + 1,
                [] __host__ __device__ (const SizeT &e){
                    return util::isValid(e);
                }, stream));
            ...
            //util::PrintMsg("num_neigbhor_comms = " + std::to_string(n_neighbor_comms));
            ...
            GUARD_CU(seg_offsets0.ForAll(
                [seg_offsets1, n_neighbor_comms, graph]
                __host__ __device__ (SizeT *offsets, const VertexT &v)
                {
                    if (v == graph.nodes)
                        ...
                        offsets[v] = util::BinarySearch_LeftMost(
                            ...
                            seg_offsets1, (SizeT)0, n_neighbor_comms + 1, false);//,
            ...
            GUARD_CU(max_gains.ForAll(
                [next_communities, current_communities]
                __host__ __device__ (ValueT *gains, const VertexT &v)
                {
                    gains[v] = 0;
            ...
            GUARD_CU(current_communities.ForAll(
                [next_communities, community_sizes, max_gains, w_v2, w_c2]
                __host__ __device__ (VertexT *communities, const VertexT &v)
                {
                    VertexT c_comm = communities[v];
                    VertexT n_comm = next_communities[v];
                    if (c_comm == v && community_sizes[v] + community_sizes[n_comm] <= 2)
                    {
            ...
            GUARD_CU(util::cubReduce(
                cub_temp_space,
                max_gains, iter_gain, graph.nodes,
                [] __host__ __device__ (const ValueT &a, const ValueT &b)
                {
                    return a + b;
                }, (ValueT)0, stream));
        ...
        GUARD_CU(oprtr::Advance<oprtr::OprtrType_V2V>(
            ...
            [edge_comms0, current_communities, edge_pairs0] 
            __host__ __device__ (
            const VertexT &src, VertexT &dest, const SizeT &edge_id,
            const VertexT &input_item, const SizeT &input_pos,
            SizeT &output_pos) -> bool
            {
                ...
                edge_pairs0[edge_id] = (((EdgePairT)src_comm) << (sizeof(VertexT) * 8))
                    + (EdgePairT)dest_comm;
                return false;
            }));
        ...
        GUARD_CU(seg_offsets0.ForAll(
            [graph, edge_pairs1] __host__ __device__ (SizeT *offsets0, const SizeT &e)
            {
                if (e != 0 && e != graph.edges)
                {
                    auto edge = edge_pairs1[e];
                    auto pervious_edge = edge_pairs1[e - 1];
                    if (edge == pervious_edge)
                        return;
                }
                offsets0[e] = e;
            }, graph.edges + 1, target, stream));
        ...
        if (enactor.pass_stats)
        {
            pass_timer.Stop();
            util::PrintMsg("pass " + std::to_string(pass_num)
                + ", #v = " + std::to_string(graph.nodes) 
                + " -> " + std::to_string(n_new_comms)
                + ", #e = " + std::to_string(graph.edges)
                + " -> " + std::to_string(n_new_edges)
                + ", #iter = " + std::to_string(iter_num)
                + ", q = " + std::to_string(data_slice.q)
                + ", pass_gain = " + std::to_string(pass_gain)
                + ", elapsed = "
    ...
    bool Stop_Condition(int gpu_num = 0)
    {
        ...
        ValueT        pass_gain_threshold =  this -> enactor ->
            problem -> parameters.template Get<ValueT>("pass-th");
