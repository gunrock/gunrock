...
struct SampleIteration
  ...
  typedef IterationBase<AdvanceKernelPolicy, FilterKernelPolicy, Enactor, false,
                        true, false, true, false>  // MARK_PREDECESSORS
      BaseIteration;
...
template <typename AdvanceKernelPolicy, typename FilterKernelPolicy,
          typename Enactor>
static CUT_THREADPROC SampleThread(void *thread_data_) {
  typedef typename Enactor::Problem Problem;
  typedef typename Enactor::SizeT SizeT;
  typedef typename Enactor::VertexId VertexId;
  typedef typename Enactor::Value Value;
  ...
  while (thread_data->status != ThreadSlice::Status::ToKill) {
    ...
    gunrock::app::Iteration_Loop<
        Enactor, Functor,
        SampleIteration<AdvanceKernelPolicy, FilterKernelPolicy, Enactor>, 0,
        1>(thread_data);
...
class SampleEnactor : public EnactorBase<typename _Problem::SizeT> {
 ...
 public:
   ...
   */
  SampleEnactor(int num_gpus = 1, int *gpu_idx = NULL, bool instrument = false,
   ...
   */
  template <typename AdvanceKernelPolicy, typename FilterKernelPolicy>
  cudaError_t InitSample(ContextPtr *context, Problem *problem,
                         int max_grid_size = 0) {
    cudaError_t retval = cudaSuccess;
    ...
    for (int gpu = 0; gpu < this->num_gpus; gpu++) {
      ...
      thread_slices[gpu].thread_Id = cutStartThread(
          (CUT_THREADROUTINE) &
              (SampleThread<AdvanceKernelPolicy, FilterKernelPolicy,
   ...
   */
  template <typename AdvanceKernelPolicy, typename FilterKernelPolicy>
  cudaError_t EnactSample(VertexId src) {
  ...
  struct MODE_SWITCH<Dummy, gunrock::oprtr::advance::LB> {
    static cudaError_t Enact(Enactor &enactor, VertexId src) {
      return enactor.EnactSample<LB_AdvanceKernelPolicy, FilterKernelPolicy>(
          src);
    }
  ...
  struct MODE_SWITCH<Dummy, gunrock::oprtr::advance::TWC_FORWARD> {
    static cudaError_t Enact(Enactor &enactor, VertexId src) {
      return enactor.EnactSample<TWC_AdvanceKernelPolicy, FilterKernelPolicy>(
          src);
    }
    static cudaError_t Init(Enactor &enactor, ContextPtr *context,
                            Problem *problem, int max_grid_size = 0) {
  ...
  struct MODE_SWITCH<Dummy, gunrock::oprtr::advance::LB_LIGHT> {
    static cudaError_t Enact(Enactor &enactor, VertexId src) {
      return enactor
          .EnactSample<LB_LIGHT_AdvanceKernelPolicy, FilterKernelPolicy>(src);
    }
    static cudaError_t Init(Enactor &enactor, ContextPtr *context,
                            Problem *problem, int max_grid_size = 0) {
      return enactor
          .InitSample<LB_LIGHT_AdvanceKernelPolicy, FilterKernelPolicy>(
              context, problem, max_grid_size);
    }
  };
  ...
  struct MODE_SWITCH<Dummy, gunrock::oprtr::advance::LB_CULL> {
    static cudaError_t Enact(Enactor &enactor, VertexId src) {
      return enactor
          .EnactSample<LB_CULL_AdvanceKernelPolicy, FilterKernelPolicy>(src);
    }
    static cudaError_t Init(Enactor &enactor, ContextPtr *context,
                            Problem *problem, int max_grid_size = 0) {
      return enactor
          .InitSample<LB_CULL_AdvanceKernelPolicy, FilterKernelPolicy>(
              context, problem, max_grid_size);
    }
  };
  ...
  struct MODE_SWITCH<Dummy, gunrock::oprtr::advance::LB_LIGHT_CULL> {
    static cudaError_t Enact(Enactor &enactor, VertexId src) {
      return enactor
          .EnactSample<LB_LIGHT_CULL_AdvanceKernelPolicy, FilterKernelPolicy>(
              src);
    }
    static cudaError_t Init(Enactor &enactor, ContextPtr *context,
                            Problem *problem, int max_grid_size = 0) {
      return enactor
          .InitSample<LB_LIGHT_CULL_AdvanceKernelPolicy, FilterKernelPolicy>(
              context, problem, max_grid_size);
    }
  };
  ...
  cudaError_t Init(ContextPtr *context, Problem *problem, int max_grid_size = 0,
    ...
    if (this->min_sm_version >= 300) {
      if (traversal_mode == "LB")
        ...
        printf("Traversal_mode %s is undefined for Sample problem\n",
