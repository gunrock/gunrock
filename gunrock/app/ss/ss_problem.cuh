...
struct Problem : ProblemBase<_GraphT, _FLAG> {
  ...
  struct DataSlice : BaseDataSlice {
     ...
     */
    DataSlice() : BaseDataSlice() { scan_stats.SetName("scan_stats"); }
   ...
   * \return     cudaError_t Error message(s), if any
   */
  cudaError_t Extract(VertexT *h_scan_stats,
                      util::Location target = util::DEVICE) {
    cudaError_t retval = cudaSuccess;
    ...
    if (this->num_gpus == 1) {
      ...
      if (target == util::DEVICE) {
        ...
        GUARD_CU(
            data_slice.scan_stats.SetPointer(h_scan_stats, nodes, util::HOST));
        GUARD_CU(data_slice.scan_stats.Move(util::DEVICE, util::HOST));
      } else if (target == util::HOST) {
        GUARD_CU(data_slice.scan_stats.ForEach(
            h_scan_stats,
