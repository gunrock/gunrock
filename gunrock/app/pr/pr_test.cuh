...
cudaError_t Compensate_ZeroDegrees(
    GraphT &graph,
    bool quiet = false)
{
    ...
    GUARD_CU(vertex_markers.ForEach(
        []__host__ __device__(int &marker)
        {
            marker = 0;
    ...
    {
        GUARD_CU(new_coo.edge_values.ForEach(graph_coo.edge_values,
    ...
    {
        GUARD_CU(new_coo.node_values.ForEach(graph_coo.node_values,
        []__host__ __device__(ValueT &new_value, const ValueT &old_value)
        {
            new_value = old_value;
        }, graph_coo.nodes, util::HOST));
                     ...
                     ValueT  *ranks)
{
    ...
    if (!normalize && undirected)
    {
        ...
        typedef adjacency_list< vecS, vecS, bidirectionalS, no_property,
                property<edge_index_t, VertexT> > BGraphT;
    ...
    SizeT    iteration    = 0;
    ValueT   init_value   = normalize ?
                (scale ? 1.0 : (1.0 / (ValueT)nodes)) : (1.0 - delta);
    ValueT   reset_value  = normalize ?
                (scale ? (1.0 - delta) : ((1.0 - delta) / (ValueT)nodes)) :
                (1.0 - delta);
    ...
    for (VertexT v = 0; v < nodes; v++)
    {
        if (rev_degrees[v] > 1e-6)
            rev_degrees[v] = 1 / rev_degrees[v];
    }
    ...
    {
        node_ids[v] = pr_list[v].vertex_id;
        ranks[v] = (scale & normalize) ?
            (pr_list[v].page_rank / (ValueT)nodes) : pr_list[v].page_rank;
    }
...
typename GraphT::SizeT Validate_Results(
             ...
             GraphT           &graph,
    typename GraphT::VertexT   src,
    typename GraphT::VertexT  *h_node_ids,
                     ValueT   *h_ranks,
                     ...
                     bool      verbose      = true)
{
    ...
    if (v_counts[v] == 0)
    {
        util::PrintMsg("FAIL: vertex "
            + std::to_string(v) + " does not appear in result.",
            error_count == 0 && !quiet);
        error_count ++;
    }
    ...
    if (error_count == 0)
        ...
        util::PrintMsg("number of errors : "
            + std::to_string(error_count), !quiet);
    free(unorder_ranks); unorder_ranks = NULL;
