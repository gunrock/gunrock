...
__global__ void Assign_Init_Value_Kernel(SizeT num_elements, Value init_value,
                                         SizeT *d_degrees,
  ...
  while (x < num_elements) {
    if (d_degrees[x] != 0) d_rank_current[x] = init_value / d_degrees[x];
    x += STRIDE;
  }
}
...
// true> // _KEEP_NODE_NUM
{
  ...
  struct DataSlice : BaseDataSlice {
    ...
    util::Array1D<SizeT, Value>
        rank_curr; /**< Used for ping-pong page rank value */
    util::Array1D<SizeT, Value>
        rank_next; /**< Used for ping-pong page rank value */
    ...
    cudaError_t Release() {
      ...
      // if (temp_keys_out != NULL) {delete[] temp_keys_out; temp_keys_out =
      // NULL;} if (retval = context     .Release()) return retval;
    ...
    cudaError_t Init(int num_gpus, int gpu_idx, bool use_double_buffer,
      ...
      if (num_gpus > 1)
      {
          ...
          if (retval = this->keys_out[0].Allocate(local_nodes, util::DEVICE))
      return retval; this->keys_outs[0] =
      this->keys_out[0].GetPointer(util::DEVICE); for (int peer_ = 0; peer_ <
      num_gpus; peer_++)
          {
              if (peer_ == 0)
              {// only need the first one, can be reused
                  if (retval = this->keys_marker[peer_].EnsureSize(nodes))
      return retval; } else { if (retval = this->keys_marker[peer_].Release())
      return retval;
              }
              this->keys_markers[peer_] =
      this->keys_marker[peer_].GetPointer(util::DEVICE);
          }
      ...
      for (int peer_ = 2; peer_ < num_gpus; peer_++)
      {
          ...
          //this->keys_out
      [peer_].SetPointer(this->keys_out[1].GetPointer(util::DEVICE),
      this->keys_out[1].GetSize(), util::DEVICE); this->keys_outs[peer_] =
      this->keys_out[1].GetPointer(util::DEVICE);

          //this->value__associate_out[peer_][0].SetPointer(this->value__associate_out[1][0].GetPointer(util::DEVICE),
      this->value__associate_out[1][0].GetSize(), util::DEVICE);
          //this->value__associate_outs[peer_][0] =
      this->value__associate_out[1][0].GetPointer(util::DEVICE);
          //if (retval = this->value__associate_outs[peer_].Move(util::HOST,
      ...
      // if (retval = degrees_pong.Allocate(nodes+1, util::DEVICE)) return
      // retval; if (retval = node_ids    .Allocate(nodes, util::DEVICE)) return
      // retval; if (retval = markers     .Allocate(nodes, util::DEVICE)) return
      // retval;

      if (this->num_gpus == 1) {
        // local_nodes = nodes;

        // if (retval = this -> frontier_queues[0].keys[0].Release()) return
        // retval; if (this ->
        // frontier_queues[0].keys[0].GetPointer(util::DEVICE) == NULL)
        //    if (retval = this -> frontier_queues[0].keys[0].Allocate(nodes,
        //    util::DEVICE))
        //        return retval;
        ...
        remote_vertices_out =
            new util::Array1D<SizeT, VertexId>[this->num_gpus];
    ...
    cudaError_t Reset(VertexId src, Value delta, Value threshold,
                      SizeT max_iter, bool scaled, FrontierType frontier_type,
  ...
  cudaError_t Init(bool stream_from_host,  // Only meaningful for single-GPU
    ...
    /*if (this->num_gpus > 1)
    {
        ...
        for (int peer=0; peer<this->num_gpus; peer++)
        {
            ...
            SizeT max_nodes = local_nodes[gpu] > local_nodes[peer]?
    local_nodes[gpu] : local_nodes[peer];
    ...
    for (int gpu = 0; gpu < this->num_gpus; gpu++) {
      ...
      data_slice_->streams.SetPointer(streams + gpu * num_gpus * 2,
                                      num_gpus * 2);
      // data_slice_ -> context.SetPointer(context + gpu * num_gpus * 2,
      // num_gpus * 2);
      ...
      for (int peer = 0; peer < this->num_gpus; peer++) {
        ...
        if (gpu != 0) {
          data_slices[gpu]->remote_vertices_in[peer_].SetPointer(
              data_slices[peer]->remote_vertices_out[gpu_].GetPointer(
                  util::HOST),
