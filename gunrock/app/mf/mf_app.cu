...
float runMF(GRGraph* output, MF_Parameter *parameter)
{
    ...
    for (int i = 0; i < num_iters; ++i)
    {
        ...
        util::GRError(
                enactor->Reset(), "MF Enactor Reset failed", __FILE__, __LINE__);
...
{
    MF_Parameter *parameter = new MF_Parameter;
...
/*
 * @brief Entry of gunrock_mf function
 *
 * @param[out] grapho Pointer to output graph structure of the problem
 * @param[in]  graphi Pointer to input graph we need to process on
    ...
    const GRTypes  data_t)
{
    ...
    return dispatchMF(grapho, graphi, config, data_t, context, streams);
}
