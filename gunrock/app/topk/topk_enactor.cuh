...
class TOPKEnactor : public EnactorBase<typename _Problem::SizeT> {
 ...
 public:
   ...
   */
  TOPKEnactor(int num_gpus = 1, int *gpu_idx = NULL, bool instrument = false,
  ...
  cudaError_t InitTOPK(ContextPtr *context, Problem *problem,
                       int max_grid_size = 0) {
   ...
   */
  template <typename AdvanceKernelPolicy, typename FilterKernelPolicy>
  cudaError_t EnactTOPK(SizeT top_nodes) {
    ...
    util::MemsetCopyVectorKernel<<<128, 128>>>(
        ...
        data_slice->degrees_o.GetPointer(util::DEVICE), nodes);
    util::MemsetAddVectorKernel<<<128, 128>>>(
        data_slice->degrees_s.GetPointer(util::DEVICE),
        data_slice->degrees_i.GetPointer(util::DEVICE), nodes);

    // sort node_ids by degree centralities
    util::MemsetCopyVectorKernel<<<128, 128>>>(
        data_slice->temp_i.GetPointer(util::DEVICE),
        data_slice->degrees_s.GetPointer(util::DEVICE), nodes);
    util::MemsetCopyVectorKernel<<<128, 128>>>(
        data_slice->temp_o.GetPointer(util::DEVICE),
        data_slice->degrees_s.GetPointer(util::DEVICE), nodes);

    util::CUBRadixSort<SizeT, VertexId>(
        false, nodes, data_slice->degrees_s.GetPointer(util::DEVICE),
        data_slice->node_id.GetPointer(util::DEVICE));
    util::CUBRadixSort<SizeT, SizeT>(
        false, nodes, data_slice->temp_i.GetPointer(util::DEVICE),
        data_slice->degrees_i.GetPointer(util::DEVICE));
    util::CUBRadixSort<SizeT, SizeT>(
        false, nodes, data_slice->temp_o.GetPointer(util::DEVICE),
  ...
  cudaError_t Init(ContextPtr *context, Problem *problem,
    ...
    if (min_sm_version >= 300) {
      return InitTOPK<AdvanceKernelPolicy, FilterKernelPolicy>(context, problem,
                                                               max_grid_size);
    }
   ...
   * calls.
   */
  cudaError_t Enact(SizeT top_nodes) {
    int min_sm_version = -1;
    for (int i = 0; i < this->num_gpus; i++)
      if (min_sm_version == -1 ||
          this->cuda_props[i].device_sm_version < min_sm_version) {
