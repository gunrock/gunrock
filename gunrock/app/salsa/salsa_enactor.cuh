...
class SALSAEnactor : public EnactorBase<typename _Problem::SizeT> {
 ...
 public:
   ...
   */
  SALSAEnactor(int num_gpus = 1, int* gpu_idx = NULL, bool instrument = false,
  ...
  void NormalizeRank(
      // Problem *problem,
      // CudaContext &context,
      int hub_or_auth, cudaStream_t stream = 0) {
    Value* rank_curr;
    Value* rank_next;
    if (hub_or_auth == 0) {
  ...
  cudaError_t InitSALSA(ContextPtr* context, Problem* problem,
                        int max_grid_size = 0) {
   ...
   */
  template <typename AdvanceKernelPolicy, typename FilterKernelPolicy>
  cudaError_t EnactSALSA(SizeT max_iteration) {
    typedef HFORWARDFunctor<VertexId, SizeT, Value, Problem> HForwardFunctor;

    typedef AFORWARDFunctor<VertexId, SizeT, Value, Problem> AForwardFunctor;

    typedef HBACKWARDFunctor<VertexId, SizeT, Value, Problem> HBackwardFunctor;

    typedef ABACKWARDFunctor<VertexId, SizeT, Value, Problem> ABackwardFunctor;
    ...
    // Step through SALSA iterations
    {
      ...
      gunrock::oprtr::advance::LaunchKernel<AdvanceKernelPolicy, Problem,
          ...
          typename HForwardFunctor::LabelT(), data_slice, d_data_slice,
      ...
      gunrock::oprtr::advance::LaunchKernel<AdvanceKernelPolicy, Problem,
                                            ...
                                            gunrock::oprtr::advance::V2E>(
          enactor_stats[0], frontier_attribute[0],
          typename AForwardFunctor::LabelT(), data_slice, d_data_slice,
          (VertexId*)NULL, (bool*)NULL, (bool*)NULL, d_scanned_edges,
          frontier_queue->keys[frontier_attribute->selector].GetPointer(
              util::DEVICE),  // d_in_queue
          frontier_queue->keys[frontier_attribute->selector ^ 1].GetPointer(
              util::DEVICE),  // d_out_queue
          (Value*)NULL,       // d_pred_in_queue
          (Value*)NULL, graph_slice->column_offsets.GetPointer(util::DEVICE),
    ...
    while (true) {
      ...
      gunrock::oprtr::advance::LaunchKernel<AdvanceKernelPolicy, Problem,
          ...
          enactor_stats[0], frontier_attribute[0],
          typename HBackwardFunctor::LabelT(), data_slice, d_data_slice,
          (VertexId*)NULL, (bool*)NULL, (bool*)NULL, d_scanned_edges,
          frontier_queue->keys[frontier_attribute->selector].GetPointer(
              util::DEVICE),  // d_in_queue
          frontier_queue->keys[frontier_attribute->selector ^ 1].GetPointer(
          ...
          work_progress[0], context[0], stream, false, true);
      ...
      gunrock::oprtr::advance::LaunchKernel<AdvanceKernelPolicy, Problem,
                                            ...
                                            gunrock::oprtr::advance::E2V>(
          enactor_stats[0], frontier_attribute[0],
          typename ABackwardFunctor::LabelT(), data_slice, d_data_slice,
          (VertexId*)NULL, (bool*)NULL, (bool*)NULL, d_scanned_edges,
          frontier_queue->keys[frontier_attribute->selector].GetPointer(
              util::DEVICE),  // d_in_queue
          frontier_queue->keys[frontier_attribute->selector ^ 1].GetPointer(
  ...
  cudaError_t Enact(SizeT max_iteration) {
    ...
    if (min_sm_version >= 300) {
      return EnactSALSA<AdvanceKernelPolicy, FilterKernelPolicy>(max_iteration);
    }
