...
struct HBACKWARDFunctor {
  ...
  static __device__ __forceinline__ void ApplyEdge(
      VertexId s_id, VertexId d_id, DataSlice *d_data_slice, SizeT edge_id,
      VertexId input_item, LabelT label, SizeT input_pos, SizeT &output_pos) {
    Value hrank_dst = (d_data_slice->in_degrees[s_id] == 0 ||
                       d_data_slice->out_degrees[d_id] == 0)
                          ? 0
                          : d_data_slice->hrank_curr[d_id] /
                                (d_data_slice->in_degrees[s_id] *
                                 d_data_slice->out_degrees[d_id]);
    VertexId v_id = d_data_slice->hub_predecessors[input_pos];
...
struct ABACKWARDFunctor {
  ...
  static __device__ __forceinline__ void ApplyEdge(
      VertexId s_id, VertexId d_id, DataSlice *d_data_slice, SizeT edge_id,
      VertexId input_item, LabelT label, SizeT input_pos, SizeT &output_pos) {
    Value arank_dst = (d_data_slice->out_degrees[s_id] == 0 ||
                       d_data_slice->in_degrees[d_id] == 0)
                          ? 0
                          : d_data_slice->arank_curr[d_id] /
                                (d_data_slice->out_degrees[s_id] *
                                 d_data_slice->in_degrees[d_id]);
    VertexId v_id = d_data_slice->auth_predecessors[input_pos];
