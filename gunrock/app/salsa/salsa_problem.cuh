...
// false> // KEEP_NODE_NUM
{
  ...
  struct DataSlice : BaseDataSlice {
    ...
    util::Array1D<SizeT, SizeT>
        in_degrees; /**< Used for keeping in-degree for each vertex */
    util::Array1D<SizeT, SizeT>
        out_degrees; /**< Used for keeping out-degree for each vertex */
    util::Array1D<SizeT, VertexId>
        hub_predecessors; /**< Used for keeping the predecessor for each vertex
                           */
    util::Array1D<SizeT, VertexId>
        auth_predecessors; /**< Used for keeping the predecessor for each vertex
                            */
  };
    ...
    do {
      if (num_gpus <= 1) {
        ...
        data_slices[gpu][0].streams.SetPointer(streams + gpu * num_gpus * 2,
                                               num_gpus * 2);
        if (retval = data_slices[gpu]->Init(this->num_gpus, this->gpu_idx[gpu],
                                            this->use_double_buffer,
                                            // 0,
                                            // 0,
                                            hub_graph, NULL, NULL, in_sizing))
          return retval;
        ...
        if (retval = data_slices[gpu]->in_degrees.Allocate(this->nodes,
                                                           util::DEVICE))
          return retval;

        data_slices[gpu]->hub_predecessors.SetName("hub_predecessors");
        if (retval = data_slices[gpu]->hub_predecessors.Allocate(this->edges,
                                                                 util::DEVICE))
          return retval;

        data_slices[gpu]->auth_predecessors.SetName("auth_predecessors");
        if (retval = data_slices[gpu]->auth_predecessors.Allocate(this->edges,
                                                                  util::DEVICE))
          return retval;

        data_slices[gpu]->out_degrees.SetName("out_degrees");
        if (retval = data_slices[gpu]->out_degrees.Allocate(this->nodes,
                                                            util::DEVICE))
          return retval;
      }
  ...
  cudaError_t Reset(
    ...
    for (int gpu = 0; gpu < this->num_gpus; ++gpu) {
      ...
      util::MemsetKernel<<<128, 128>>>(
          data_slices[gpu]->hrank_next.GetPointer(util::DEVICE), (Value)0,
          this->nodes);

      util::MemsetKernel<<<128, 128>>>(
          data_slices[gpu]->arank_next.GetPointer(util::DEVICE), (Value)0,
          this->nodes);

      util::MemsetKernel<<<128, 128>>>(
          data_slices[gpu]->out_degrees.GetPointer(util::DEVICE), (SizeT)0,
          this->nodes);

      util::MemsetKernel<<<128, 128>>>(
          data_slices[gpu]->in_degrees.GetPointer(util::DEVICE), (SizeT)0,
          this->nodes);
