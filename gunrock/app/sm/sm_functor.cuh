	    ...
	    for(VertexId i=0; i < d_data_slice->nodes_query; i++)
            {
   	        if ((d_data_slice->tos_data [node] == d_data_slice->froms_data[i]) && 
		        (d_data_slice->d_data_degrees[node] >= d_data_slice->tos[i]))
		{
			d_data_slice->d_c_set[node+i*d_data_slice->nodes_data]=1;
			d_data_slice->froms[node]=1;
		}
		else
			d_data_slice->d_c_set[node+i*d_data_slice->nodes_data]=0;
...
struct UpdateDegreeFunctor
{
    ...
    {
        if (s_id >= 0 && d_id >= 0 && s_id < d_data_slice->nodes_data && d_id < d_data_slice->nodes_data)
        {
	        #pragma unroll
	        for(VertexId i=0; i < d_data_slice->nodes_query; i++)
	            if(d_data_slice -> d_c_set[s_id + i * d_data_slice->nodes_data]==1)
	    	        return true;
        }
        return false;
    }
        ...
        VertexId e_id = 0, VertexId e_id_in = 0)
    {
	    if (d_data_slice -> froms[d_id]==0) 
	        atomicSub( d_data_slice -> d_data_degrees + s_id, (SizeT)1);
    }
}; // UpdateDegreeFunctor
...
struct PruneFunctor
{
    ...
    */
        ...
        VertexId e_id = 0, VertexId e_id_in = 0)
    {
        ...
        for(VertexId i=0; i< d_data_slice->nodes_query; i++)
        {
            if(d_data_slice -> d_c_set[s_id + i * d_data_slice->nodes_data] != 0)
            {
    ...
    #pragma unroll
        for(int i=0; i < d_data_slice -> edges_query; i++)
        {
	        ...
	        if (d_data_slice -> d_c_set[s_id + source * d_data_slice->nodes_data]==1 && 
	            ...
	            d_data_slice -> froms[e_id]|=(1<<i); // label the candidate edge with the index of its query edge plus 1
	    }
    }
