...
/**
 * @brief A* Path Finding Problem structure stores device-side vectors for doing
 * A* computing on the GPU.
 *
...
template <typename VertexId, typename SizeT, typename Value,
          ...
          bool _MARK_PATHS, int DISTANCE_HEURISTIC>
struct ASTARProblem
...
// false> //KEEP_NODE_NUM
{
  ...
  static const int MAX_NUM_VERTEX_ASSOCIATES =
      MARK_PATHS ? 1 : 0;  // MARK_PREDECESSORS ? 1 : 0;
  ...
  struct DataSlice : BaseDataSlice {
    ...
    util::Array1D<SizeT, VertexId>
        dst_node; /**< Used for storing dst node ID */
    util::Array1D<SizeT, int>
        quit_flag; /**< Finish flag for label propagation, indicates that all
                      labels are stable.*/
    util::Array1D<SizeT, Value>
        sample_weight; /**< Used for storing sample weight (avg or min)*/
    ...
    cudaError_t Init(int num_gpus, int gpu_idx, bool use_double_buffer,
                     ...
                     Value *latitudes, Value *longitudes,
                     // VertexId *original_vertex,
                     int delta_factor = 16, float queue_sizing = 2.0,
                     float in_sizing = 1.0, bool skip_makeout_selection = false,
      ...
      if (retval = BaseDataSlice::Init(num_gpus, gpu_idx, use_double_buffer,
                                       ...
                                       in_sizing, skip_makeout_selection))
        return retval;
      ...
      if (DISTANCE_HEURISTIC) {
        ...
        if (retval = bfs_levels.Allocate(graph->nodes, util::DEVICE))
          return retval;
      ...
      if (num_gpus > 1) {
        ...
        if (MARK_PATHS)  //(MARK_PREDECESSORS)
        {
          ...
          if (retval =
                  this->vertex_associate_orgs.Move(util::HOST, util::DEVICE))
            return retval;
        }
        if (retval = this->value__associate_orgs.Move(util::HOST, util::DEVICE))
          return retval;
      }
    ...
    cudaError_t Reset(FrontierType frontier_type,
                      GraphSlice<VertexId, SizeT, Value> *graph_slice,
                      double queue_sizing = 2.0, double queue_sizing1 = -1.0) {
      cudaError_t retval = cudaSuccess;
      ...
      for (int gpu = 0; gpu < this->num_gpus; gpu++) this->wait_marker[gpu] = 0;
      for (int i = 0; i < 4; i++)
      ...
      for (int peer = 0; peer < (this->num_gpus > 1 ? this->num_gpus + 1 : 1);
        ...
        for (int i = 0; i < 2; i++) {
          ...
          switch (frontier_type) {
            case VERTEX_FRONTIERS:
              ...
              new_frontier_elements[0] =
                  double(this->num_gpus > 1 ? graph_slice->in_counter[peer]
                                            : nodes) *
                      queue_sizing_ +
                  2;
          ...
          // for (int i = 0; i < 2; i++)
          {
            ...
            // if (peer == this->num_gpus && new_frontier_elements[i] > nodes *
            // this->num_gpus) new_frontier_elements[i] = nodes * this->num_gpus;
      ...
      if (this->visit_lookup.GetPointer(util::DEVICE) == NULL)
        if (retval = this->visit_lookup.Allocate(nodes, util::DEVICE))
          return retval;
      ...
      util::MemsetKernel<<<256, 1024>>>(this->g_cost.GetPointer(util::DEVICE),
                                        util::MaxValue<Value>(), nodes);

      util::MemsetKernel<<<256, 1024>>>(this->f_cost.GetPointer(util::DEVICE),
                                        util::MaxValue<Value>(), nodes);

      util::MemsetKernel<<<256, 1024>>>(this->labels.GetPointer(util::DEVICE),
      ...
      util::MemsetKernel<<<256, 1024>>>(
          this->visit_lookup.GetPointer(util::DEVICE), (VertexId)-1, nodes);
      util::MemsetKernel<<<256, 1024>>>(marker.GetPointer(util::DEVICE), (int)0,
                                        nodes);
  ...
  cudaError_t Extract(VertexId *h_preds) {
    ...
    if (this->num_gpus == 1) {
      ...
      if (MARK_PATHS)  //(MARK_PREDECESSORS)
      {
        ...
        if (retval = data_slices[0]->preds.Move(util::DEVICE, util::HOST))
          return retval;
      }
      ...
      if (MARK_PATHS)  //(MARK_PREDECESSORS)
        for (VertexId node = 0; node < this->nodes; node++)
          h_preds[node] = th_preds[this->partition_tables[0][node]]
                                  [this->convertion_tables[0][node]];
      for (int gpu = 0; gpu < this->num_gpus; gpu++) {
  ...
  cudaError_t Init(bool stream_from_host,  // Only meaningful for single-GPU
                   ...
                   cudaStream_t *streams = NULL, int delta_factor = 16,
                   float queue_sizing = 2.0, float in_sizing = 1.0,
                   float partition_factor = -1.0, int partition_seed = -1) {
    cudaError_t retval = cudaSuccess;
    ...
    for (int gpu = 0; gpu < this->num_gpus; gpu++) {
      ...
      _data_slice->streams.SetPointer(&streams[gpu * num_gpus * 2],
                                      num_gpus * 2);
      ...
      if (retval = _data_slice->Init(
              ...
              this->num_gpus > 1
                  ...
                  : NULL,
              // this->num_gpus > 1?
              // this->graph_slices[gpu]->original_vertex.GetPointer(util::HOST)
              // : NULL,
              latitudes, longitudes, delta_factor, queue_sizing, in_sizing,
   ...
   *  \return cudaError_t object Indicates the success of all CUDA calls.
   */
  cudaError_t Reset(VertexId src, VertexId dst, Value sample_weight,
                    FrontierType frontier_type, double queue_sizing,
    ...
    if (retval = util::GRError(
            cudaMemcpy(data_slices[gpu]->frontier_queues[0].keys[0].GetPointer(
                           util::DEVICE),
                       &tsrc, sizeof(VertexId), cudaMemcpyHostToDevice),
            "ASTARProblem cudaMemcpy frontier_queues failed", __FILE__,
            __LINE__))
      return retval;

    if (retval = util::GRError(
            cudaMemcpy(data_slices[gpu]->dst_node.GetPointer(util::DEVICE),
                       &dst, sizeof(VertexId), cudaMemcpyHostToDevice),
            "ASTARProblem cudaMemcpy dst_node failed", __FILE__, __LINE__))
      return retval;

    if (retval = util::GRError(
            cudaMemcpy(data_slices[gpu]->sample_weight.GetPointer(util::DEVICE),
                       &sample_weight, sizeof(Value), cudaMemcpyHostToDevice),
            "ASTARProblem cudaMemcpy sample_weight failed", __FILE__, __LINE__))
      return retval;
    Value src_distance = 0;
    if (retval = util::GRError(
            cudaMemcpy(data_slices[gpu]->g_cost.GetPointer(util::DEVICE) + tsrc,
                       &src_distance, sizeof(Value), cudaMemcpyHostToDevice),
            "ASTARProblem cudaMemcpy frontier_queues failed", __FILE__,
            __LINE__))
      return retval;
    if (retval = util::GRError(
            cudaMemcpy(data_slices[gpu]->f_cost.GetPointer(util::DEVICE) + tsrc,
                       &src_distance, sizeof(Value), cudaMemcpyHostToDevice),
            "ASTARProblem cudaMemcpy frontier_queues failed", __FILE__,
            __LINE__))
      return retval;
    ...
    if (MARK_PATHS)  //(MARK_PREDECESSORS)
    {
      ...
      if (retval = util::GRError(
              cudaMemcpy(
                  ...
                  &src_pred, sizeof(Value), cudaMemcpyHostToDevice),
              "ASTARProblem cudaMemcpy frontier_queues failed", __FILE__,
              __LINE__))
        return retval;
    }
