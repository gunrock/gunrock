...
__global__ void Expand_Incoming_ASTAR(const SizeT num_elements,
                                                          ...
                                                          // UPDATE_PREDECESSORS
{
  ...
  typedef ASTARFunctor<VertexId, SizeT, Value, Problem> GraphFunctor;

  typedef ASTARDistanceFunctor<VertexId, SizeT, Value, Problem> DistanceFunctor;
  ...
  static void FullQueue_Core(
      ...
      cudaStream_t stream) {
    typedef gunrock::priority_queue::KernelPolicy<Problem, 300, false, 8, 10>
    ...
    // printf("Advance:\n");
    // util::DisplayDeviceResults(frontier_queue->keys[frontier_attribute->selector]
    // .GetPointer(util::DEVICE), frontier_attribute -> queue_length);
    // Edge Map
    ...
    // util::DisplayDeviceResults(frontier_queue->keys[frontier_attribute->selector^1]
    // .GetPointer(util::DEVICE), frontier_attribute -> output_length[0]);
    ...
    if (!gunrock::oprtr::advance::isFused<
      ...
      gunrock::oprtr::filter::LaunchKernel<FilterKernelPolicy, Problem,
          ...
          data_slice, d_data_slice, (SizeT *)NULL, (unsigned char *)NULL,
          frontier_queue->keys[frontier_attribute->selector].GetPointer(
              util::DEVICE),  // d_in_queue
          frontier_queue->keys[frontier_attribute->selector ^ 1].GetPointer(
              util::DEVICE),  // d_out_queue
          (Value *)NULL, (Value *)NULL, frontier_attribute->output_length[0],
          graph_slice->nodes, work_progress[0], context[0], stream,
          frontier_queue->keys[frontier_attribute->selector]
              .GetSize(),  // max_in_queue
          frontier_queue->keys[frontier_attribute->selector ^ 1]
              .GetSize(),  // max_out_queue
    ...
    // queue, put near pile as the input queue for next round.
    if (data_slice->num_gpus == 1) {  // when #gpus == 1, use priority queue
      frontier_attribute->queue_index++;
      frontier_attribute->selector ^= 1;
      ...
      level and split the far pile. if ( out_length == 0 && pq->queue_length >
      0) { while (pq->queue_length > 0 && out_length == 0) { pq->selector ^= 1;
              pq->level++;
              out_length = gunrock::priority_queue::Bisect
                  <PriorityQueueKernelPolicy, Problem, NearFarPriorityQueue,
      PqFunctor>( (int*)pq->nf_pile[0]->d_queue[pq->selector^1], pq, (unsigned
      int)pq->queue_length, d_data_slice,
      ...
      pq->queue_length); if (out_length > 0) { if (enactor_stats->retval =
      work_progress->SetQueueLength(frontier_attribute->queue_index,
      out_length)) return;
              }
          }
    ...
    if (enactor_stats->retval = work_progress->GetQueueLength(
            ...
            false, stream, true))
      return;
    // Move the device quit_flag to host. If frontier is empty, unreachable
    // node, so directly set quit_flag to 1
    // data_slice -> quit_flag.Move(util::DEVICE, util::HOST, 1, 0, stream);
    /*if (frontier_attribute->queue_length == 0)
  ...
  static void Iteration_Update_Preds(
      ...
      SizeT num_elements,
      cudaStream_t stream) {  // do nothing here
  }
  /*
   * @brief Expand incoming function.
   *
  ...
  static void Expand_Incoming_Old(Enactor *enactor, int grid_size,
    ...
    Expand_Incoming_ASTAR<VertexId, SizeT, Value, NUM_VERTEX_ASSOCIATES,
  ...
  NUM_VALUE__ASSOCIATES> ( enactor, thread_num, num_elements, num_gpus,
  ...
  gpu%num_gpus, cudaGetErrorString(enactor_stats[gpu].retval)); fflush(stdout);
          return true;
      }
  ...
  data_slice[gpu]->PR_queue_length); reached_final = false;
      }
...
template <typename AdvanceKernelPolicy, typename FilterKernelPolicy,
          typename Enactor>
static CUT_THREADPROC ASTARThread(void *thread_data_) {
  typedef typename Enactor::Problem Problem;
  typedef typename Enactor::SizeT SizeT;
  typedef typename Enactor::VertexId VertexId;
  typedef typename Enactor::Value Value;
  ...
  while (thread_data->status != ThreadSlice::Status::ToKill) {
    ...
    for (int peer_ = 0; peer_ < num_gpus; peer_++) {
      ...
      frontier_attribute[peer_].selector =
          0;  // frontier_attrbute[peer_].queue_length == 0 ? 0 : 1;
    ...
    gunrock::app::Iteration_Loop<
        ...
        Problem::MARK_PATHS ? 1 : 0, 1>(thread_data);
...
class ASTAREnactor
 ...
 public:
   ...
   */
  ASTAREnactor(int num_gpus = 1, int *gpu_idx = NULL, bool instrument = false,
   ...
   */
  template <typename AdvanceKernelPolicy, typename FilterKernelPolicy>
  cudaError_t InitASTAR(ContextPtr *context, Problem *problem,
                        int max_grid_size = 0)
  // bool        size_check    = true)
  {
    ...
    for (int gpu = 0; gpu < this->num_gpus; gpu++) {
      ...
      bitmask_desc; if (retval = util::GRError(cudaBindTexture( 0,
              gunrock::oprtr::filter::BitmaskTex<unsigned
      char>::ref,//ts_bitmask[gpu],
    ...
    for (int gpu = 0; gpu < this->num_gpus; gpu++) {
      ...
      thread_slices[gpu].thread_Id = cutStartThread(
          (CUT_THREADROUTINE) &
              (ASTARThread<AdvanceKernelPolicy, FilterKernelPolicy,
   ...
   */
  template <typename AdvanceKernelPolicy, typename FilterKernelPolicy>
  cudaError_t EnactASTAR(VertexId src) {
  ...
  struct MODE_SWITCH<Dummy, gunrock::oprtr::advance::LB> {
    static cudaError_t Enact(Enactor &enactor, VertexId src) {
      return enactor.EnactASTAR<LB_AdvanceKernelPolicy, FilterKernelPolicy>(
          src);
    }
  ...
  struct MODE_SWITCH<Dummy, gunrock::oprtr::advance::TWC_FORWARD> {
    static cudaError_t Enact(Enactor &enactor, VertexId src) {
      return enactor.EnactASTAR<TWC_AdvanceKernelPolicy, FilterKernelPolicy>(
          src);
    }
    static cudaError_t Init(Enactor &enactor, ContextPtr *context,
                            Problem *problem, int max_grid_size = 0) {
  ...
  struct MODE_SWITCH<Dummy, gunrock::oprtr::advance::LB_LIGHT> {
    static cudaError_t Enact(Enactor &enactor, VertexId src) {
      return enactor
          .EnactASTAR<LB_LIGHT_AdvanceKernelPolicy, FilterKernelPolicy>(src);
    }
    static cudaError_t Init(Enactor &enactor, ContextPtr *context,
                            Problem *problem, int max_grid_size = 0) {
      return enactor
          .InitASTAR<LB_LIGHT_AdvanceKernelPolicy, FilterKernelPolicy>(
              context, problem, max_grid_size);
    }
  };
  ...
  struct MODE_SWITCH<Dummy, gunrock::oprtr::advance::LB_CULL> {
    static cudaError_t Enact(Enactor &enactor, VertexId src) {
      return enactor
          .EnactASTAR<LB_CULL_AdvanceKernelPolicy, FilterKernelPolicy>(src);
    }
    static cudaError_t Init(Enactor &enactor, ContextPtr *context,
                            Problem *problem, int max_grid_size = 0) {
  ...
  struct MODE_SWITCH<Dummy, gunrock::oprtr::advance::LB_LIGHT_CULL> {
    static cudaError_t Enact(Enactor &enactor, VertexId src) {
      return enactor
          .EnactASTAR<LB_LIGHT_CULL_AdvanceKernelPolicy, FilterKernelPolicy>(
              src);
    }
    static cudaError_t Init(Enactor &enactor, ContextPtr *context,
                            Problem *problem, int max_grid_size = 0) {
      return enactor
          .InitASTAR<LB_LIGHT_CULL_AdvanceKernelPolicy, FilterKernelPolicy>(
              context, problem, max_grid_size);
    }
  };
  ...
  cudaError_t Enact(VertexId src, std::string traversal_mode = "LB") {
    if (this->min_sm_version >= 300) {
      ...
      //            if (traversal_mode == 0)
      //                return EnactASTAR< LBAdvanceKernelPolicy,
      //                FilterKernelPolicy>(src);
      //            else
      //                return EnactASTAR<FWDAdvanceKernelPolicy,
      //                FilterKernelPolicy>(src);
    }
  ...
  cudaError_t Init(ContextPtr *context, Problem *problem, int max_grid_size = 0,
    ...
    if (this->min_sm_version >= 300) {
      if (traversal_mode == "LB")
        ...
        printf("Traversal_mode %s is undefined for A*\n",
               traversal_mode.c_str());
      //            if (traversal_mode == 0)
      //                return InitASTAR< LBAdvanceKernelPolicy,
      //                FilterKernelPolicy>(
      //                    context, problem, max_grid_size);
      //            else
      //                return InitASTAR<FWDAdvanceKernelPolicy,
      //                FilterKernelPolicy>(
      //                    context, problem, max_grid_size);
    }
