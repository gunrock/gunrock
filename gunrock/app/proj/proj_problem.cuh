...
struct Problem : ProblemBase<_GraphT, _FLAG> {
  ...
  struct DataSlice : BaseDataSlice {
     ...
     */
    DataSlice() : BaseDataSlice() { projections.SetName("projections"); }
    ...
    cudaError_t Init(GraphT &sub_graph, int num_gpus, int gpu_idx,
      ...
      // size_t required_mem = sub_graph.nodes * sub_graph.nodes *
      // sizeof(ValueT); printf("free=%lu | total=%lu | required_mem=%lu\n",
      // free, total, required_mem); if(required_mem > free) {
      //     return retval;
...
...
   * \return     cudaError_t Error message(s), if any
   */
  cudaError_t Extract(ValueT *h_projections,
    ...
    if (this->num_gpus == 1) {
      ...
      if (target == util::DEVICE) {
        ...
        GUARD_CU(data_slice.projections.SetPointer(h_projections, nodes * nodes,
                                                   util::HOST));
        GUARD_CU(data_slice.projections.Move(util::DEVICE, util::HOST));

      } else if (target == util::HOST) {
        GUARD_CU(data_slice.projections.ForEach(
            h_projections,
