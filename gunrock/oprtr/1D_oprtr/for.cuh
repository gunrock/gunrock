    ...
    int            block_size = util::PreDefinedValues<int>::InvalidValue)
{
    ...
    if ((target & util::HOST) == util::HOST)
    {
        ...
        for (ForIterT i=0; i<loop_size; i++)
            op(i);
    }
    ...
    int            block_size = util::PreDefinedValues<int>::InvalidValue)
{
    ...
    if ((target & util::DEVICE) == util::DEVICE)
    {
        ...
        retval = cudaLaunchCooperativeKernel((void*)RepeatFor0_Kernel<OpT>,
    ...
    if ((target & util::DEVICE) == util::DEVICE)
    {
        ...
        if (first_in)
        {
            ...
            for (int r = 0; r < num_repeats; r++)
            {
                ...
                retval = util::GRError(cudaGraphAddKernelNode(
                    &kernel_node, cuda_graph, node_dependencies.data(),
            ...
            retval = util::GRError(cudaGraphInstantiate(&graph_exec, cuda_graph, NULL, NULL, 0),
