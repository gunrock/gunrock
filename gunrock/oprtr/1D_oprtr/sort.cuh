    ...
    cudaStream_t stream) // = 0)
{
    ...
    if (target == util::LOCATION_DEFAULT)
        target = this -> setted | this -> allocated;

    if (target & DEVICE)
    {
        thrust::sort(thrust::cuda::par.on(stream),
            this -> GetPointer(util::DEVICE) + offset,
            this -> GetPointer(util::DEVICE) + (offset + length),
            compare);
        if (target & HOST)
        {
        ...
        if (retval = util::omp_sort((*this) + offset,
            length, compare))
        return retval;
    }
    return retval;
    ...
    cudaStream_t stream)// = 0)
{
    ...
    if (target & DEVICE)
    {
        thrust::device_ptr<ValueT> org_values(
            this -> GetPointer(DEVICE) + offset);
        thrust::device_ptr<AuxValueT> aux_values(
            array_in.GetPointer(DEVICE) + offset);
        thrust::device_vector<ValueT> org_vector(
            org_values, org_values + length);
        thrust::device_vector<AuxValueT> aux_vector(
            aux_values, aux_values + length);
        thrust::sort_by_key(thrust::cuda::par.on(stream),
            org_vector.begin(), org_vector.end(),
            aux_vector.begin(), compare);
        if (target & HOST)
        {
            if (retval = this -> Move(DEVICE, HOST,
                length, offset, stream))
                return retval;
            if (retval = array_in.Move(DEVICE, HOST,
                length, offset, stream))
                return retval;
        }
    } else {
        // perpare temp array for sorting
        Array1D<SizeT, tValueT, FLAG, cudaHostRegisterFlag>
            temp_array;
        temp_array.SetName("Array1D::Sort::temp_array");
        if (retval = temp_array.Allocate(length, target))
            return retval;
        if (retval = oprtr::ForEach(temp_array + 0, (*this) + offset,
            array_in + offset, []__host__ __device__ (tValueT &temp_value,
                const ValueT &value, const AuxValueT &value_in){
        ...
        if (retval = oprtr::ForEach(temp_array + 0,
            (*this) + offset, array_in + offset,
            []__host__ __device__(
                const tValueT &temp_value,
                ValueT &value, AuxValueT &value_out){
