...
struct Dispatch<KernelPolicy, Problem, Functor, ADVANCE_TYPE, R_TYPE, R_OP,
  ...
  static __device__ __forceinline__ SizeT Binary_Search(T *data, T item_to_find,
    ...
    while (lower_bound < upper_bound) {
      ...
      // printf("(%d, %d) looking for %d, current range [%d, %d], mid_point =
      // %d\n",
      //    blockIdx.x, threadIdx.x, item_to_find, lower_bound, upper_bound,
      //    data[mid_point]);
    ...
    //    blockIdx.x, threadIdx.x, lower_bound, upper_bound, data[upper_bound],
    //    retval);
    return retval;
  }

  static __device__ __forceinline__ void Advance_Edges(
      bool &queue_reset, VertexId &queue_index, LabelT &label, SizeT &nodes,
      SizeT &edges, SizeT *&d_row_offsets, VertexId *&d_column_indices,
      SizeT *&d_edges_in, SizeT *&d_edges_out, DataSlice *&d_data_slice,
      SizeT &input_queue_length, SizeT *output_queue_length,
    ...
    while (x - threadIdx.x < input_queue_length) {
      ...
      if (x < input_queue_length) {
        ...
        if (Functor::CondEdge(src, des, d_data_slice, edge_id, edge_id, label,
                              x, output_pos)) {
          Functor::ApplyEdge(src, des, d_data_slice, edge_id, edge_id, label, x,
                             output_pos);
        }  // else to_process = false;
      }    // else to_process = false;

      /*KernelPolicy::BlockScanT::LogicScan(to_process, output_pos,
      smem.scan_space); if (threadIdx.x == blockDim.x -1)
      {
          smem.block_offset = atomicAdd(output_queue_length, output_pos +
      ((to_process) ? 1 : 0));
      }
      __syncthreads();
...
__launch_bounds__(KernelPolicy::THREADS, KernelPolicy::CTA_OCCUPANCY) __global__
    void Advance_Edges(
        ...
        typename Functor ::LabelT label, typename KernelPolicy::SizeT nodes,
  ...
  Dispatch<KernelPolicy, Problem, Functor, ADVANCE_TYPE, R_TYPE,
           R_OP>::Advance_Edges(queue_reset, queue_index, label, nodes, edges,
                                d_row_offsets, d_column_indices, d_edges_in,
                                d_edges_out, d_data_slice, input_queue_length,
                                output_queue_length, work_progress);
}
