...
struct Cta {
  ...
  // Work progress
  // VertexId                iteration;                  // Current graph
  // traversal iteration
  ...
  struct Tile {
    ...
    struct Iterate {
      ...
      static __device__ __forceinline__ void InitFlags(Tile *tile) {
        ...
        tile->flags[LOAD][VEC] =
            (tile->element_id[LOAD][VEC] == util::InvalidValue<VertexId>()) ? 0
                                                                            : 1;
      ...
      static __device__ __forceinline__ void BitmaskCull(Cta *cta, Tile *tile) {
        if (tile->element_id[LOAD][VEC] >= 0) {
          ...
          unsigned char tex_mask_byte =
              _ldg(cta->d_visited_mask + mask_byte_offset);
      ...
      struct VertexC<DummyT, true> {
        static __device__ __forceinline__ void Cull(Cta *cta, Tile *tile) {
          if (tile->element_id[LOAD][VEC] >=
              0)  // util::isValid(row_id))//tile->element_id[LOAD][VEC]))
          {
            VertexId row_id =
                (tile->element_id[LOAD][VEC] &
                 KernelPolicy::ELEMENT_ID_MASK);  /// cta->num_gpus;
            ...
            if (cta->d_data_slice->labels[row_id] != util::MaxValue<LabelT>()) {
              ...
              if (Problem::MARK_PREDECESSORS) {
                if (Functor::CondFilter(tile->pred_id[LOAD][VEC], row_id,
                                        cta->d_data_slice,
                                        util::InvalidValue<SizeT>(), cta->label,
                                        util::InvalidValue<SizeT>(),
                                        util::InvalidValue<SizeT>())) {
                  Functor::ApplyFilter(
                      tile->pred_id[LOAD][VEC], row_id, cta->d_data_slice,
                      util::InvalidValue<SizeT>(), cta->label,
                      util::InvalidValue<SizeT>(), util::InvalidValue<SizeT>());
                }  // else tile -> element_id[LOAD][VEC] =
                   // util::InvalidValue<VertexId>();
              } else {
                if (Functor::CondFilter(util::InvalidValue<VertexId>(), row_id,
                                        cta->d_data_slice,
                                        util::InvalidValue<SizeT>(), cta->label,
                                        util::InvalidValue<SizeT>(),
                                        util::InvalidValue<SizeT>())) {
                  Functor::ApplyFilter(
                      util::InvalidValue<VertexId>(), row_id, cta->d_data_slice,
                      util::InvalidValue<SizeT>(), cta->label,
                      util::InvalidValue<SizeT>(), util::InvalidValue<SizeT>());
                }  // else tile -> element_id[LOAD][VEC] =
                   // util::InvalidValue<VertexId>();
  ...
  struct BitMask_Cull<LabelT, true> {
    static __device__ __forceinline__ bool Eval(LabelT label) {
      if (KernelPolicy::END_BITMASK_CULL < 0)
        ...
        return (label < KernelPolicy::END_BITMASK_CULL);
    }
  };
