          ...
          FilterOpT       filter_op)
{
    if (parameters.advance_mode == "LB")
        return LB::Launch      <FLAG>(graph, frontier_in, frontier_out,
    ...
    if (parameters.advance_mode == "ALL_EDGES")
        return AE::Launch      <FLAG>(graph, frontier_in, frontier_out,
            parameters, advance_op, filter_op);

    return util::GRError(cudaErrorInvalidValue,
        "AdvanceMode " + parameters.advance_mode + " undefined.", __FILE__, __LINE__);
}
