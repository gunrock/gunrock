...
struct Dispatch<KernelPolicy, ProblemData, Functor, true>
{
                                ...
                                SizeT &max_edges)
    {
        ...
        while (edges_processed < my_work_size && my_start_partition < my_end_partition)
        {
            ...
            SizeT v_index = FindIntersection<KernelPolicy::THREADS>(tid+e_offset, smem_storage.s_edges);
            ...
            for (int i = (tid + e_offset); i < e_last + e_offset; i+=KernelPolicy::THREADS)
            {
                if (i >= end_last)
                {
                    v_index = FindIntersection<KernelPolicy::THREADS>(i, smem_storage.s_edges);
                    v = d_queue[v_index];
