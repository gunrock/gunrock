...
{
   static texture<SizeT, cudaTextureType1D, cudaReadModeElementType>
row_offsets;
};
template <typename SizeT>
texture<SizeT, cudaTextureType1D, cudaReadModeElementType>
RowOffsetsTex<SizeT>::row_offsets;*/
...
struct Dispatch<KernelPolicy, Problem, Functor, ADVANCE_TYPE, R_TYPE, R_OP,
  ...
  static __device__ __forceinline__ void RelaxPartitionedEdges2(
      ...
      DataSlice *&d_data_slice, SizeT &input_queue_len, SizeT *output_queue_len,
    ...
    SizeT block_first_v_skip_count =
        (block_output_start != d_scanned_edges[iter_input_start])
            ? block_output_start - (iter_input_start > 0
                                        ? d_scanned_edges[iter_input_start - 1]
                                        : 0)
            : 0;
    ...
    while (block_output_processed < block_output_size &&
      ...
      SizeT iter_input_size = min((SizeT)KernelPolicy::SCRATCH_ELEMENTS - 1,
      ...
      SizeT iter_output_end = iter_input_end < input_queue_len
                                  ? d_scanned_edges[iter_input_end]
                                  : output_queue_len[0];
      ...
      if (threadIdx.x < KernelPolicy::SCRATCH_ELEMENTS) {
        if (thread_input <= block_input_end && thread_input < input_queue_len) {
          VertexId input_item =
              (d_queue == NULL) ? thread_input : d_queue[thread_input];
      ...
      for (SizeT thread_output_offset = threadIdx.x + block_output_processed;
        ...
        //    edge_id);
        ProcessNeighbor<KernelPolicy, Problem, Functor, ADVANCE_TYPE, R_TYPE,
                        R_OP>(
            v, u, d_data_slice, edge_id, iter_input_start + v_index, input_item,
            block_output_start + thread_output_offset, label, d_keys_out,
            d_values_out, d_value_to_reduce, d_reduce_frontier);
  ...
  static __device__ __forceinline__ void RelaxLightEdges(
    ...
    SizeT block_input_start =
        (SizeT)blockIdx.x * KernelPolicy::SCRATCH_ELEMENTS;
    SizeT block_input_end =
    ...
    if (threadIdx.x < KernelPolicy::SCRATCH_ELEMENTS) {
      if (thread_input <= block_input_end + 1 &&
          thread_input < input_queue_length) {
        ...
        if (ADVANCE_TYPE == gunrock::oprtr::advance::V2V ||
          ...
          if (input_item >= 0)
            smem_storage.row_offset[threadIdx.x] =
                _ldg(row_offsets + input_item);  // row_offsets[input_item];
          else
            smem_storage.row_offset[threadIdx.x] = util::MaxValue<SizeT>();
        } else if (ADVANCE_TYPE == gunrock::oprtr::advance::E2V ||
                   ADVANCE_TYPE == gunrock::oprtr::advance::E2E) {
    ...
    for (SizeT thread_output = threadIdx.x;
      ...
      if (thread_output < block_output_size) {
        ...
        ProcessNeighbor<KernelPolicy, Problem, Functor, ADVANCE_TYPE, R_TYPE,
                        R_OP>(
            v, u, d_data_slice, edge_id, block_input_start + v_index,
            input_item, block_output_start + thread_output, label, d_keys_out,
            d_values_out, d_value_to_reduce, d_reduce_frontier);
      }
