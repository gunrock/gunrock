...
struct Cta {
      ...
      static __device__ __forceinline__ void Inspect(Cta *cta, Tile *tile) {
        if (tile->vertex_id[LOAD][VEC] != -1) {
          ...
          if (ADVANCE_TYPE == gunrock::oprtr::advance::E2V ||
            ...
            util::io::ModifiedLoad<Problem::COLUMN_READ_MODIFIER>::Ld(
                row_range.x, cta->d_row_offsets + row_id1);
            // row_range.y = tex1Dfetch(cta->ts_rowoffset[0], row_id1+1);
            util::io::ModifiedLoad<Problem::COLUMN_READ_MODIFIER>::Ld(
                row_range.y, cta->d_row_offsets + row_id1 + 1);
          }
      ...
      static __device__ __forceinline__ void CtaExpand(Cta *cta, Tile *tile) {
        ...
        while (true) {
            ...
            if (ADVANCE_TYPE == gunrock::oprtr::advance::E2V ||
              ...
              cta->smem_storage.state.warp_comm[0][3] =
                  cta->input_inverse_graph
                      ? cta->d_inverse_column_indices[tile->vertex_id[LOAD]
                                                                     [VEC]]
          ...
          while (coop_offset + threadIdx.x < coop_oob) {
            ...
            ProcessNeighbor<KernelPolicy, Problem, Functor, ADVANCE_TYPE,
                            R_TYPE, R_OP>(
                pred_id, neighbor_id, cta->d_data_slice,
                (SizeT)(coop_offset + threadIdx.x),
                util::InvalidValue<SizeT>(),  // input_pos
                input_item,
                cta->smem_storage.state.coarse_enqueue_offset + coop_rank,
                cta->label, cta->d_keys_out, cta->d_values_out,
                cta->d_value_to_reduce, cta->d_reduce_frontier);
            // ProcessNeighbor(
            //    cta, tile,
  ...
  Cta(bool queue_reset, VertexId queue_index, LabelT label,
      SizeT *d_row_offsets, SizeT *d_inverse_row_offsets,
      VertexId *d_column_indices, VertexId *d_inverse_column_indices,
      VertexId *d_keys_in, VertexId *d_keys_out, Value *d_values_out,
      DataSlice *d_data_slice, SizeT input_queue_length, SizeT max_in_frontier,
      SizeT max_out_frontier, util::CtaWorkProgress<SizeT> &work_progress,
      SmemStorage &smem_storage,
      // gunrock::oprtr::advance::TYPE ADVANCE_TYPE,
      bool input_inverse_graph,
      // gunrock::oprtr::advance::REDUCE_TYPE    R_TYPE,
      // gunrock::oprtr::advance::REDUCE_OP      R_OP,
      Value *d_value_to_reduce, Value *d_reduce_frontier)
      :
  ...
  __device__ __forceinline__ void ProcessTile(
    ...
    util::io::LoadTile<KernelPolicy::LOG_LOADS_PER_TILE,
                       ...
                       false>::LoadValid(tile.vertex_id, d_keys_in, cta_offset,
    ...
    while (tile.progress < tile.fine_count) {
      ...
      for (SizeT scratch_offset = threadIdx.x;
        ...
        ProcessNeighbor<KernelPolicy, Problem, Functor, ADVANCE_TYPE, R_TYPE,
                        R_OP>(predecessor_id, neighbor_id, d_data_slice,
                              edge_id,
                              util::InvalidValue<SizeT>(),  // input_pos
                              input_item,
        ...
        smem_storage.gather_offsets[scratch_offset], edge_id)) {
            Functor::ApplyEdge(predecessor_id, neighbor_id, problem,
        smem_storage.gather_offsets[scratch_offset], edge_id); if (advance_type
        == gunrock::oprtr::advance::V2E || advance_type ==
        gunrock::oprtr::advance::E2E) neighbor_id =
        smem_storage.gather_offsets[scratch_offset];
        ...
        scratch_offset); } else if (r_type == gunrock::oprtr::advance::EDGE) {
                    util::io::ModifiedStore<ProblemData::QUEUE_WRITE_MODIFIER>::St(
                ...
                switch (r_op) {
                    case gunrock::oprtr::advance::PLUS :
                        util::io::ModifiedStore<ProblemData::QUEUE_WRITE_MODIFIER>::St(
                                ...
                                d_reduce_frontier +
        smem_storage.state.fine_enqueue_offset + tile.progress +
        scratch_offset); break; case gunrock::oprtr::advance::MULTIPLIES :
                        util::io::ModifiedStore<ProblemData::QUEUE_WRITE_MODIFIER>::St(
                                (Value)1,
                                d_reduce_frontier +
        smem_storage.state.fine_enqueue_offset + tile.progress +
        scratch_offset); break; case gunrock::oprtr::advance::MAXIMUM :
                        util::io::ModifiedStore<ProblemData::QUEUE_WRITE_MODIFIER>::St(
                                (Value)INT_MIN,
                                d_reduce_frontier +
        smem_storage.state.fine_enqueue_offset + tile.progress +
        scratch_offset); break; case gunrock::oprtr::advance::MINIMUM :
                        util::io::ModifiedStore<ProblemData::QUEUE_WRITE_MODIFIER>::St(
                                (Value)INT_MAX,
                                d_reduce_frontier +
        smem_storage.state.fine_enqueue_offset + tile.progress +
        scratch_offset); break; case gunrock::oprtr::advance::BIT_OR :
                        util::io::ModifiedStore<ProblemData::QUEUE_WRITE_MODIFIER>::St(
                                (Value)0,
                                d_reduce_frontier +
        smem_storage.state.fine_enqueue_offset + tile.progress +
        scratch_offset); break; case gunrock::oprtr::advance::BIT_AND :
                        util::io::ModifiedStore<ProblemData::QUEUE_WRITE_MODIFIER>::St(
                                (Value)0xffffffff,
                                d_reduce_frontier +
        smem_storage.state.fine_enqueue_offset + tile.progress +
        scratch_offset); break; case gunrock::oprtr::advance::BIT_XOR :
                        util::io::ModifiedStore<ProblemData::QUEUE_WRITE_MODIFIER>::St(
                                (Value)0,
                                d_reduce_frontier +
        smem_storage.state.fine_enqueue_offset + tile.progress +
        scratch_offset); break; default:
                        util::io::ModifiedStore<ProblemData::QUEUE_WRITE_MODIFIER>::St(
                                (Value)0,
                                d_reduce_frontier +
        smem_storage.state.fine_enqueue_offset + tile.progress +
        scratch_offset); break;
                }
            }
        }
        ...
        if (d_out != NULL) {
             //util::io::ModifiedStore<ProblemData::QUEUE_WRITE_MODIFIER>::St(
             //       neighbor_id,
             //       d_out + smem_storage.state.fine_enqueue_offset +
        tile.progress + scratch_offset); util::Store_d_out<VertexId, SizeT,
        ProblemData>( neighbor_id, d_out, 9,
        ...
        d_value_out != NULL) {
            util::io::ModifiedStore<ProblemData::QUEUE_WRITE_MODIFIER>::St(
                    predecessor_id,
                    d_value_out + smem_storage.state.fine_enqueue_offset +
        tile.progress + scratch_offset);
        }*/
      }
