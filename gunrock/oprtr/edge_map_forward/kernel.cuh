...
struct Sweep {
  static __device__ __forceinline__ void Invoke(
    ...
    typedef Cta<KernelPolicy, Problem, Functor, ADVANCE_TYPE, R_TYPE, R_OP> Cta;
    ...
    Cta cta(queue_reset, queue_index, label, d_row_offsets,
            d_inverse_row_offsets, d_column_indices, d_inverse_column_indices,
            d_keys_in, d_keys_out, d_values_out, d_data_slice,
            input_queue_length, max_in_frontier, max_out_frontier,
...
struct Dispatch {
  ...
  static __device__ __forceinline__ void Kernel(
      ...
      SizeT &input_queue_length, DataSlice *&d_data_slice,
...
struct Dispatch<KernelPolicy, Problem, Functor, ADVANCE_TYPE, R_TYPE, R_OP,
  ...
  static __device__ __forceinline__ void Kernel(
      ...
      VertexId *&d_column_indices, VertexId *&d_inverse_column_indices,
      VertexId *&d_keys_in, VertexId *&d_keys_out, Value *&d_values_out,
      DataSlice *&d_data_slice, SizeT &input_queue_length,
      SizeT &max_in_frontier, SizeT &max_out_frontier,
      util::CtaWorkProgress<SizeT> &work_progress,
      util::KernelRuntimeStats &kernel_stats, bool &input_inverse_graph,
      Value *&d_value_to_reduce, Value *&d_reduce_frontier) {
    ...
    if (threadIdx.x == 0) {
      ...
      smem_storage.state.work_decomposition
          .template Init<KernelPolicy::LOG_SCHEDULE_GRANULARITY>(
    ...
    Sweep<KernelPolicy, Problem, Functor, ADVANCE_TYPE, R_TYPE, R_OP>::Invoke(
        queue_reset, queue_index, label, d_row_offsets, d_inverse_row_offsets,
        d_column_indices, d_inverse_column_indices, d_keys_in, d_keys_out,
        d_values_out, d_data_slice, input_queue_length, max_in_frontier,
        max_out_frontier, work_progress, smem_storage,
        smem_storage.state.work_decomposition,
        // ADVANCE_TYPE,
        input_inverse_graph,
        // R_TYPE,
 ...
 */
template <
    typename KernelPolicy, typename ProblemData, typename Functor,
    gunrock::oprtr::advance::TYPE ADVANCE_TYPE = gunrock::oprtr::advance::V2V,
    gunrock::oprtr::advance::REDUCE_TYPE R_TYPE =
        gunrock::oprtr::advance::EMPTY,
    gunrock::oprtr::advance::REDUCE_OP R_OP = gunrock::oprtr::advance::NONE>
__launch_bounds__(KernelPolicy::THREADS, KernelPolicy::CTA_OCCUPANCY) __global__
  ...
  Dispatch<KernelPolicy, ProblemData, Functor, ADVANCE_TYPE, R_TYPE,
           R_OP>::Kernel(queue_reset, queue_index, label, d_row_offsets,
                         d_inverse_row_offsets, d_column_indices,
                         d_inverse_column_indices, d_keys_in, d_keys_out,
                         d_values_out, d_data_slice, input_queue_length,
                         max_in_frontier, max_out_frontier, work_progress,
                         kernel_stats,
                         // ADVANCE_TYPE,
                         input_inverse_graph,
                         // R_TYPE,
