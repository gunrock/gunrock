...
struct KernelParameter {
  ...
  gunrock::app::EnactorStats<SizeT> &enactor_stats;
  gunrock::app::FrontierAttribute<SizeT> &frontier_attribute;
  LabelT &label;
  ...
  KernelParameter(gunrock::app::EnactorStats<SizeT> &enactor_stats,
                  gunrock::app::FrontierAttribute<SizeT> &frontier_attribute,
                  LabelT &label, DataSlice *&h_data_slice,
                  DataSlice *&d_data_slice, SizeT *&d_vertex_markers,
                  unsigned char *&d_visited_mask, VertexId *&d_in_key_queue,
                  VertexId *&d_out_key_queue, Value *&d_in_value_queue,
                  Value *&d_out_value_queue, SizeT &num_elements,
                  SizeT &num_nodes,
                  util::CtaWorkProgressLifetime<SizeT> &work_progress,
                  CudaContext &context, cudaStream_t &stream,
                  SizeT &max_in_queue, SizeT &max_out_queue,
                  util::KernelRuntimeStatsLifetime &kernel_stats,
                  bool &filtering_flag, bool &skip_marking)
...
template <typename Parameter, MODE FILTER_MODE>
struct Dispatch {
    ...
    gunrock::oprtr::cull_filter::Kernel<
        ...
        <<<parameter.enactor_stats.filter_grid_size, KernelPolicy::THREADS,
           (size_t)0, parameter.stream>>>(
            (VertexId)parameter.label, parameter.frontier_attribute.queue_reset,
...
struct Dispatch<Parameter, SIMPLIFIED2> {
  ...
  static cudaError_t Launch(Parameter &parameter) {
    ...
    if (parameter.h_data_slice->visit_lookup[0].GetPointer(util::DEVICE) ==
        NULL) {
      if (retval = parameter.h_data_slice->visit_lookup[0].Allocate(
              parameter.num_nodes + 1, util::DEVICE))
        return retval;
    } else {
      if (retval = parameter.h_data_slice->visit_lookup[0].EnsureSize(
              parameter.num_nodes + 1))
        return retval;
    }

    if (parameter.h_data_slice->valid_in[0].GetPointer(util::DEVICE) == NULL) {
      if (retval = parameter.h_data_slice->valid_in[0].Allocate(
              parameter.num_elements + 1, util::DEVICE))
        return retval;
    } else {
      if (retval = parameter.h_data_slice->valid_in[0].EnsureSize(
              parameter.num_elements + 1))
        return retval;
    }

    if (parameter.h_data_slice->valid_out[0].GetPointer(util::DEVICE) == NULL) {
      if (retval = parameter.h_data_slice->valid_out[0].Allocate(
              parameter.num_elements + 1, util::DEVICE))
        return retval;
    } else {
      if (retval = parameter.h_data_slice->valid_out[0].EnsureSize(
              parameter.num_elements + 1))
        return retval;
    }
...
struct Dispatch<Parameter, BY_PASS> {
  ...
  static cudaError_t Launch(Parameter &parameter) {
    ...
    gunrock::oprtr::bypass_filter::Kernel<KernelPolicy,
                                          typename Parameter::Problem,
                                          typename Parameter::Functor>
        <<<num_blocks,  // parameter. enactor_stats.filter_grid_size,
           KernelPolicy::THREADS, (size_t)0, parameter.stream>>>(
            parameter.label, parameter.frontier_attribute.queue_reset,
            (VertexId)parameter.frontier_attribute.queue_index,
            parameter.num_elements, parameter.d_in_key_queue,
            // parameter. d_in_value_queue,
            parameter.d_out_key_queue, parameter.d_data_slice,
...
cudaError_t LaunchKernel(
    ...
    typename Problem::SizeT *d_vertex_markers, unsigned char *d_visited_mask,
    ...
    typename Problem::SizeT num_elements, typename Problem::SizeT num_nodes,
    util::CtaWorkProgressLifetime<typename Problem::SizeT> &work_progress,
    CudaContext &context, cudaStream_t &stream,
    typename Problem::SizeT max_in_queue, typename Problem::SizeT max_out_queue,
    util::KernelRuntimeStatsLifetime &kernel_stats, bool filtering_flag = true,
  ...
  Parameter parameter(enactor_stats, frontier_attribute, label, h_data_slice,
                      d_data_slice, d_vertex_markers, d_visited_mask,
                      d_in_key_queue, d_out_key_queue, d_in_value_queue,
                      d_out_value_queue, num_elements, num_nodes, work_progress,
                      context, stream, max_in_queue, max_out_queue,
                      kernel_stats, filtering_flag, skip_marking);

  if (filtering_flag) {
    if (retval =
            Dispatch<Parameter, KernelPolicy::FILTER_MODE>::Launch(parameter))
      return retval;
  } else {
    if (retval = Dispatch<Parameter, BY_PASS>::Launch(parameter)) return retval;
  }
