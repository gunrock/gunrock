        ...
        FilterOpT filter_op)
    {
        ...
        while (pos - threadIdx.x < num_inputs)
        {
            ...
            if (keys_out != NULL && pos < num_inputs)
                keys_out[pos] = (to_process) ? key
                    : util::PreDefinedValues<OutKeyT>::InvalidValue;
...
template <
    ...
    typename  FilterOpT>
__launch_bounds__ (
    Dispatch<FLAG, InKeyT, OutKeyT, SizeT, true>
          ...
          FilterOpT filter_op)
{
    ...
    Dispatch<FLAG, InKeyT, OutKeyT, SizeT>
        ::Kernel(
        keys_in, num_inputs,
        keys_out, work_progress.GetQueueCounter(queue_index + 1),
        filter_op);
}
          ...
          FilterOpT        filter_op)
{
    ...
    typedef typename Dispatch<FLAG, InKeyT, OutKeyT,
        SizeT, true>::KernelPolicyT KernelPolicyT;
    ...
    if (parameters.frontier -> queue_reset)
    {
        ...
        num_blocks =  (parameters.frontier -> queue_length
            + KernelPolicyT::THREADS - 1) / KernelPolicyT::THREADS;
        if (num_blocks > KernelPolicyT::BLOCKS)
            num_blocks = KernelPolicyT::BLOCKS;
    }

    Kernel
        <FLAG, InKeyT, OutKeyT, SizeT>
