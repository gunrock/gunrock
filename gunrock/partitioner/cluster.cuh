...
struct CsrSwitch<GraphT, true>
{
        ...
        float      *weitage = NULL)
    {
        ...
        else partition_factor = parameters.Get<double>("partition-factor");
        ...
        for (SizeT pos = 0; pos <= nodes; pos++)
        {
            ...
            for (SizeT i = 0; i <= num_subgraphs; i++)
                counter[i] = 0;
            ...
            if (max_GPU != -1 && (1.0 * max_count / total_count) > partition_factor
                && current_count[max_GPU] + counter[num_subgraphs] <= nodes * weitage[max_GPU])
                set_GPU = max_GPU;
            else {
                float max_empty = 0;
                for (int i=0; i<num_subgraphs; i++)
    ...
    float      *weitage = NULL)
{
    return CsrSwitch<GraphT, (GraphT::FLAG & gunrock::graph::HAS_CSR) != 0>
        ::Partition(org_graph, sub_graphs, parameters,
