    ...
    curandState   *d_rand_states)
{
    ...
    while (i < edge_count)
    {
        ...
        *col = v - 1;
        if (WITH_VALUES)
            *val = Sprng(rand_state) * vmultipiler + vmin;
...
/**
 * @brief Builds a R-MAT graph in GPU passes the pointers back to calling function.
 *
 * @tparam WITH_VALUES Whether or not associate with per edge weight values.
 * @tparam VertexId Vertex identifier.
 * @tparam Value Value type.
 ...
 * @param[in] coo
 */
template <bool WITH_VALUES, typename VertexId, typename SizeT, typename Value, typename EdgeTupleType>
