...
__global__ void Rmat_Kernel(SizeT num_nodes, SizeT edge_count,
                            Coo<VertexId, Value> *d_edges, bool undirected,
                            Value vmultipiler, Value vmin, double a0, double b0,
...
template <bool WITH_VALUES, typename VertexId, typename SizeT, typename Value>
__global__ void Rmat_Kernel_nv(SizeT num_nodes, SizeT edge_count,
                               VertexId *rows, VertexId *cols, Value *vals,
                               bool undirected, Value vmultipiler, Value vmin,
 ...
 */
template <bool WITH_VALUES, typename VertexId, typename SizeT, typename Value>
cudaError_t BuildRmatGraph(SizeT num_nodes, SizeT num_edges,
                           Csr<VertexId, SizeT, Value> &graph, bool undirected,
                           double a0 = 0.55, double b0 = 0.2, double c0 = 0.2,
                           double d0 = 0.05, double vmultipiler = 1.00,
                           double vmin = 1.00, int seed = -1,
                           bool quiet = false, int num_gpus = 1,
                           int *gpu_idx = NULL) {
  ...
  for (int gpu = 0; gpu < num_gpus; gpu++) {
    ...
    if (retval = edges[gpu].SetPointer(coo + start_edge * (undirected ? 2 : 1),
    ...
    Rmat_Kernel<WITH_VALUES, VertexId, SizeT, Value>
        <<<grid_size, block_size, 0, streams[gpu]>>>(
            ...
            edges[gpu].GetPointer(util::DEVICE), undirected, vmultipiler, vmin,
            a0, b0, c0, d0, rand_states[gpu].GetPointer(util::DEVICE));
  ...
  graph.template FromCoo<WITH_VALUES, EdgeTupleType>(out_file, coo, num_nodes,
...
/**
 * @brief Builds a meta R-MAT CSR graph that connects n R-MAT graphs by a single
 * root node.
 *
 * @tparam WITH_VALUES Whether or not associate with per edge weight values.
 * @tparam VertexId Vertex identifier.
...
cudaError_t BuildMetaRmatGraph(
    ...
    int seed = -1, bool quiet = false, int num_gpus = 1, int *gpu_idx = NULL) {
  ...
  for (int gpu = 0; gpu < num_gpus; gpu++) {
    ...
    for (int copy_idx = 0; copy_idx < num_gpus; ++copy_idx) {
      ...
      util::MemsetAddEdgeValKernel<<<256, 1024>>>(edges_pointer, offset,
                                                  edge_count);
      if (retval = edges[gpu].SetPointer(coo + ((num_gpus > 1) ? num_gpus : 0) +
...
cudaError_t BuildRmatGraph_coo_nv(SizeT num_nodes, SizeT num_edges,
                                  EdgeTupleType &coo, bool undirected,
                                  double a0 = 0.55, double b0 = 0.2,
                                  double c0 = 0.2, double d0 = 0.05,
                                  double vmultipiler = 1.00, double vmin = 1.00,
