    ...
    public GraphBase<VertexT, SizeT, ValueT, _FLAG | HAS_GP, cudaHostRegisterFlag>
{
    ...
    util::Array1D<SizeT, int    , ARRAY_FLAG, cudaHostRegisterFlag>
        partition_table    ; // Partition tables indicating which GPU the vertices are hosted
    util::Array1D<SizeT, VertexT, ARRAY_FLAG, cudaHostRegisterFlag>
        convertion_table   ; // Conversions tables indicating vertex IDs on local / remote GPUs
    util::Array1D<SizeT, VertexT, ARRAY_FLAG, cudaHostRegisterFlag>
        ...
        util::Location target   = GRAPH_DEFAULT_TARGET)
    {
        ...
        if (flag & partitioner::Enable_Backward)
        {
            ...
            if (retval) return retval;
            if ((flag & partitioner::Keep_Node_Num) == 0)
                retval = backward_convertion.Allocate(in_counter[num_subgraphs], target);
            if (retval) return retval;
        }
        return retval;
    }
