...
__device__ void insertWarpEdges(PairT& thread_edge, ValueT& thread_value,
  ...
  while (work_queue = __ballot_sync(0xFFFFFFFF, to_insert)) {
    ...
    bool success = hashContexts[cur_src].insertPairUnique(
        same_src, laneId, thread_edge.y, thread_value, dst_bucket,
        local_allocator_ctx);
...
template <typename PairT, typename ValueT, typename SizeT, typename ContextT>
__global__ void InsertEdges(PairT* d_edges, ValueT* d_values,
                            ContextT* hashContexts, SizeT num_edges,
                            SizeT* d_edges_per_vertex,
  ...
  insertWarpEdges(thread_edge, thread_value, hashContexts, d_edges_per_vertex,
  ...
  if (make_batch_undirected) {
    ...
    insertWarpEdges(reverse_edge, thread_value, hashContexts,
                    d_edges_per_vertex, d_edges_per_bucket, d_buckets_offset,
                    laneId, to_insert, local_allocator_ctx);
  }
}
}  // namespace slabhash_map_kernels
}  // namespace graph
