...
{   
    ...
    */
    SlabHashGraphBase(SizeT max_nodes = 1 << 20, SizeT max_buckets = 1 << 25){
        ...
        CHECK_ERROR(cudaMemset(d_edges_per_node, 0, sizeof(SizeT)* nodes_capacity));
        CHECK_ERROR(cudaMemset(d_edges_per_bucket, 0, sizeof(SizeT)* buckets_capacity));
        CHECK_ERROR(cudaMemset(d_buckets_offset, 0, sizeof(SizeT)* buckets_capacity));


		size_t slab_unit_size = GpuSlabHashContext<VertexT, ValueT, SlabHashTypeT::ConcurrentMap>::getSlabUnitSize();
      	CHECK_ERROR(cudaMalloc((void**)&d_base_slabs, slab_unit_size * buckets_capacity));
        CHECK_ERROR(cudaMemset(d_base_slabs, 0xFF, slab_unit_size * buckets_capacity));
    ...
    */
    ...
    cudaError_t Init(SizeT* h_row_offsets, SizeT num_nodes_, SizeT num_edges_, SizeT edges_per_slab, float load_factor,
        ...
        for(SizeT i = 0; i < num_nodes; i++){
            ...
            buckets_per_table[i] =  ceil(node_edges / (load_factor * float(edges_per_slab)));
        ...
        assert(total_num_edges == num_edges_ && "Edges in the graph mismatch the expected count");
        
      	CHECK_ERROR(cudaMalloc((void**)&d_edges_pairs, sizeof(PairT) * num_edges_));
        CHECK_ERROR(cudaMemcpy(d_edges_pairs, h_edges_pairs.data(), sizeof(PairT) * num_edges_, cudaMemcpyHostToDevice));

      	CHECK_ERROR(cudaMemcpy(d_hash_context, h_hash_context, sizeof(HashContextT) * num_nodes, cudaMemcpyHostToDevice));
        return cudaSuccess;
    }
