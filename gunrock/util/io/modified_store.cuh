...
struct ModifiedStore
{
    ...
    #define GR_STORE(base_type, ptx_type, reg_mod, cast_type, modifier)                                                                 \
        template<> template<> void ModifiedStore<st::modifier>::St(base_type val, base_type* ptr) {                                         \
            asm volatile ("st.global."#modifier"."#ptx_type" [%0], %1;" : : _GR_ASM_PTR_(ptr), #reg_mod(reinterpret_cast<cast_type&>(val)));            \
        }

    /**
     * Vector store ops
     */
    #define GR_STORE_VEC1(component_type, base_type, ptx_type, reg_mod, cast_type, modifier)                                                                    \
        template<> template<> void ModifiedStore<st::modifier>::St(base_type val, base_type* ptr) {                                         \
            component_type c = val.x;                                                                                                           \
            asm volatile ("st.global."#modifier"."#ptx_type" [%0], %1;" : : _GR_ASM_PTR_(ptr), #reg_mod(reinterpret_cast<cast_type&>(c)));          \
        }

    #define GR_STORE_VEC2(component_type, base_type, ptx_type, reg_mod, cast_type, modifier)                                                                    \
        template<> template<> void ModifiedStore<st::modifier>::St(base_type val, base_type* ptr) {                                         \
            component_type cx = val.x;                                                                                                          \
            component_type cy = val.y;                                                                                                          \
            asm volatile ("st.global."#modifier".v2."#ptx_type" [%0], {%1, %2};" : : _GR_ASM_PTR_(ptr), #reg_mod(reinterpret_cast<cast_type&>(cx)), #reg_mod(reinterpret_cast<cast_type&>(cy)));        \
        }

    #define GR_STORE_VEC4(component_type, base_type, ptx_type, reg_mod, cast_type, modifier)                                                                    \
        template<> template<> void ModifiedStore<st::modifier>::St(base_type val, base_type* ptr) {                                         \
            component_type cx = val.x;                                                                                                          \
            component_type cy = val.y;                                                                                                          \
            component_type cz = val.z;                                                                                                          \
            component_type cw = val.w;                                                                                                          \
            asm volatile ("st.global."#modifier".v4."#ptx_type" [%0], {%1, %2, %3, %4};" : : _GR_ASM_PTR_(ptr), #reg_mod(reinterpret_cast<cast_type&>(cx)), #reg_mod(reinterpret_cast<cast_type&>(cy)), #reg_mod(reinterpret_cast<cast_type&>(cz)), #reg_mod(reinterpret_cast<cast_type&>(cw)));        \
        }
     ...
     */
    #define GR_STORE_BASE_ONE_TWO(base_type, dest_type, short_type, ptx_type, reg_mod, cast_type)       \
     ...
     */
    #define GR_STORE_BASE_ONE_TWO_FOUR(base_type, dest_type, short_type, ptx_type, reg_mod, cast_type)  \
        GR_STORE_BASE_ONE_TWO(base_type, dest_type, short_type, ptx_type, reg_mod, cast_type)               \
                                                                                                            \
    ...
    GR_STORE_BASE_ONE_TWO_FOUR(unsigned char,       unsigned char,  uchar,  u8,     GR_REG8,        unsigned GR_CAST8)
    GR_STORE_BASE_ONE_TWO_FOUR(unsigned short,      unsigned short, ushort, u16,    GR_REG16,       unsigned short)
    GR_STORE_BASE_ONE_TWO_FOUR(unsigned int,        unsigned int,   uint,   u32,    r,              unsigned int)
    ...
    GR_STORE_BASE_ONE_TWO_FOUR(unsigned long,       unsigned long,  ulong,  u32,    r, unsigned long)
    #endif
     ...
     */
    GR_STORE_BASE_ONE_TWO(unsigned long long,   unsigned long long,     ulonglong,  u64, l, unsigned long long)
    GR_STORE_BASE_ONE_TWO(long long,                long long,              longlong,   s64, l, long long)
    GR_STORE_BASE_ONE_TWO(double,               double,                 double,     s64, l, long long)              // Cast to 64-bit long long a workaround for the fact that the 3.x assembler has no register constraint for doubles
