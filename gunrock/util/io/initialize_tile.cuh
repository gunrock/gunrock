 ...
 * initialize_tile.cuh
 *
 * @brief Kernel utilities for initializing 2D arrays (tiles)
 */
 ...
 */
template <int LOG_LOADS_PER_TILE, int LOG_LOAD_VEC_SIZE, int ACTIVE_THREADS>
struct InitializeTile {
  ...
  struct Iterate {
    ...
    static __device__ __forceinline__ void Copy(T target[][LOAD_VEC_SIZE],
    ...
    template <typename T, typename S>
    static __device__ __forceinline__ void Init(T target[][LOAD_VEC_SIZE],
                                                S datum) {
    ...
    static __device__ __forceinline__ void Transform(T target[][LOAD_VEC_SIZE],
                                                     S source[][LOAD_VEC_SIZE],
    ...
    static __device__ __forceinline__ void Transform(
        T target[][LOAD_VEC_SIZE], S source[][LOAD_VEC_SIZE],
        TransformOp transform_op, const SizeT &guarded_elements,
        T oob_default) {
    ...
    static __device__ __forceinline__ void Transform(SoaT target_soa,
                                                     SoaS source_soa,
                                                     TransformOp transform_op) {
      SoaS source;
  ...
  static __device__ __forceinline__ void Transform(
      T target[][LOAD_VEC_SIZE], S source[][LOAD_VEC_SIZE],
      TransformOp transform_op, const SizeT &guarded_elements, T oob_default) {
    if (guarded_elements >= TILE_SIZE) {
      ...
      // guarded
      Iterate<0, 0>::Transform(target, source, transform_op, guarded_elements,
                               oob_default);
    }
  }
