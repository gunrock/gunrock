 ...
 /*	if(c!=0 && d!=0)
	{
	    ...
	    SizeT edge2=d/2;
	    if(c%2==1){
		if(froms_out[x/edges*edges+edge1]!=froms[b]) {flag[x/edges]=0; return;}}
	    else{ 
		if(tos_out[x/edges*edges+edge1-1]!=froms[b]) {flag[x/edges]=0; return;}}
	    if(d%2==1){
		if(froms_out[x/edges*edges+edge2]!=tos[b])   {flag[x/edges]=0; return;}}
	    else{
		if(tos_out[x/edges*edges+edge2-1]!=tos[b])   {flag[x/edges]=0; return;}}
	}*/
	{if(c!=0)  
 	{
	  ...
	  SizeT*		counts)
{
    ...
    if(x>=0 && x<size*edges)
    {
	if(flag[x/edges]>=1 && (x==0 || flag[x/edges]>flag[x/edges-1]))
	{
