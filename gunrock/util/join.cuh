...
__global__ void MaskOut(const SizeT iter, const SizeT nodes_query,
...
template <typename VertexId, typename SizeT>
__global__ void WriteToPartial(const SizeT* const src_node,
                               const SizeT* const dest_node,
                               const SizeT* const flag,  // index when iter>1
                               const SizeT* const count, const SizeT const iter,
  ...
  if (iter == 0) {
    ...
    while (x < bound * nodes_query) {
      VertexId temp =
          d_partial[flag[x / nodes_query] * nodes_query + x % nodes_query];
