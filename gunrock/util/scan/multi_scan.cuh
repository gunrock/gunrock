...
namespace scan {
   ...
   template <
    ...
    template <
        ...
        typename _SizeT,
        int      Block_N>
    __global__ void Step0d(
        const _SizeT           N,
        const _SizeT           M,
        const _SizeT           N_Next,
        const _VertexId* const Keys,
        const _SizeT*    const Offsets,
        const int*       const Splict,
              _SizeT*          Buffer,
              _SizeT*          Sum)
    {
	...
	_VertexId K0=0,K1=1;
        ...
        for (int y=0;y<M;y++)
        {
            s_Buffer[threadIdx.x] = 0; mark0 = false;
            s_Buffer[threadIdx.x + blockDim.x] = 0; mark1 = false;
            if (x < N)
            {
                for (_SizeT i=Offsets[K0];i<Offsets[K0+1];i++)
                if (y == Splict[i]) {s_Buffer[threadIdx.x]=1; mark0=true; break;}
            }
            if (x + blockDim.x < N)
            {
                for (_SizeT i=Offsets[K1];i<Offsets[K1+1];i++)
                if (y == Splict[i]) {s_Buffer[threadIdx.x + blockDim.x] =1; mark1=true; break;}
            }
            __syncthreads();
    ...
    template <
        ...
        typename _Value,
        bool EXCLUSIVE,
        _SizeT   num_vertex_associate,
        _SizeT   num_value__associate>
    __global__ void Step3c(
        const _SizeT            N,
        const _SizeT            N_Next,
        //const _SizeT            num_vertex_associate,
...
struct MultiScan
{
              ...
              Value**          d_Value__Associate_out)    // The scan result
    {
        ...
        if ((History_Size[1]%32)!=0) Grid_Size.x++;
        Step3c <VertexId,SizeT,Value,EXCLUSIVE> 
            <<<Grid_Size,Block_Size>>> (
            Num_Elements,
            History_Size[1],
