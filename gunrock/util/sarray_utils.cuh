...
public:
        ...
        unsigned int target = HOST)
    {
        ...
        {
            MemsetKernel<<<128, 128>>>(
                temp_Data.GetPointer(DEVICE), 
    ...
    //virtual __host__ __device__ __forceinline__ SizeT Get_Pos(IndexT idx) const;
    /*{
    ...
    /*virtual cudaError_t Build_(unsigned int source, unsigned int processor, unsigned int target)
    {
    ...
    cudaError_t Build(unsigned int source, unsigned int processor, unsigned int target,
        DataT *data_in, SizeT index_range = -1)
    {
        cudaError_t retval = cudaSuccess;
        ...
        if (retval = temp_Data.Release()) return retval;
        if (retval = temp_Data.SetPointer(data_in, index_range, source)) return retval;
...
template <
    ...
    typename _SizeT,
    typename _DataT>
class SparseArray1D_FLAT : public SparseArray1D_Base<_IndexT, _SizeT, _DataT>//, FLAT>
{
public:
    typedef _IndexT IndexT;
    typedef _SizeT  SizeT;
    ...
    cudaError_t Build_(unsigned int source, unsigned int processor, unsigned int target)
    {
        cudaError_t retval = cudaSuccess;
        ...
        if (retval = this -> Data.Allocate(this -> index_range, target | processor)) return retval;
        if (processor == HOST)
        {
            memcpy(
                this -> Data     .GetPointer(HOST), 
                this -> temp_Data.GetPointer(HOST), 
        ...
        {
            MemsetCopyVectorKernel<<<256, 256>>>(
    ...
    cudaError_t Build(std::string name, unsigned int source, unsigned int processor, unsigned int target,
        DataT *data_in, SizeT index_range = -1)
    {
        this -> SetName(name);
        cudaError_t retval = cudaSuccess;
        ...
        if (retval = this -> temp_Data.Release()) return retval;
        if (retval = this -> temp_Data.SetPointer(data_in, index_range, source)) return retval;
        return Build_(source, processor, target);
    }
...
template <
    ...
    typename SizeT,
    typename DataT>
__global__ void Mark_Valid_Data_Kernel(
    DataT *d_data,
    SizeT *d_marker,
    SizeT  index_range)
{
...
template <
    ...
    typename SizeT,
    typename DataT>
__global__ void Assign_Data_Index_Kernel(
    DataT *d_data_in,
    SizeT *d_marker,
    SizeT  index_range,
    IndexT *d_index_out,
...
template <
    ...
    typename _SizeT,
    typename _DataT>
class SparseArray1D_ORDERED : public SparseArray1D_Base<_IndexT, _SizeT, _DataT>
{
public:
    typedef _IndexT IndexT;
    typedef _SizeT  SizeT;
    ...
    {
        //return BinarySearch(idx, this -> Index.GetPointer(), this -> Index.GetSize());
    ...
    cudaError_t Build_(unsigned int source, unsigned int processor, unsigned int target)
    {
        ...
        if (processor == HOST)
        {
            ...
            for (SizeT i=0; i< this -> index_range; i++)
                if (isValid( this -> temp_Data[i])) 
                    this -> num_occuplied_items++;

            if (retval = this -> Data .Allocate(this -> num_occuplied_items, target | processor)) return retval;
            if (retval = this -> Index.Allocate(this -> num_occuplied_items, target | processor)) return retval;
            SizeT counter = 0;
            for (SizeT i=0; i<this -> index_range; i++)
            {
        ...
        } else if (processor == DEVICE)
        {
            ...
            if (retval = markers.Allocate(this -> index_range + 1, DEVICE)) return retval;
            ...
            cub::DeviceScan::ExclusiveSum(
                cub_temp_storage.GetPointer(DEVICE), 
                request_bytes,
                markers         .GetPointer(DEVICE),
                markers         .GetPointer(DEVICE),
                this -> index_range + 1);

            if (retval = cub_temp_storage.Allocate(request_bytes, DEVICE)) return retval;
            cub::DeviceScan::ExclusiveSum(
                cub_temp_storage.GetPointer(DEVICE), 
                request_bytes,
                markers         .GetPointer(DEVICE),
            ...
            if (retval = this -> Data .Allocate(this -> num_occuplied_items, DEVICE)) return retval;
            if (retval = this -> Index.Allocate(this -> num_occuplied_items, DEVICE)) return retval;
            Assign_Data_Index_Kernel <IndexT, SizeT, DataT>
                <<<num_blocks, 1024>>> (
                this -> temp_Data.GetPointer(DEVICE),
                markers          .GetPointer(DEVICE),
                this -> index_range,
                this -> Index    .GetPointer(DEVICE),
                this -> Data     .GetPointer(DEVICE));
...
__host__ __device__ __forceinline__ SizeT Hash_Function(int fun_idx, IndexT idx, SizeT table_size)
{   
    ...
    } else pos = ((idx+1) / (table_size >> (fun_idx >>1))) % table_size;
    if (pos < 0) pos = 0 - pos;
    return pos;
}

template <typename IndexT, typename SizeT, typename DataT>
__global__ void Build_Cuckoo_Kernel(
    SizeT   index_range,
    SizeT   table_size,
    DataT  *d_temp_data,
    IndexT *d_index,
    bool   *d_has_conflict)
{
    ...
    while (i < index_range && !d_has_conflict[0])
    {
        ...
        while (pos1 != InvalidValue<SizeT>())
        {
            ...
            //    idx, step, new_index, pos1, old_index & Step_Mask, ((unsigned int)old_index >> Index_Bits), old_index);
...
template <
    ...
    typename SizeT,
    typename DataT>
__global__ void Assign_Cuckoo_Data_Kernel(
    SizeT table_size,
    DataT *d_temp_data,
    IndexT *d_index,
    DataT  *d_data)
...
template <
    ...
    typename _SizeT,
    typename _DataT>
class SparseArray1D_CUCKOO : public SparseArray1D_Base<_IndexT, _SizeT, _DataT>//, CUCKOO_HASH>
{
public:
    typedef _IndexT IndexT;
    typedef _SizeT  SizeT;
        ...
        } else if (processor == DEVICE)
        {
            ...
            while (has_conflict[0])
            {
                ...
                Build_Cuckoo_Kernel<<<num_blocks, 1024>>>(
                    ...
                    this -> temp_Data.GetPointer(DEVICE),
                    this -> Index.GetPointer(DEVICE),
                    has_conflict.GetPointer(DEVICE));
