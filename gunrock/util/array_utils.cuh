...
struct Array1D {
 ...
 public:
  ...
  /*Array1D(SizeT size, std::string name = "", unsigned int target = HOST, bool
  use_cuda_alloc = false, unsigned int flag = cudaHostAllocDefault)
  {
      this->name= name;
      file_name = "";
      h_pointer = NULL;
  ...
  cudaError_t
      Init(SizeT size, unsigned int target = HOST, bool use_cuda_alloc = false,
  ...
  cudaError_t Allocate(SizeT size, unsigned int target = HOST) {
        ...
        (use_cuda_alloc ))
    {
        ...
        if (retval = GRError(cudaHostAlloc((void **)&h_pointer, sizeof(Value) *
    size, flag), name + "cudaHostAlloc failed", __FILE__, __LINE__)) return
    retval;
        //if (retval = GRError(cudaHostGetDevicePointer((void **)&d_pointer,
    (void *)h_pointer,0),
                     //name + "cudaHostGetDevicePointer failed", __FILE__,
    __LINE__)) return retval; allocated = allocated | HOST  ;
        //allocated = allocated | DEVICE;
        if (ARRAY_DEBUG) {printf("%s allocated on HOST, size = %d, flag =
    %d\n",name.c_str(),size, flag);fflush(stdout);} } else {*/
    if ((target & HOST) == HOST) {
      if (retval = Release(HOST)) return retval;
      UnSetPointer(HOST);
      if ((setted & (~(target | DISK)) == NONE) &&
      ...
      if (ARRAY_DEBUG) {
        printf(
            "%s\t allocated on HOST, length =\t %lld, "
            "size =\t %lld bytes, pointer =\t %p\n",
            name.c_str(), (long long)size, (long long)size * sizeof(Value),
            h_pointer);
        fflush(stdout);
      }
    }
    ...
    if ((target & DEVICE) == DEVICE) {
      ...
      if (size != 0) {
        retval = GRError(cudaMalloc((void**)&(d_pointer), sizeof(Value) * size),
      ...
      if (ARRAY_DEBUG) {
        printf(
            "%s\t allocated on DEVICE, length =\t %lld, "
            "size =\t %lld bytes, pointer =\t %p\n",
            name.c_str(), (long long)size, (long long)size * sizeof(Value),
            d_pointer);
        fflush(stdout);
  ...
  cudaError_t Release(unsigned int target = TARGETALL) {
    ...
    {
        if (retval = GRError(cudaFreeHost(h_pointer),name+" cudaFreeHost
    failed",__FILE__, __LINE__)) return retval; h_pointer = NULL;
        //d_pointer = NULL;
        allocated = allocated - HOST   + TARGETBASE;
        //allocated = allocated - DEVICE + TARGETBASE;
        if (ARRAY_DEBUG) {printf("%s released on HOST & DEVICE\n",
    name.c_str());fflush(stdout);} } else {*/
    if (((target & HOST) == HOST) && ((allocated & HOST) == HOST)) {
      if (use_cuda_alloc) {
        retval =
            GRError(cudaHostUnregister((void*)h_pointer),
    ...
    if (((target & DEVICE) == DEVICE) && ((allocated & DEVICE) == DEVICE)) {
      ...
      retval = GRError(cudaFree((void*)d_pointer), name + " cudaFree failed",
                       __FILE__, __LINE__);
      if (retval) return retval;
      d_pointer = NULL;
      allocated = allocated - DEVICE + TARGETBASE;
  ...
  __host__ __device__ SizeT GetSize() const { return this->size; }

  cudaError_t EnsureSize(SizeT size, bool keep = false,
                         cudaStream_t stream = 0) {
    if (ARRAY_DEBUG) {
      printf("%s EnsureSize : %lld -> %lld\n", name.c_str(),
             (long long)this->size, (long long)size);
      fflush(stdout);
    }
    if (this->size >= size)
      return cudaSuccess;
      ...
      if (!keep)
        return Allocate(size, allocated);
      else {
  ...
  cudaError_t SetPointer(Value* pointer, SizeT size = -1,
    ...
    if (target == HOST) {
      ...
      if (ARRAY_DEBUG) {
        printf(
            "%s\t setted on HOST, size =\t %lld, "
            "pointer =\t %p setted = %d\n",
            name.c_str(), (long long)this->size, h_pointer, setted);
        fflush(stdout);
      }
    }
    ...
    if (target == DEVICE) {
      ...
      if (ARRAY_DEBUG) {
        printf(
            "%s\t setted on DEVICE, size =\t %lld, "
            "pointer =\t %p\n",
            name.c_str(), (long long)this->size, d_pointer);
        fflush(stdout);
      }
    }
    return retval;
  ...
  void ForceUnSetPointer(unsigned int target = HOST) {
    if ((setted & target) == target) setted = setted - target + TARGETBASE;
    if ((allocated & target) == target)
      allocated = allocated - target + TARGETBASE;

    if (target == HOST && h_pointer != NULL) {
      if (use_cuda_alloc)
        util::GRError(cudaHostUnregister((void*)h_pointer),
                      name + " cudaHostUnregister failed.", __FILE__, __LINE__);
      h_pointer = NULL;
      if (ARRAY_DEBUG) {
  ...
  cudaError_t Move(unsigned int source, unsigned int target, SizeT size = -1,
    ...
    if ((target == HOST || target == DEVICE) && ((target & setted) != target) &&
    ...
    if (source == HOST && target == DEVICE) {
      if (use_cuda_alloc && stream != 0) {
        retval =
            GRError(cudaMemcpyAsync(d_pointer + offset, h_pointer + offset,
                                    sizeof(Value) * size,
        ...
        retval =
            GRError(cudaMemcpy(d_pointer + offset, h_pointer + offset,
      ...
      if (use_cuda_alloc && stream != 0) {
        ...
        retval =
            GRError(cudaMemcpyAsync(h_pointer + offset, d_pointer + offset,
                                    sizeof(Value) * size,
        ...
        retval =
            GRError(cudaMemcpy(h_pointer + offset, d_pointer + offset,
  ...
  __host__ __device__ __forceinline__ Value& operator[](std::size_t idx) {
    ...
    if (ARRAY_DEBUG) {
      if (h_pointer == NULL)
        GRError(name + " not defined on HOST", __FILE__, __LINE__);
      if (idx >= size)
  ...
  __host__ __device__ const __forceinline__ Value& operator[](
  ...
  __host__ __device__ __forceinline__ Value* operator->() const {
    ...
    if (ARRAY_DEBUG) {
      if (h_pointer == NULL)
        GRError(name + " not deined on HOST", __FILE__, __LINE__);
