...
inline constexpr const char* arch_name(compute_capability_t capability) {
  return (capability.major == 8)
             ? "Ampere"
             : (capability.major == 7 && capability.minor == 5)
                   ? "Turing"
                   : (capability.major == 7)
                         ? "Volta"
                         : (capability.major == 6)
...
inline constexpr unsigned sm_max_shared_memory_bytes(
  ...
  unsigned sm3XConfiguredSmem =
      (sm3XCacheConfig == cudaFuncCachePreferNone)
          ? 48 * KiB
          : (sm3XCacheConfig == cudaFuncCachePreferShared)
                ? 48 * KiB
                : (sm3XCacheConfig == cudaFuncCachePreferL1)
                      ? 16 * KiB
                      : (sm3XCacheConfig == cudaFuncCachePreferEqual)
                            ? 32 * KiB
                            : 48 * KiB;
...
inline constexpr unsigned shared_memory_bank_stride() {
  ...
  return (sm3XSmemConfig == cudaSharedMemBankSizeDefault)
             ? 1 << 2
             : (sm3XSmemConfig == cudaSharedMemBankSizeFourByte)
                   ? 1 << 2
                   : (sm3XSmemConfig == cudaSharedMemBankSizeEightByte)
                         ? 1 << 3
                         : 1 << 2;
}
