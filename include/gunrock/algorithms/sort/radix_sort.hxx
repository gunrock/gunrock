...
error_t sort(const key_t* keys_in,
  ...
  if (order == order_t::ascending) {
    ...
    cub::DeviceRadixSort::SortKeys(temp_storage, temp_storage_bytes, keys,
                                   num_items);
  ...
  {
    // Determine temporary device storage requirements
    cub::DeviceRadixSort::SortKeysDescending(temp_storage, temp_storage_bytes,
                                             keys, num_items);
...
error_t sort_pairs(const key_t* keys_in,
  ...
  if (order == order_t::ascending) {
    // Determine temporary device storage requirements
    cub::DeviceRadixSort::SortPairs(temp_storage, temp_storage_bytes, keys,
                                    values, num_items);
...
error_t sort_segments(const key_t* keys_in,
  ...
  if (order == order_t::ascending) {
    // Determine temporary device storage requirements
    cub::DeviceSegmentedRadixSort::SortKeys(temp_storage, temp_storage_bytes,
  ...
  else  // order_t::descending
  {
    ...
    cub::DeviceSegmentedRadixSort::SortKeysDescending(
        temp_storage, temp_storage_bytes, num_items, num_segments, d_offsets,
        d_offsets + 1);
