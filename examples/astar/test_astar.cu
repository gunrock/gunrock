...
void DisplaySolution (VertexId *source_path, std::string *names, VertexId dst, SizeT num_nodes)
{
    cout << names[dst] << "->";
    ...
    VertexId                          dst)
{
  ...
  {
      for (SizeT e = graph.row_offsets[node];
              e < graph.row_offsets[node + 1];
              e++)
      {
...
{
  // specify some types
  typedef adjacency_list<listS, vecS, undirectedS, no_property, property<edge_weight_t, float> > mygraph_t;
  typedef property_map<mygraph_t, edge_weight_t>::type WeightMap;
  typedef mygraph_t::vertex_descriptor vertex;
  typedef mygraph_t::edge_descriptor edge_descriptor;
  typedef std::pair<int, int> edge;
...
cudaError_t RunTests(Info<VertexId, SizeT, Value> *info, Value *latitudes, Value *longitudes, std::string *names)
{
    typedef ASTARProblem < VertexId,
            SizeT,
            Value,
            true,   // always mark predecessor
            DISTANCE_HEURISTIC> Problem;
    ...
    for (int iter = 0; iter < iterations; ++iter)
    {
        ...
        if (retval = util::GRError(problem->Reset(
            src, dst, sample_weight, enactor->GetFrontierType(),
    ...
    if (!quick_mode)
    {
        ...
        if (DIRECTED)
            ReferenceAStarDirected<VertexId, SizeT, Value>(*graph,names,latitudes,longitudes,reference_preds,src,dst);
        else
            ReferenceAStarUndirected<VertexId, SizeT, Value>(*graph,names,latitudes,longitudes,reference_preds,src,dst);
    ...
    if (retval = util::GRError(problem->Extract(h_preds),
        "A* Problem Data Extraction Failed", __FILE__, __LINE__))
        return retval;

    // Verify the result
    if (!quick_mode)
    {
        if (!quiet_mode) { printf("Path Validity: "); }
    ...
    if (enactor         )
    {
        if (retval = util::GRError(enactor -> Release(),
            "A* Enactor Release failed", __FILE__, __LINE__))
            return retval;
    ...
    if (problem         )
    {
        if (retval = util::GRError(problem -> Release(),
            "A* Problem Release failed", __FILE__, __LINE__))
            return retval;
