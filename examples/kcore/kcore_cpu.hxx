...
float run(csr_t& csr,
  ...
  for (int k = 1; k < csr.number_of_rows; k++) {
    while (true) {
      ...
      if (to_be_deleted.empty()) break; //increment k when all vertices have degree > k
    ...
    if (remaining.empty()) break;   //stop when graph is empty
  }

  auto t_stop = std::chrono::high_resolution_clock::now();
  auto elapsed = std::chrono::duration_cast<std::chrono::microseconds>(t_stop - t_start).count();
