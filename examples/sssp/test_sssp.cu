...
void ReferenceSssp(const Csr<VertexId, SizeT, Value> &graph, Value *node_values,
                   VertexId *node_preds, VertexId src, bool quiet) {
...
cudaError_t RunTests(Info<VertexId, SizeT, Value> *info) {
  ...
  VertexId *reference_preds =
      MARK_PREDECESSORS ? new VertexId[graph->nodes] : NULL;
  VertexId *h_preds = MARK_PREDECESSORS ? new VertexId[graph->nodes] : NULL;
  VertexId *reference_check_pred =
      (quick_mode || !MARK_PREDECESSORS) ? NULL : reference_preds;
  ...
  if (retval = util::GRError(
          problem->Init(stream_from_host, graph, NULL, num_gpus, gpu_idx,
                        partition_method, streams, delta_factor,
                        max_queue_sizing, max_in_sizing, partition_factor,
                        partition_seed),
  ...
  if (!quick_mode) {
    ...
    ReferenceSssp<VertexId, SizeT, Value, MARK_PREDECESSORS>(
        *graph, reference_check_label, reference_check_pred, src, quiet_mode);
    if (!quiet_mode) {
      printf("\n");
    }
  }
  ...
  if (retval = util::GRError(problem->Extract(h_labels, h_preds),
                             "SSSP Problem Data Extraction Failed", __FILE__,
                             __LINE__))
    return retval;
  ...
  if (!quiet_mode) {
    if (MARK_PREDECESSORS) {
      ...
      if (reference_check_label != NULL) {
        printf(
            "\nFirst 40 preds of the reference CPU result (could be different "
            "because the paths are not unique).\n");
    ...
    max_queue_sizing_[j] = factor;
            ...
            if (num_gpus > 1 && i != 0 )
                ...
                {
                    SizeT x =
    problem->data_slices[gpu][0].keys_in[t][i].GetSize(); printf("\t %lld",
    (long long) x); double factor = 1.0 * x /
...
cudaError_t RunTests_mark_predecessors(Info<VertexId, SizeT, Value> *info) {
  if (info->info["mark_predecessors"].get_bool())
    return RunTests<VertexId, SizeT, Value,
                    /*INSTRUMENT,
