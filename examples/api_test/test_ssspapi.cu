...
int main(int argc, char *argv[]) {
  ...
  if (*argv[1] == '1') {
    ...
    double elapsed =
        sssp(num_nodes, num_edges, row_offsets, col_indices, edge_values,
             &source, mark_pred, &distances, &preds, 1 /*num_runs*/, memspace);
  ...
  if (*argv[1] == '2') {
    ...
    auto distance_ptr = device_distances.data().get();
    auto preds_ptr = device_preds.data().get();

    // See gunrock.hxx file.
    double elapsed =
        sssp(num_nodes, num_edges, device_row_offsets.data().get(),
             device_col_indices.data().get(), device_edge_values.data().get(),
             &source, mark_pred, &distance_ptr,
             &preds_ptr, 1 /*num_runs*/, memspace);
