...
struct main_struct {
  ...
  operator()(util::Parameters &parameters, VertexT v, SizeT s, ValueT val) {
    ...
    app::mf::InitReverse(d_graph, u_edge_id,
                         reverse_edges.GetPointer(util::HOST));
    ...
    if (!quick) {
      GUARD_CU(app::gtf::CPU_Reference(parameters, d_graph, u_edge_id,
                                       reverse_edges, elapsed));
      util::PrintMsg(
          "-----------------------------------\n"
          "Elapsed: " +
    ...
    GUARD_CU(app::Switch_Parameters(
        ...
        [reverse_edges](util::Parameters &parameters_, GraphT &d_graph) {
          return app::gtf::RunTests<GraphT, ValueT, VertexT, SizeT>(
              parameters_, d_graph, reverse_edges + 0);
          // return cudaSuccess;
        }));
