...
struct main_struct
{
        ...
        VertexT v, SizeT s, ValueT val)
    {
        ...
        if (!quick)
        {
            for (int i = 0; i < num_srcs; i ++)
            {
                free(ref_labels[i]); ref_labels[i] = NULL;
            }
            delete[] ref_labels; ref_labels = NULL;
        }
        return retval;
    }
};
