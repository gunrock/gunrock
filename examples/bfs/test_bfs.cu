...
void Usage() {
  printf(
      ...
      "                          If randomize2: from a different random source "
      "vertex for each iteration.\n"
      "                          If list: need to provide a source list "
      "through --source_list=n0,n1,...,nk\n"
...
void ReferenceBFS(const Csr<VertexId, SizeT, Value> *graph,
  ...
  while (!frontier.empty()) {
    ...
    for (SizeT edge = edges_begin; edge < edges_end; ++edge) {
      ...
      if (source_path[neighbor] >
          neighbor_dist)  //|| source_path[neighbor] == -1)
      {
...
cudaError_t RunTests(Info<VertexId, SizeT, Value> *info) {
  ...
  Problem *problem =
      new Problem(direction_optimized, undirected);  // allocate problem on GPU
  if (retval = util::GRError(
          problem->Init(stream_from_host, graph, inv_graph, num_gpus, gpu_idx,
  ...
  Enactor *enactor =
      new Enactor(num_gpus, gpu_idx, instrument, debug, size_check,
                  direction_optimized);  // enactor map
  if (retval = util::GRError(
  ...
  if (src_type == "list") source_list = info->info["source_list"].get_array();
  for (int iter = 0; iter < iterations; ++iter) {
    if (src_type == "random2") {
      ...
      if (source_list.size() == 0) {
        if (!quiet_mode) printf("No source list found. Use 0 as source.\n");
        src = 0;
      } else {
    ...
    if (retval = util::GRError(enactor->Reset(), "BFS Enactor Reset failed",
                               __FILE__, __LINE__))
      return retval;
    ...
    if (retval = util::GRError(enactor->Enact(src, traversal_mode),
                               "BFS Enact failed", __FILE__, __LINE__))
      return retval;
    cpu_timer.Stop();
  ...
  if (!quick_mode) {
    ...
    ReferenceBFS<VertexId, SizeT, Value, MARK_PREDECESSORS, ENABLE_IDEMPOTENCE>(
  ...
  if (retval =
          util::GRError(problem->Extract(h_labels, h_preds),
                        "BFS Problem Extraction failed", __FILE__, __LINE__))
    return retval;

  // verify the result
  if ((!quick_mode) && (!quiet_mode)) {
    printf("Label Validity: ");
    int num_errors = CompareResults(h_labels, reference_check_label,
...
#pragma omp parallel for
      for (VertexId v = 0; v < graph->nodes; v++) {
        ...
        if (v == src && h_preds[v] == util::InvalidValue<VertexId>())
          continue;  // source vertex
        ...
        if (h_labels[v] != h_labels[pred] + 1) {
          if (num_errors == 0)
            printf("INCORRECT: label[%lld] (%lld) != label[%lld] (%lld) + 1\n",
                   (long long)v, (long long)h_labels[v], (long long)pred,
                   (long long)h_labels[pred]);
#pragma omp atomic
          num_errors++;
          continue;
        }
        ...
        bool v_found = false;
        for (SizeT t = graph->row_offsets[pred];
             t < graph->row_offsets[pred + 1]; t++)
        ...
        if (!v_found) {
          if (num_errors == 0)
            printf(
                "INCORRECT: Vertex %lld not in Vertex %lld's neighbor list\n",
...
#pragma omp atomic
  ...
  if (!quick_mode && TO_TRACK) {
    ...
    if (num_gpus > 1) {
      ...
      for (int gpu = 0; gpu < num_gpus; gpu++) {
        ...
        for (VertexId v = 0; v < graph->nodes; v++) v_[gpu][v] = -1;
        for (VertexId v = 0; v < problem->sub_graphs[gpu].nodes; v++)
          v_[gpu][problem->original_vertexes[gpu][v]] = v;
      }
    }
    util::Track_Results(graph, num_gpus, (VertexId)1, h_labels,
                        reference_check_label,
                        num_gpus > 1 ? problem->partition_tables[0] : NULL, v_);
    char file_name[512];
    sprintf(file_name, "./eval/error_dump/error_%lld_%d.txt",
            (long long)time(NULL), gpu_idx[0]);
    util::Output_Errors(file_name, graph->nodes, num_gpus, (VertexId)0,
  ...
  // Clean up
  if (enactor) {
    if (retval = util::GRError(enactor->Release(), "BFS Enactor Release failed",
                               __FILE__, __LINE__))
      return retval;
    delete enactor;
    enactor = NULL;
  ...
  if (problem) {
    if (retval = util::GRError(problem->Release(), "BFS Problem Release failed",
                               __FILE__, __LINE__))
      return retval;
    delete problem;
    problem = NULL;
  ...
  if (h_preds) {
    if (info->info["output_filename"].get_str() != "") {
      ...
      for (VertexId v = 0; v < graph->nodes; v++) {
        if (v == src)
          fout << v + 1 << "," << v + 1 << std::endl;  // root node
...
cudaError_t RunTests_mark_predecessors(Info<VertexId, SizeT, Value> *info) {
  if (info->info["mark_predecessors"].get_bool())
    return RunTests_enable_idempotence<VertexId, SizeT, Value,
                                       /*INSTRUMENT,
...
int main_VertexId(CommandLineArgs *args) {
  ...
  if (args->CheckCmdLineFlag("64bit-VertexId"))
    ...
    //    {
    //        printf("64bit-VertexId disabled, because atomicMin(long long) is
    //        only supported by compute capability 3.5 or higher\n"); return 1;
    //    }
    //#else
