...
{ 
    ...
    GUARD_CU(elements.Allocate(num_elements, gunrock::util::HOST | gunrock::util::DEVICE));
    GUARD_CU(offsets .Allocate(num_segments + 1, gunrock::util::HOST | gunrock::util::DEVICE));
    GUARD_CU(results .Allocate(num_segments, gunrock::util::HOST | gunrock::util::DEVICE));
    ...
    for (int t = 0; t < num_tests; t++)
    {
        ...
        for (int i = 0; i <= num_segments; i++)
            offsets [i] = rand() % num_elements;
        std::sort(offsets + 0, offsets + num_segments);
        offsets[0] = 0; offsets[num_segments] = num_elements;
        ...
        GUARD_CU(gunrock::util::SegmentedReduce(
            temp_space, elements, results,
            num_segments, offsets,
            [] __host__ __device__ (const int &a, const int &b)
            {
                return a+b;
            }, 0, 0, false, gunrock::util::DEVICE));
        GUARD_CU2(cudaDeviceSynchronize(),
            "cudaDeviceSynchronize");
        ...
        GUARD_CU(gunrock::util::SegmentedReduce(
            ...
            {
                return a+b;
            }, 0, 0, false, gunrock::util::HOST));
        GUARD_CU2(cudaDeviceSynchronize(),
            "cudaDeviceSynchronize");

        //std::cout << "CPU Finished" << std::endl;
        for (int i = 0; i < num_segments; i++)
            h_results[i] = results[i];
        ...
        for (int i = 0; i < num_segments; i++)
        {
            if (h_results[i] == results[i])
                continue;

            if (num_errors == 0)
                std::cout << "FAIL, seed = " << seed << std::endl;
            std::cout << "Segment " << i
                << " on CPU " << h_results[i]
                << " != on GPU " << results[i]
                << ", Segment [" << offsets[i]
