...
cudaError_t RepeatForTest()
{
    ...
    //device_sfor_results.Allocate(loop_size, gunrock::util::HOST | gunrock::util::DEVICE);
    //device_rfor_results.Allocate(loop_size, gunrock::util::HOST | gunrock::util::DEVICE);
    ...
    auto op = [counter, results] __host__ __device__ 
        (const int &r, const SizeT &i)
        {
            if (i == 0)
            {
                //counter[(r + 1) % 2] = r + 1;
    ...
    std::string test_names[] = {"CPU OpenMP", "GPU Cooperactive Group", "GPU CUDA Graph", "GPU stacked kernels"};
    for (int t = 0; t < 4; t++)
    {
        ...
        { 
            gunrock::oprtr::RepeatFor(op, num_repeats, loop_size, target, stream,
        ...
        for (SizeT i = 0; i < loop_size; i++)
        {
            ...
            {
                if (num_errors == 0)
                    std::cout << " FAIL, results[" << i << "] = "
                        << results[i] << ", should be " << i + extra << std::endl;
                num_errors ++;
            }
        }
        if (num_errors == 0) 
