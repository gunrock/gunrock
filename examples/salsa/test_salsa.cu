...
void ReferenceSALSA(const Csr<VertexId, SizeT, Value> &graph,
...
void RunTests(Info<VertexId, SizeT, Value> *info) {
  ...
  typedef SALSAEnactor<Problem> Enactor;
  ...
  if (!quick_mode) {
    ...
    ReferenceSALSA(*csr, *csc, reference_check_h, reference_check_a, max_iter);
  ...
  util::GRError(enactor->Reset(), "SALSA Enactor Reset failed.", __FILE__,
                __LINE__);

  cpu_timer.Start();
  util::GRError(enactor->Enact(max_iter), "SALSA Problem Enact Failed",
                __FILE__, __LINE__);
  cpu_timer.Stop();
